...
允许访客用户通过微信一键登录转为正式用户，系统自动处理两种场景：(1) 新用户场景下直接将访客账号升级为正式账号；(2) 老用户场景下将访客期间的所有数据（血糖、饮食、运动、问诊）迁移到已存在的老账号，并软删除访客账号。整个过程在单个事务中完成，确保数据一致性，用户体验无缝。
...
- **REST** `POST /api/v1/users/guest/convert`
  ...
  - **Params**: `ConvertUserRequest { guestUserId: string, wechatOpenId: string, wechatUnionId?: string, phone?: string }`
  - **Returns**: `UserResponse { userId: string, userType: string, wechatOpenId: string (encrypted), ... }`
  - **Throws**: 
    - `400 BadRequest` - 访客用户不存在或已转正
...
1. **用户点击"微信一键登录"**：前端调用微信授权接口
2. **获取微信授权**：微信返回 OpenID 和 UnionID [FUN-004]
3. **调用转正接口**：前端携带访客令牌和微信信息调用转正接口 [FUN-005]
...
|------|----|-------|-------------|--------|
| [NEW] | FUN-004 | 微信授权信息获取 | 调用微信开放平台获取 OpenID 和 UnionID | v0.1 |
...
- **FAC-01**: Given 访客用户持有有效令牌和新的微信 OpenID，When 调用转正接口，Then 访客账号直接升级为正式账号，userType 变为 NORMAL，OpenID 已绑定
