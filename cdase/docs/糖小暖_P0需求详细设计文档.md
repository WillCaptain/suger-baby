# 糖小暖 P0 需求详细设计文档

## 文档信息
- **项目名称**: 糖小暖 (Tang Xiao Nuan)
- **版本**: v1.1 (技术优化版)
- **创建日期**: 2026-01-29
- **最后更新**: 2026-01-29 (技术架构优化)
- **技术栈**: Java + PostgreSQL + 阿里百炼
- **设计范围**: 所有 P0 优先级需求

### 📌 v1.1 更新说明
本次更新针对数字人实时交互和 AI Agent 深度集成进行了架构优化：
- ✅ 数字人对话改为 SSE 流式接口，解决长延迟问题
- ✅ 增加 Agent 工具执行器，实现 Function Calling
- ✅ 优化敏感数据加密与检索策略
- ✅ 明确访客转正的数据迁移逻辑
- ✅ 增加 OSS 文件上传模块
- ✅ 强化支付回调幂等性处理

---

## 1. 系统架构概览

### 1.1 整体架构
```
┌─────────────────────────────────────────────────────────┐
│                     前端层 (Mini Program)                 │
│  - 用户交互界面                                            │
│  - 数字人交互组件                                          │
│  - 拍照上传功能                                            │
└─────────────────────────────────────────────────────────┘
                          ↓ HTTPS/REST
┌─────────────────────────────────────────────────────────┐
│                    API 网关层                             │
│  - 身份验证                                               │
│  - 请求路由                                               │
│  - 限流熔断                                               │
└─────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────┐
│                   应用服务层 (Spring Boot)                │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐     │
│  │  用户服务    │  │  数字人服务  │  │  数据服务    │     │
│  └─────────────┘  └─────────────┘  └─────────────┘     │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐     │
│  │  问诊服务    │  │  订阅服务    │  │  安全服务    │     │
│  └─────────────┘  └─────────────┘  └─────────────┘     │
└─────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────┐
│                  数据持久层 (PostgreSQL)                   │
│  - 用户表                                                 │
│  - 血糖记录表                                             │
│  - 饮食记录表                                             │
│  - 问诊记录表                                             │
└─────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────┐
│                 外部服务集成层                             │
│  - 阿里百炼 AI 服务                                       │
│  - OSS 对象存储                                           │
│  - 支付服务                                               │
└─────────────────────────────────────────────────────────┘
```

### 1.2 技术选型说明
- **后端框架**: Spring Boot 3.x - 成熟稳定，生态完善
- **数据库**: PostgreSQL 15+ - 支持 JSONB，性能优秀
- **ORM**: MyBatis-Plus - 简化 CRUD 操作
- **AI 服务**: 阿里百炼 - 提供数字人对话能力
- **缓存**: Redis - 会话缓存和热点数据
- **消息队列**: RocketMQ - 异步任务处理

---

## 2. 核心模块设计

### 2.1 用户模块 (User Module)

#### 2.1.1 功能描述
负责用户身份管理、免登录模式、个人信息管理和病例信息管理。

#### 2.1.2 类设计

**UserController.java**
```java
package com.tangxiaonuan.user.controller;

/**
 * 用户控制器
 * 负责处理用户相关的所有 HTTP 请求
 */
@RestController
@RequestMapping("/api/v1/users")
public class UserController {
    
    /**
     * 免登录模式访客注册
     * 生成临时访客身份，用于 showcase 演示
     */
    @PostMapping("/guest/register")
    public ResponseEntity<GuestTokenResponse> registerGuest();
    
    /**
     * 访客转正式用户
     * 将访客账号转换为正式用户账号
     */
    @PostMapping("/guest/convert")
    public ResponseEntity<UserResponse> convertGuestToUser(@RequestBody ConvertUserRequest request);
    
    /**
     * 获取用户基本信息
     */
    @GetMapping("/{userId}")
    public ResponseEntity<UserResponse> getUserInfo(@PathVariable String userId);
    
    /**
     * 更新用户基本信息
     * 包括姓名、性别、年龄、联系方式等
     */
    @PutMapping("/{userId}/profile")
    public ResponseEntity<UserResponse> updateUserProfile(
        @PathVariable String userId,
        @RequestBody UpdateProfileRequest request
    );
    
    /**
     * 更新用户病例信息
     * 包括患病时间、血压、病史、用药情况等
     */
    @PutMapping("/{userId}/medical-history")
    public ResponseEntity<MedicalHistoryResponse> updateMedicalHistory(
        @PathVariable String userId,
        @RequestBody UpdateMedicalHistoryRequest request
    );
    
    /**
     * 获取用户病例信息
     */
    @GetMapping("/{userId}/medical-history")
    public ResponseEntity<MedicalHistoryResponse> getMedicalHistory(@PathVariable String userId);
    
    /**
     * 同意隐私协议
     */
    @PostMapping("/{userId}/privacy-agreement")
    public ResponseEntity<Void> agreePrivacyPolicy(
        @PathVariable String userId,
        @RequestBody PrivacyAgreementRequest request
    );
}
```

**UserService.java**
```java
package com.tangxiaonuan.user.service;

/**
 * 用户服务接口
 * 定义用户业务逻辑
 */
public interface UserService {
    
    /**
     * 创建访客用户
     * @return 访客令牌和用户ID
     */
    GuestTokenResponse createGuestUser();
    
    /**
     * 访客转正
     * @param guestUserId 访客用户ID
     * @param request 转正请求信息
     * @return 正式用户信息
     */
    UserResponse convertGuestToUser(String guestUserId, ConvertUserRequest request);
    
    /**
     * 获取用户信息
     * @param userId 用户ID
     * @return 用户信息，包含加密字段的解密结果
     */
    UserResponse getUserById(String userId);
    
    /**
     * 更新用户基本信息
     * @param userId 用户ID
     * @param request 更新请求
     * @return 更新后的用户信息
     */
    UserResponse updateUserProfile(String userId, UpdateProfileRequest request);
    
    /**
     * 更新病例信息
     * @param userId 用户ID
     * @param request 病例更新请求
     * @return 病例信息
     */
    MedicalHistoryResponse updateMedicalHistory(String userId, UpdateMedicalHistoryRequest request);
    
    /**
     * 获取病例信息
     * @param userId 用户ID
     * @return 病例信息
     */
    MedicalHistoryResponse getMedicalHistory(String userId);
    
    /**
     * 记录隐私协议同意状态
     * @param userId 用户ID
     * @param agreementVersion 协议版本
     */
    void recordPrivacyAgreement(String userId, String agreementVersion);
}
```

**UserServiceImpl.java**
```java
package com.tangxiaonuan.user.service.impl;

/**
 * 用户服务实现
 */
@Service
public class UserServiceImpl implements UserService {
    
    @Autowired
    private UserMapper userMapper;
    
    @Autowired
    private MedicalHistoryMapper medicalHistoryMapper;
    
    @Autowired
    private EncryptionService encryptionService;
    
    @Autowired
    private PrivacyAgreementMapper privacyAgreementMapper;
    
    @Override
    public GuestTokenResponse createGuestUser() {
        // 生成访客ID和临时令牌
        // 创建访客用户记录
        // 返回令牌信息
    }
    
    @Override
    @Transactional(rollbackFor = Exception.class)
    public UserResponse convertGuestToUser(String guestUserId, ConvertUserRequest request) {
        /**
         * 🔥 访客转正逻辑（核心业务）
         * 
         * 场景说明：
         * 用户以访客身份（Guest_ID_123）体验了3天，记录了血糖、饮食等数据。
         * 现在点击"微信一键登录"，微信返回 OpenID_456。
         * 
         * 需要处理两种情况：
         * 1. OpenID_456 是新用户 -> 将访客账号升级为正式账号
         * 2. OpenID_456 对应已存在的老账号 -> 数据迁移+合并
         */
        
        // 1. 验证访客用户存在
        User guestUser = userMapper.selectByUserId(guestUserId);
        if (guestUser == null || !guestUser.getUserType().equals("guest")) {
            throw new BusinessException("访客用户不存在或已转正");
        }
        
        // 2. 根据 OpenID 查询是否存在正式用户
        String openId = request.getWechatOpenId();
        User existingUser = userMapper.selectByWechatOpenId(openId);
        
        if (existingUser == null) {
            // 策略 A：OpenID 是新用户，直接升级访客账号
            guestUser.setUserType("normal");
            guestUser.setWechatOpenId(openId);
            guestUser.setWechatUnionid(request.getWechatUnionId());
            
            // 加密并保存手机号（如果提供）
            if (StringUtils.isNotBlank(request.getPhone())) {
                guestUser.setEncryptedPhone(encryptionService.encrypt(request.getPhone()));
                guestUser.setPhoneHash(generateHash(request.getPhone()));
            }
            
            userMapper.updateByUserId(guestUser);
            
            log.info("访客转正成功（新用户）：guestUserId={}, openId={}", guestUserId, openId);
            return buildUserResponse(guestUser);
            
        } else {
            // 策略 B：OpenID 对应已存在的老账号，需要数据迁移
            log.info("检测到老账号，开始数据迁移：guestUserId={}, existingUserId={}", guestUserId, existingUser.getUserId());
            
            // 2.1 迁移血糖记录
            int glucoseCount = glucoseRecordMapper.updateUserIdByUserId(guestUserId, existingUser.getUserId());
            
            // 2.2 迁移饮食记录
            int dietCount = dietRecordMapper.updateUserIdByUserId(guestUserId, existingUser.getUserId());
            
            // 2.3 迁移运动记录
            int exerciseCount = exerciseRecordMapper.updateUserIdByUserId(guestUserId, existingUser.getUserId());
            
            // 2.4 迁移问诊记录
            int consultationCount = consultationMapper.updateUserIdByUserId(guestUserId, existingUser.getUserId());
            
            // 2.5 软删除访客账号
            guestUser.setIsDeleted(true);
            userMapper.updateByUserId(guestUser);
            
            log.info("数据迁移完成：血糖{}条，饮食{}条，运动{}条，问诊{}条", 
                glucoseCount, dietCount, exerciseCount, consultationCount);
            
            return buildUserResponse(existingUser);
        }
    }
    
    /**
     * 生成哈希值（用于检索）
     * @param plainText 明文
     * @return SHA256(plainText + global_salt)
     */
    private String generateHash(String plainText) {
        String saltedText = plainText + globalSalt; // globalSalt 从配置中心读取
        return DigestUtils.sha256Hex(saltedText);
    }
    
    @Override
    public UserResponse getUserById(String userId) {
        // 查询用户信息
        // 解密敏感字段
        // 返回脱敏后的用户信息
    }
    
    @Override
    public UserResponse updateUserProfile(String userId, UpdateProfileRequest request) {
        // 验证用户存在
        // 加密敏感字段（姓名、电话、身份证等）
        // 更新用户信息
        // 返回更新结果
    }
    
    @Override
    public MedicalHistoryResponse updateMedicalHistory(String userId, UpdateMedicalHistoryRequest request) {
        // 验证用户存在
        // 加密病例敏感信息
        // 更新或创建病例记录
        // 返回病例信息
    }
    
    @Override
    public MedicalHistoryResponse getMedicalHistory(String userId) {
        // 查询病例信息
        // 解密敏感字段
        // 返回病例信息
    }
    
    @Override
    public void recordPrivacyAgreement(String userId, String agreementVersion) {
        // 记录用户同意隐私协议的时间和版本
        // 保存到数据库
    }
}
```

**UserMapper.java**
```java
package com.tangxiaonuan.user.mapper;

/**
 * 用户数据访问层
 */
@Mapper
public interface UserMapper extends BaseMapper<User> {
    
    /**
     * 根据用户ID查询用户
     */
    User selectByUserId(String userId);
    
    /**
     * 更新用户信息
     */
    int updateByUserId(User user);
    
    /**
     * 创建用户
     */
    int insert(User user);
}
```

#### 2.1.3 数据模型

**User (用户实体)**
```java
package com.tangxiaonuan.user.entity;

@Data
@TableName("tbl_user")
public class User {
    @TableId(type = IdType.ASSIGN_UUID)
    private String userId;              // 用户ID
    private String userType;            // 用户类型：guest/normal
    private String encryptedName;       // 加密的姓名
    private String gender;              // 性别
    private Integer age;                // 年龄
    private String encryptedPhone;      // 加密的手机号
    private String encryptedIdCard;     // 加密的身份证号
    private String avatar;              // 头像URL
    private LocalDateTime createdAt;    // 创建时间
    private LocalDateTime updatedAt;    // 更新时间
    private Boolean isDeleted;          // 逻辑删除标记
}
```

**MedicalHistory (病例实体)**
```java
package com.tangxiaonuan.user.entity;

@Data
@TableName("tbl_medical_history")
public class MedicalHistory {
    @TableId(type = IdType.ASSIGN_UUID)
    private String historyId;           // 病例ID
    private String userId;              // 用户ID
    private LocalDate diagnosisDate;    // 患病时间
    private String diabetesType;        // 糖尿病类型：1型/2型
    private Integer systolicPressure;   // 收缩压
    private Integer diastolicPressure;  // 舒张压
    private String complications;       // 并发症（JSON）
    private Boolean isOnMedication;     // 是否用药
    private String medicationDetails;   // 用药详情（JSON）
    private Boolean isUsingInsulin;     // 是否使用胰岛素
    private String insulinDetails;      // 胰岛素详情（JSON）
    private String otherDiseases;       // 其他疾病（JSON）
    private LocalDateTime createdAt;    // 创建时间
    private LocalDateTime updatedAt;    // 更新时间
}
```

**PrivacyAgreement (隐私协议同意记录)**
```java
package com.tangxiaonuan.user.entity;

@Data
@TableName("tbl_privacy_agreement")
public class PrivacyAgreement {
    @TableId(type = IdType.ASSIGN_UUID)
    private String agreementId;         // 记录ID
    private String userId;              // 用户ID
    private String agreementVersion;    // 协议版本
    private LocalDateTime agreedAt;     // 同意时间
    private String ipAddress;           // 同意时的IP地址
    private String deviceInfo;          // 设备信息
}
```

#### 2.1.4 设计理由

1. **免登录模式**: 采用访客令牌机制，降低用户体验门槛，适合 showcase 演示场景。访客用户可以快速体验核心功能，后续可转为正式用户。

2. **数据加密**: 用户姓名、手机号、身份证等敏感信息使用 AES-256 加密存储，符合《个人信息保护法》要求。

3. **病例信息结构化**: 将病例信息独立为单表，支持历史记录追溯，便于医生查看完整病史。

4. **隐私协议版本化**: 记录用户同意的具体协议版本和时间，满足合规要求。

---

### 2.2 数字人模块 (Digital Human Module)

#### 2.2.1 功能描述
集成阿里百炼 AI 服务，提供徐医生数字人对话能力，引导用户录入信息、记录数据、进行问诊。

#### 2.2.2 类设计

**DigitalHumanController.java**
```java
package com.tangxiaonuan.digitalhuman.controller;

/**
 * 数字人交互控制器
 */
@RestController
@RequestMapping("/api/v1/digital-human")
public class DigitalHumanController {
    
    /**
     * 初始化对话会话
     * 创建新的对话上下文
     */
    @PostMapping("/sessions")
    public ResponseEntity<SessionResponse> createSession(@RequestBody CreateSessionRequest request);
    
    /**
     * 🔥 核心优化：流式发送消息给数字人（SSE）
     * 解决大模型推理 + TTS 延迟问题，支持打字机效果和语音流式播放
     * 
     * 流式输出顺序：
     * 1. event: thinking -> data: {"status": "thinking"}
     * 2. event: text_chunk -> data: {"text": "您好..."}（逐字推送）
     * 3. event: audio_chunk -> data: {"audioUrl": "oss://xxx-1.mp3", "duration": 2.5}
     * 4. event: tool_call -> data: {"toolName": "save_blood_sugar", "params": {...}}
     * 5. event: tool_result -> data: {"result": "记录成功"}
     * 6. event: done -> data: {"messageId": "msg-xxx"}
     * 
     * 兼容性说明：微信小程序原生支持通过 wx.request 的 enableChunked 参数接收 SSE
     */
    @PostMapping(value = "/sessions/{sessionId}/messages/stream", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public SseEmitter streamMessage(
        @PathVariable String sessionId,
        @RequestBody SendMessageRequest request
    );
    
    /**
     * 降级接口：同步发送消息（非流式）
     * 用于不支持 SSE 的客户端或简单场景
     */
    @PostMapping("/sessions/{sessionId}/messages")
    public ResponseEntity<MessageResponse> sendMessage(
        @PathVariable String sessionId,
        @RequestBody SendMessageRequest request
    );
    
    /**
     * 获取对话历史
     */
    @GetMapping("/sessions/{sessionId}/messages")
    public ResponseEntity<List<MessageResponse>> getSessionMessages(
        @PathVariable String sessionId,
        @RequestParam(defaultValue = "0") int page,
        @RequestParam(defaultValue = "20") int size
    );
    
    /**
     * 获取数字人引导流程状态
     * 例如：个人信息录入进度、病例信息录入进度
     */
    @GetMapping("/sessions/{sessionId}/guidance-status")
    public ResponseEntity<GuidanceStatusResponse> getGuidanceStatus(@PathVariable String sessionId);
    
    /**
     * 结束对话会话
     */
    @DeleteMapping("/sessions/{sessionId}")
    public ResponseEntity<Void> endSession(@PathVariable String sessionId);
}
```

**DigitalHumanService.java**
```java
package com.tangxiaonuan.digitalhuman.service;

/**
 * 数字人服务接口
 */
public interface DigitalHumanService {
    
    /**
     * 创建对话会话
     * @param userId 用户ID
     * @param sceneType 场景类型：profile_input/medical_input/glucose_record/diet_record/consultation
     * @return 会话信息
     */
    SessionResponse createSession(String userId, String sceneType);
    
    /**
     * 🔥 流式处理用户消息（核心方法）
     * @param sessionId 会话ID
     * @param request 消息请求
     * @param emitter SSE 发射器
     * 
     * 处理流程：
     * 1. 获取对话上下文
     * 2. 调用百炼流式 API
     * 3. 检测 Function Call，如果有则调用 AgentToolExecutor
     * 4. 将文本逐块推送给前端
     * 5. 调用 TTS 生成音频（分段生成，边生成边推送）
     * 6. 保存完整消息到数据库
     * 7. 发送 [DONE] 标记
     */
    void processMessageStream(String sessionId, SendMessageRequest request, SseEmitter emitter);
    
    /**
     * 同步处理用户消息（降级方法）
     * @param sessionId 会话ID
     * @param request 消息请求
     * @return 数字人回复
     */
    MessageResponse processMessage(String sessionId, SendMessageRequest request);
    
    /**
     * 获取会话消息历史
     * @param sessionId 会话ID
     * @param page 页码
     * @param size 每页大小
     * @return 消息列表
     */
    List<MessageResponse> getSessionMessages(String sessionId, int page, int size);
    
    /**
     * 获取引导状态
     * @param sessionId 会话ID
     * @return 引导进度状态
     */
    GuidanceStatusResponse getGuidanceStatus(String sessionId);
    
    /**
     * 结束会话
     * @param sessionId 会话ID
     */
    void endSession(String sessionId);
}
```

**BailianAIClient.java**

```java
package com.tangxiaonuan.digitalhuman.client;

/**
 * 阿里百炼 AI 客户端
 * 封装对阿里百炼 API 的调用
 */
@Component
public class BailianAIClient {
    
    @Value("${bailianai.api.key}")
    private String apiKey;
    
    @Value("${bailianai.api.endpoint}")
    private String endpoint;
    
    /**
     * 调用百炼对话接口
     * @param prompt 用户输入
     * @param context 对话上下文
     * @param knowledgeBase 知识库ID
     * @return AI 回复
     */
    public BailianResponse chat(String prompt, List<Message> context, String knowledgeBase) {
        // 构建请求
        // 调用百炼 API
        // 解析响应
        // 返回结果
    }
    
    /**
     * 提取结构化信息
     * 从对话中提取用户信息、血糖数据等
     * @param conversation 对话内容
     * @param extractionType 提取类型
     * @return 结构化数据
     */
    public Map<String, Object> extractStructuredData(List<Message> conversation, String extractionType) {
        // 使用 AI 能力提取结构化信息
        // 返回 JSON 格式数据
    }
    
    /**
     * 语音转文字
     * @param audioUrl 音频文件URL
     * @return 文字内容
     */
    public String speechToText(String audioUrl) {
        // 调用语音识别接口
        // 返回文字结果
    }
    
    /**
     * 文字转语音
     * @param text 文字内容
     * @param voiceId 声音ID（徐医生声音）
     * @return 音频文件URL
     */
    public String textToSpeech(String text, String voiceId) {
        // 调用语音合成接口
        // 返回音频URL
    }
}
```

**AgentToolExecutor.java**
```java
package com.tangxiaonuan.digitalhuman.agent;

/**
 * 🔥 Agent 工具执行器（核心优化）
 * 处理阿里百炼 Agent 的 Function Calling
 * 
 * 工作原理：
 * 1. 百炼 Agent 在配置时定义了若干工具（Tool），如 save_blood_sugar、save_diet 等
 * 2. 用户说"我刚测了血糖 6.5"时，Agent 会返回 function_call 指令
 * 3. 本类解析 function_call，路由到对应的业务方法
 * 4. 执行完毕后，将结果返回给 Agent，Agent 生成最终回复
 * 
 * 设计模式：策略模式 + 反射/注解路由
 */
@Service
@Slf4j
public class AgentToolExecutor {
    
    @Autowired
    private GlucoseService glucoseService;
    
    @Autowired
    private DietService dietService;
    
    @Autowired
    private ExerciseService exerciseService;
    
    @Autowired
    private UserService userService;
    
    /**
     * 执行工具调用
     * @param toolName 工具名称（百炼返回的 function name）
     * @param params 参数（JSON 格式）
     * @param userId 用户ID
     * @return 执行结果（返回给 Agent）
     */
    public ToolExecutionResult executeTool(String toolName, Map<String, Object> params, String userId) {
        // 根据 toolName 路由到具体方法
        // 返回执行结果
    }
    
    /**
     * 工具：保存血糖记录
     * 对应百炼 Agent 配置中的 save_blood_sugar 工具
     */
    @AgentTool(name = "save_blood_sugar", description = "保存用户的血糖测量记录")
    public String saveBloodSugar(
        @Param("userId") String userId,
        @Param("value") Double glucoseValue,
        @Param("measurementType") String measurementType,
        @Param("measurementTime") String measurementTimeStr
    ) {
        try {
            // 1. 解析时间
            LocalDateTime measurementTime = parseTimeExpression(measurementTimeStr); // 支持"刚才"、"早上8点"等自然语言
            
            // 2. 调用 Service 保存
            CreateGlucoseRecordRequest request = new CreateGlucoseRecordRequest();
            request.setUserId(userId);
            request.setGlucoseValue(new BigDecimal(glucoseValue));
            request.setMeasurementType(measurementType);
            request.setMeasurementTime(measurementTime);
            
            GlucoseRecordResponse response = glucoseService.createRecord(request);
            
            // 3. 返回友好的文本结果给 Agent
            return String.format("已成功记录血糖值 %.1f mmol/L（%s）。", glucoseValue, getMeasurementTypeText(measurementType));
            
        } catch (Exception e) {
            log.error("保存血糖记录失败", e);
            return "抱歉，记录失败了，请稍后重试。";
        }
    }
    
    /**
     * 工具：保存饮食记录
     * 对应百炼 Agent 配置中的 save_diet 工具
     */
    @AgentTool(name = "save_diet", description = "保存用户的饮食记录")
    public String saveDiet(
        @Param("userId") String userId,
        @Param("mealType") String mealType,
        @Param("foodItems") List<Map<String, Object>> foodItems,
        @Param("mealTime") String mealTimeStr
    ) {
        try {
            // 1. 解析食物清单
            List<FoodItem> foods = parseFoodItems(foodItems);
            
            // 2. 计算总营养成分
            NutritionInfo totalNutrition = calculateTotalNutrition(foods);
            
            // 3. 保存记录
            CreateDietRecordRequest request = new CreateDietRecordRequest();
            request.setUserId(userId);
            request.setMealType(mealType);
            request.setFoodItems(foods);
            request.setTotalCalories(totalNutrition.getCalories());
            request.setCarbohydrate(totalNutrition.getCarbohydrate());
            request.setProtein(totalNutrition.getProtein());
            request.setFat(totalNutrition.getFat());
            
            DietRecordResponse response = dietService.createRecord(request);
            
            // 4. 返回结果给 Agent
            return String.format("已记录您的%s，总热量约 %.0f 千卡。", getMealTypeText(mealType), totalNutrition.getCalories());
            
        } catch (Exception e) {
            log.error("保存饮食记录失败", e);
            return "记录失败，请稍后重试。";
        }
    }
    
    /**
     * 工具：保存运动记录
     */
    @AgentTool(name = "save_exercise", description = "保存用户的运动记录")
    public String saveExercise(
        @Param("userId") String userId,
        @Param("exerciseType") String exerciseType,
        @Param("duration") Integer duration,
        @Param("distance") Double distance
    ) {
        // 实现逻辑类似
    }
    
    /**
     * 工具：更新用户个人信息
     */
    @AgentTool(name = "update_user_profile", description = "更新用户的个人信息")
    public String updateUserProfile(
        @Param("userId") String userId,
        @Param("name") String name,
        @Param("age") Integer age,
        @Param("gender") String gender
    ) {
        // 实现逻辑
    }
    
    /**
     * 工具：更新病例信息
     */
    @AgentTool(name = "update_medical_history", description = "更新用户的病例信息")
    public String updateMedicalHistory(
        @Param("userId") String userId,
        @Param("diagnosisDate") String diagnosisDate,
        @Param("diabetesType") String diabetesType,
        @Param("isOnMedication") Boolean isOnMedication
    ) {
        // 实现逻辑
    }
    
    /**
     * 工具：查询今日血糖数据
     */
    @AgentTool(name = "query_today_glucose", description = "查询用户今日的血糖记录")
    public String queryTodayGlucose(@Param("userId") String userId) {
        // 查询并返回结构化文本
    }
    
    /**
     * 工具：查询今日饮食摄入
     */
    @AgentTool(name = "query_today_diet", description = "查询用户今日的饮食摄入情况")
    public String queryTodayDiet(@Param("userId") String userId) {
        // 查询并返回结构化文本
    }
    
    // ========== 私有辅助方法 ==========
    
    /**
     * 解析时间表达式（自然语言转 LocalDateTime）
     */
    private LocalDateTime parseTimeExpression(String timeStr) {
        // "刚才" -> 当前时间
        // "早上8点" -> 今天 08:00
        // "昨天晚上" -> 昨天 20:00
        // 使用 NLP 或规则引擎解析
    }
    
    /**
     * 解析食物清单
     */
    private List<FoodItem> parseFoodItems(List<Map<String, Object>> rawItems) {
        // 将 Map 转为 FoodItem 对象
    }
    
    /**
     * 计算总营养成分
     */
    private NutritionInfo calculateTotalNutrition(List<FoodItem> foods) {
        // 累加各食物的营养成分
    }
}

/**
 * Agent 工具注解
 * 用于标记可被 Agent 调用的方法
 */
@Target(ElementType.METHOD)
@Retention(RetentionPolicy.RUNTIME)
public @interface AgentTool {
    String name();
    String description();
}

/**
 * 工具参数注解
 */
@Target(ElementType.PARAMETER)
@Retention(RetentionPolicy.RUNTIME)
public @interface Param {
    String value();
}

/**
 * 工具执行结果
 */
@Data
public class ToolExecutionResult {
    private boolean success;
    private String result;          // 返回给 Agent 的文本
    private Map<String, Object> data; // 结构化数据（可选）
}
```

**ConversationContextManager.java**
```java
package com.tangxiaonuan.digitalhuman.service;

/**
 * 对话上下文管理器
 * 管理对话历史和场景状态
 */
@Service
public class ConversationContextManager {
    
    @Autowired
    private RedisTemplate<String, Object> redisTemplate;
    
    /**
     * 保存对话上下文
     * @param sessionId 会话ID
     * @param context 上下文信息
     */
    public void saveContext(String sessionId, ConversationContext context);
    
    /**
     * 获取对话上下文
     * @param sessionId 会话ID
     * @return 上下文信息
     */
    public ConversationContext getContext(String sessionId);
    
    /**
     * 添加消息到上下文
     * @param sessionId 会话ID
     * @param message 消息
     */
    public void appendMessage(String sessionId, Message message);
    
    /**
     * 更新引导状态
     * @param sessionId 会话ID
     * @param step 当前步骤
     * @param data 已收集的数据
     */
    public void updateGuidanceState(String sessionId, String step, Map<String, Object> data);
    
    /**
     * 清除会话上下文
     * @param sessionId 会话ID
     */
    public void clearContext(String sessionId);
}
```

#### 2.2.3 数据模型

**Session (会话实体)**
```java
package com.tangxiaonuan.digitalhuman.entity;

@Data
@TableName("tbl_conversation_session")
public class Session {
    @TableId(type = IdType.ASSIGN_UUID)
    private String sessionId;           // 会话ID
    private String userId;              // 用户ID
    private String sceneType;           // 场景类型
    private String status;              // 状态：active/completed/expired
    private LocalDateTime startTime;    // 开始时间
    private LocalDateTime endTime;      // 结束时间
    private Integer messageCount;       // 消息数量
}
```

**Message (消息实体)**
```java
package com.tangxiaonuan.digitalhuman.entity;

@Data
@TableName("tbl_conversation_message")
public class Message {
    @TableId(type = IdType.ASSIGN_UUID)
    private String messageId;           // 消息ID
    private String sessionId;           // 会话ID
    private String role;                // 角色：user/assistant
    private String contentType;         // 内容类型：text/audio/image
    private String content;             // 消息内容
    private String audioUrl;            // 音频URL（如果是语音消息）
    private Map<String, Object> metadata; // 元数据（JSON）
    private LocalDateTime createdAt;    // 创建时间
}
```

**GuidanceProgress (引导进度实体)**
```java
package com.tangxiaonuan.digitalhuman.entity;

@Data
@TableName("tbl_guidance_progress")
public class GuidanceProgress {
    @TableId(type = IdType.ASSIGN_UUID)
    private String progressId;          // 进度ID
    private String sessionId;           // 会话ID
    private String userId;              // 用户ID
    private String sceneType;           // 场景类型
    private String currentStep;         // 当前步骤
    private Map<String, Object> collectedData; // 已收集数据（JSON）
    private Integer completionRate;     // 完成百分比
    private LocalDateTime updatedAt;    // 更新时间
}
```

#### 2.2.4 设计理由

1. **会话管理**: 每次对话创建独立会话，支持多场景并行，避免上下文混淆。

2. **上下文缓存**: 使用 Redis 缓存对话上下文，提升响应速度，支持高并发场景。

3. **场景区分**: 不同场景（录入信息、记录数据、问诊）使用不同的提示词和知识库，提高 AI 响应准确性。

4. **结构化提取**: AI 对话后自动提取结构化信息，减少用户手动输入工作量。

---

### 2.3 血糖管理模块 (Glucose Management Module)

#### 2.3.1 功能描述
记录和展示用户血糖数据，支持手动录入和数字人引导录入。

#### 2.3.2 类设计

**GlucoseController.java**
```java
package com.tangxiaonuan.glucose.controller;

/**
 * 血糖管理控制器
 */
@RestController
@RequestMapping("/api/v1/glucose")
public class GlucoseController {
    
    /**
     * 创建血糖记录
     * 支持手动录入和数字人引导录入
     */
    @PostMapping("/records")
    public ResponseEntity<GlucoseRecordResponse> createRecord(@RequestBody CreateGlucoseRecordRequest request);
    
    /**
     * 批量创建血糖记录
     * 支持从设备导入
     */
    @PostMapping("/records/batch")
    public ResponseEntity<BatchCreateResponse> batchCreateRecords(@RequestBody List<CreateGlucoseRecordRequest> requests);
    
    /**
     * 获取用户血糖记录列表
     */
    @GetMapping("/records")
    public ResponseEntity<PageResponse<GlucoseRecordResponse>> getRecords(
        @RequestParam String userId,
        @RequestParam(required = false) LocalDate startDate,
        @RequestParam(required = false) LocalDate endDate,
        @RequestParam(defaultValue = "0") int page,
        @RequestParam(defaultValue = "20") int size
    );
    
    /**
     * 获取最新血糖记录
     */
    @GetMapping("/records/latest")
    public ResponseEntity<GlucoseRecordResponse> getLatestRecord(@RequestParam String userId);
    
    /**
     * 获取血糖统计信息
     * 包括平均值、最高值、最低值等
     */
    @GetMapping("/statistics")
    public ResponseEntity<GlucoseStatisticsResponse> getStatistics(
        @RequestParam String userId,
        @RequestParam(required = false) LocalDate startDate,
        @RequestParam(required = false) LocalDate endDate
    );
    
    /**
     * 删除血糖记录
     */
    @DeleteMapping("/records/{recordId}")
    public ResponseEntity<Void> deleteRecord(@PathVariable String recordId);
}
```

**GlucoseService.java**
```java
package com.tangxiaonuan.glucose.service;

/**
 * 血糖服务接口
 */
public interface GlucoseService {
    
    /**
     * 创建血糖记录
     * @param request 血糖记录请求
     * @return 创建的记录
     */
    GlucoseRecordResponse createRecord(CreateGlucoseRecordRequest request);
    
    /**
     * 批量创建血糖记录
     * @param requests 批量请求
     * @return 创建结果
     */
    BatchCreateResponse batchCreateRecords(List<CreateGlucoseRecordRequest> requests);
    
    /**
     * 获取血糖记录列表
     * @param userId 用户ID
     * @param startDate 开始日期
     * @param endDate 结束日期
     * @param page 页码
     * @param size 每页大小
     * @return 记录列表
     */
    PageResponse<GlucoseRecordResponse> getRecords(String userId, LocalDate startDate, LocalDate endDate, int page, int size);
    
    /**
     * 获取最新血糖值
     * @param userId 用户ID
     * @return 最新记录
     */
    GlucoseRecordResponse getLatestRecord(String userId);
    
    /**
     * 计算血糖统计信息
     * @param userId 用户ID
     * @param startDate 开始日期
     * @param endDate 结束日期
     * @return 统计信息
     */
    GlucoseStatisticsResponse calculateStatistics(String userId, LocalDate startDate, LocalDate endDate);
    
    /**
     * 删除血糖记录
     * @param recordId 记录ID
     */
    void deleteRecord(String recordId);
}
```

#### 2.3.3 数据模型

**GlucoseRecord (血糖记录实体)**
```java
package com.tangxiaonuan.glucose.entity;

@Data
@TableName("tbl_glucose_record")
public class GlucoseRecord {
    @TableId(type = IdType.ASSIGN_UUID)
    private String recordId;            // 记录ID
    private String userId;              // 用户ID
    private BigDecimal glucoseValue;    // 血糖值 (mmol/L)
    private String measurementType;     // 测量类型：fasting/before_meal/after_meal/before_sleep/random
    private LocalDateTime measurementTime; // 测量时间
    private String deviceInfo;          // 设备信息
    private String notes;               // 备注
    private String sessionId;           // 对话会话ID（数字人录入时关联）
    private LocalDateTime createdAt;    // 创建时间
}
```

#### 2.3.4 设计理由

1. **测量类型区分**: 区分空腹、餐前、餐后等不同场景的血糖值，便于医生分析血糖波动规律。

2. **设备信息记录**: 记录测量设备信息，支持未来多设备接入场景。

3. **统计计算**: 提供统计接口，计算平均值、标准差等指标，辅助血糖管理。

---

### 2.4 饮食管理模块 (Diet Management Module)

#### 2.4.1 功能描述
记录用户饮食信息，支持拍照识别和数字人引导录入，计算今日摄入量。

#### 2.4.2 类设计

**DietController.java**
```java
package com.tangxiaonuan.diet.controller;

/**
 * 饮食管理控制器
 */
@RestController
@RequestMapping("/api/v1/diet")
public class DietController {
    
    /**
     * 创建饮食记录（文字描述）
     */
    @PostMapping("/records")
    public ResponseEntity<DietRecordResponse> createRecord(@RequestBody CreateDietRecordRequest request);
    
    /**
     * 拍照识别饮食
     * 上传图片，AI 识别食物并创建记录
     */
    @PostMapping("/records/photo")
    public ResponseEntity<DietRecordResponse> createRecordByPhoto(@RequestParam MultipartFile photo, @RequestParam String userId);
    
    /**
     * 获取饮食记录列表
     */
    @GetMapping("/records")
    public ResponseEntity<PageResponse<DietRecordResponse>> getRecords(
        @RequestParam String userId,
        @RequestParam(required = false) LocalDate startDate,
        @RequestParam(required = false) LocalDate endDate,
        @RequestParam(defaultValue = "0") int page,
        @RequestParam(defaultValue = "20") int size
    );
    
    /**
     * 获取今日饮食摄入统计
     * 包括总热量、碳水、蛋白质、脂肪等
     */
    @GetMapping("/today-intake")
    public ResponseEntity<DailyIntakeResponse> getTodayIntake(@RequestParam String userId);
    
    /**
     * 更新饮食记录
     */
    @PutMapping("/records/{recordId}")
    public ResponseEntity<DietRecordResponse> updateRecord(
        @PathVariable String recordId,
        @RequestBody UpdateDietRecordRequest request
    );
    
    /**
     * 删除饮食记录
     */
    @DeleteMapping("/records/{recordId}")
    public ResponseEntity<Void> deleteRecord(@PathVariable String recordId);
}
```

**DietService.java**
```java
package com.tangxiaonuan.diet.service;

/**
 * 饮食服务接口
 */
public interface DietService {
    
    /**
     * 创建饮食记录
     * @param request 饮食记录请求
     * @return 创建的记录
     */
    DietRecordResponse createRecord(CreateDietRecordRequest request);
    
    /**
     * 通过照片创建饮食记录
     * @param photo 食物照片
     * @param userId 用户ID
     * @return 识别并创建的记录
     */
    DietRecordResponse createRecordByPhoto(MultipartFile photo, String userId);
    
    /**
     * 获取饮食记录列表
     * @param userId 用户ID
     * @param startDate 开始日期
     * @param endDate 结束日期
     * @param page 页码
     * @param size 每页大小
     * @return 记录列表
     */
    PageResponse<DietRecordResponse> getRecords(String userId, LocalDate startDate, LocalDate endDate, int page, int size);
    
    /**
     * 计算今日摄入量
     * @param userId 用户ID
     * @return 今日营养摄入统计
     */
    DailyIntakeResponse calculateTodayIntake(String userId);
    
    /**
     * 更新饮食记录
     * @param recordId 记录ID
     * @param request 更新请求
     * @return 更新后的记录
     */
    DietRecordResponse updateRecord(String recordId, UpdateDietRecordRequest request);
    
    /**
     * 删除饮食记录
     * @param recordId 记录ID
     */
    void deleteRecord(String recordId);
}
```

**FoodRecognitionService.java**
```java
package com.tangxiaonuan.diet.service;

/**
 * 食物识别服务
 * 使用阿里百炼视觉能力识别食物
 */
@Service
public class FoodRecognitionService {
    
    @Autowired
    private BailianAIClient bailianAIClient;
    
    @Autowired
    private OssService ossService;
    
    /**
     * 识别食物图片
     * @param imageFile 图片文件
     * @return 识别结果，包括食物名称、估算重量、营养成分
     */
    public FoodRecognitionResult recognizeFood(MultipartFile imageFile) {
        // 上传图片到 OSS
        // 调用百炼视觉识别接口
        // 调用营养成分查询接口
        // 返回识别结果
    }
    
    /**
     * 查询食物营养成分
     * @param foodName 食物名称
     * @param weight 重量（克）
     * @return 营养成分信息
     */
    public NutritionInfo queryNutrition(String foodName, BigDecimal weight) {
        // 从营养成分数据库查询
        // 根据重量计算实际营养值
        // 返回营养信息
    }
}
```

#### 2.4.3 数据模型

**DietRecord (饮食记录实体)**
```java
package com.tangxiaonuan.diet.entity;

@Data
@TableName("tbl_diet_record")
public class DietRecord {
    @TableId(type = IdType.ASSIGN_UUID)
    private String recordId;            // 记录ID
    private String userId;              // 用户ID
    private String mealType;            // 餐次：breakfast/lunch/dinner/snack
    private LocalDateTime mealTime;     // 用餐时间
    private String foodItems;           // 食物清单（JSON）
    private String photoUrl;            // 照片URL
    private BigDecimal totalCalories;   // 总热量（千卡）
    private BigDecimal carbohydrate;    // 碳水化合物（克）
    private BigDecimal protein;         // 蛋白质（克）
    private BigDecimal fat;             // 脂肪（克）
    private BigDecimal fiber;           // 膳食纤维（克）
    private String notes;               // 备注
    private String sessionId;           // 对话会话ID（数字人录入时关联）
    private LocalDateTime createdAt;    // 创建时间
}
```

**FoodItem (食物条目)**
```java
package com.tangxiaonuan.diet.entity;

@Data
public class FoodItem {
    private String foodName;            // 食物名称
    private BigDecimal weight;          // 重量（克）
    private BigDecimal calories;        // 热量（千卡）
    private BigDecimal carbohydrate;    // 碳水化合物（克）
    private BigDecimal protein;         // 蛋白质（克）
    private BigDecimal fat;             // 脂肪（克）
    private BigDecimal fiber;           // 膳食纤维（克）
}
```

**NutritionDatabase (营养成分数据库)**
```java
package com.tangxiaonuan.diet.entity;

@Data
@TableName("tbl_nutrition_database")
public class NutritionDatabase {
    @TableId(type = IdType.ASSIGN_UUID)
    private String nutritionId;         // ID
    private String foodName;            // 食物名称
    private String category;            // 分类
    private BigDecimal caloriesPer100g; // 每100克热量
    private BigDecimal carbohydratePer100g; // 每100克碳水
    private BigDecimal proteinPer100g;  // 每100克蛋白质
    private BigDecimal fatPer100g;      // 每100克脂肪
    private BigDecimal fiberPer100g;    // 每100克膳食纤维
    private String source;              // 数据来源
    private LocalDateTime updatedAt;    // 更新时间
}
```

#### 2.4.4 设计理由

1. **拍照识别**: 集成阿里百炼视觉能力，降低用户记录门槛，提升用户体验。

2. **营养成分数据库**: 预置常见食物营养成分数据，快速计算摄入量，支持离线查询。

3. **餐次区分**: 区分早中晚餐和加餐，便于分析用餐规律和血糖影响。

---

### 2.5 运动消耗模块 (Exercise Module)

#### 2.5.1 功能描述
记录用户运动信息，计算今日消耗量。

#### 2.5.2 类设计

**ExerciseController.java**
```java
package com.tangxiaonuan.exercise.controller;

/**
 * 运动管理控制器
 */
@RestController
@RequestMapping("/api/v1/exercise")
public class ExerciseController {
    
    /**
     * 创建运动记录
     */
    @PostMapping("/records")
    public ResponseEntity<ExerciseRecordResponse> createRecord(@RequestBody CreateExerciseRecordRequest request);
    
    /**
     * 获取运动记录列表
     */
    @GetMapping("/records")
    public ResponseEntity<PageResponse<ExerciseRecordResponse>> getRecords(
        @RequestParam String userId,
        @RequestParam(required = false) LocalDate startDate,
        @RequestParam(required = false) LocalDate endDate,
        @RequestParam(defaultValue = "0") int page,
        @RequestParam(defaultValue = "20") int size
    );
    
    /**
     * 获取今日运动消耗统计
     */
    @GetMapping("/today-expenditure")
    public ResponseEntity<DailyExpenditureResponse> getTodayExpenditure(@RequestParam String userId);
    
    /**
     * 删除运动记录
     */
    @DeleteMapping("/records/{recordId}")
    public ResponseEntity<Void> deleteRecord(@PathVariable String recordId);
}
```

**ExerciseService.java**
```java
package com.tangxiaonuan.exercise.service;

/**
 * 运动服务接口
 */
public interface ExerciseService {
    
    /**
     * 创建运动记录
     * @param request 运动记录请求
     * @return 创建的记录
     */
    ExerciseRecordResponse createRecord(CreateExerciseRecordRequest request);
    
    /**
     * 获取运动记录列表
     * @param userId 用户ID
     * @param startDate 开始日期
     * @param endDate 结束日期
     * @param page 页码
     * @param size 每页大小
     * @return 记录列表
     */
    PageResponse<ExerciseRecordResponse> getRecords(String userId, LocalDate startDate, LocalDate endDate, int page, int size);
    
    /**
     * 计算今日消耗量
     * @param userId 用户ID
     * @return 今日运动消耗统计
     */
    DailyExpenditureResponse calculateTodayExpenditure(String userId);
    
    /**
     * 删除运动记录
     * @param recordId 记录ID
     */
    void deleteRecord(String recordId);
}
```

#### 2.5.3 数据模型

**ExerciseRecord (运动记录实体)**
```java
package com.tangxiaonuan.exercise.entity;

@Data
@TableName("tbl_exercise_record")
public class ExerciseRecord {
    @TableId(type = IdType.ASSIGN_UUID)
    private String recordId;            // 记录ID
    private String userId;              // 用户ID
    private String exerciseType;        // 运动类型：walking/running/cycling/swimming/yoga等
    private Integer duration;           // 持续时间（分钟）
    private BigDecimal distance;        // 距离（公里）
    private BigDecimal caloriesBurned;  // 消耗热量（千卡）
    private LocalDateTime startTime;    // 开始时间
    private LocalDateTime endTime;      // 结束时间
    private String notes;               // 备注
    private LocalDateTime createdAt;    // 创建时间
}
```

#### 2.5.4 设计理由

1. **运动类型丰富**: 支持多种运动类型，满足不同用户运动习惯。

2. **热量计算**: 根据运动类型、时长、用户体重等参数计算消耗热量，提供科学依据。

---

### 2.6 问诊模块 (Consultation Module)

#### 2.6.1 功能描述
提供数字人快速问诊和图文问诊服务。

#### 2.6.2 类设计

**ConsultationController.java**
```java
package com.tangxiaonuan.consultation.controller;

/**
 * 问诊控制器
 */
@RestController
@RequestMapping("/api/v1/consultations")
public class ConsultationController {
    
    /**
     * 创建问诊请求
     */
    @PostMapping
    public ResponseEntity<ConsultationResponse> createConsultation(@RequestBody CreateConsultationRequest request);
    
    /**
     * 发送问诊消息
     * 支持文字、图片、语音
     */
    @PostMapping("/{consultationId}/messages")
    public ResponseEntity<ConsultationMessageResponse> sendMessage(
        @PathVariable String consultationId,
        @RequestBody SendConsultationMessageRequest request
    );
    
    /**
     * 获取问诊消息列表
     */
    @GetMapping("/{consultationId}/messages")
    public ResponseEntity<List<ConsultationMessageResponse>> getMessages(@PathVariable String consultationId);
    
    /**
     * 获取用户问诊历史
     */
    @GetMapping
    public ResponseEntity<PageResponse<ConsultationResponse>> getConsultations(
        @RequestParam String userId,
        @RequestParam(defaultValue = "0") int page,
        @RequestParam(defaultValue = "20") int size
    );
    
    /**
     * 结束问诊
     */
    @PostMapping("/{consultationId}/complete")
    public ResponseEntity<Void> completeConsultation(@PathVariable String consultationId);
    
    /**
     * 对问诊进行评价
     */
    @PostMapping("/{consultationId}/rating")
    public ResponseEntity<Void> rateConsultation(
        @PathVariable String consultationId,
        @RequestBody RateConsultationRequest request
    );
}
```

**ConsultationService.java**
```java
package com.tangxiaonuan.consultation.service;

/**
 * 问诊服务接口
 */
public interface ConsultationService {
    
    /**
     * 创建问诊
     * @param request 问诊请求
     * @return 问诊信息
     */
    ConsultationResponse createConsultation(CreateConsultationRequest request);
    
    /**
     * 处理问诊消息
     * @param consultationId 问诊ID
     * @param request 消息请求
     * @return 回复消息
     */
    ConsultationMessageResponse processMessage(String consultationId, SendConsultationMessageRequest request);
    
    /**
     * 获取问诊消息列表
     * @param consultationId 问诊ID
     * @return 消息列表
     */
    List<ConsultationMessageResponse> getMessages(String consultationId);
    
    /**
     * 获取用户问诊历史
     * @param userId 用户ID
     * @param page 页码
     * @param size 每页大小
     * @return 问诊列表
     */
    PageResponse<ConsultationResponse> getUserConsultations(String userId, int page, int size);
    
    /**
     * 完成问诊
     * @param consultationId 问诊ID
     */
    void completeConsultation(String consultationId);
    
    /**
     * 问诊评价
     * @param consultationId 问诊ID
     * @param rating 评分
     * @param comment 评价内容
     */
    void rateConsultation(String consultationId, Integer rating, String comment);
}
```

#### 2.6.3 数据模型

**Consultation (问诊实体)**
```java
package com.tangxiaonuan.consultation.entity;

@Data
@TableName("tbl_consultation")
public class Consultation {
    @TableId(type = IdType.ASSIGN_UUID)
    private String consultationId;      // 问诊ID
    private String userId;              // 用户ID
    private String consultationType;    // 问诊类型：digital_human/text_image
    private String status;              // 状态：ongoing/completed/cancelled
    private String chiefComplaint;      // 主诉
    private LocalDateTime startTime;    // 开始时间
    private LocalDateTime endTime;      // 结束时间
    private Integer messageCount;       // 消息数量
    private Integer rating;             // 评分（1-5）
    private String comment;             // 评价内容
    private LocalDateTime createdAt;    // 创建时间
}
```

**ConsultationMessage (问诊消息实体)**
```java
package com.tangxiaonuan.consultation.entity;

@Data
@TableName("tbl_consultation_message")
public class ConsultationMessage {
    @TableId(type = IdType.ASSIGN_UUID)
    private String messageId;           // 消息ID
    private String consultationId;      // 问诊ID
    private String senderId;            // 发送者ID
    private String senderType;          // 发送者类型：user/doctor/digital_human
    private String messageType;         // 消息类型：text/image/audio
    private String content;             // 消息内容
    private String attachmentUrl;       // 附件URL（图片、音频等）
    private LocalDateTime createdAt;    // 创建时间
}
```

#### 2.6.4 设计理由

1. **问诊类型区分**: 区分数字人问诊和真人医生问诊，支持不同的业务流程。

2. **消息多媒体**: 支持文字、图片、语音多种消息类型，满足问诊场景需求。

3. **评价机制**: 收集用户评价，用于优化数字人回答质量和医生服务质量。

---

### 2.7 订阅模块 (Subscription Module)

#### 2.7.1 功能描述
管理用户订阅服务，支持图文问诊订阅等收费功能。

#### 2.7.2 类设计

**SubscriptionController.java**
```java
package com.tangxiaonuan.subscription.controller;

/**
 * 订阅控制器
 */
@RestController
@RequestMapping("/api/v1/subscriptions")
public class SubscriptionController {
    
    /**
     * 获取订阅套餐列表
     */
    @GetMapping("/plans")
    public ResponseEntity<List<SubscriptionPlanResponse>> getPlans();
    
    /**
     * 创建订阅订单
     */
    @PostMapping("/orders")
    public ResponseEntity<SubscriptionOrderResponse> createOrder(@RequestBody CreateSubscriptionOrderRequest request);
    
    /**
     * 获取用户当前订阅状态
     */
    @GetMapping("/status")
    public ResponseEntity<SubscriptionStatusResponse> getSubscriptionStatus(@RequestParam String userId);
    
    /**
     * 取消订阅
     */
    @PostMapping("/cancel")
    public ResponseEntity<Void> cancelSubscription(@RequestParam String userId);
}
```

**SubscriptionService.java**
```java
package com.tangxiaonuan.subscription.service;

/**
 * 订阅服务接口
 */
public interface SubscriptionService {
    
    /**
     * 获取所有订阅套餐
     * @return 套餐列表
     */
    List<SubscriptionPlanResponse> getAllPlans();
    
    /**
     * 创建订阅订单
     * @param userId 用户ID
     * @param planId 套餐ID
     * @return 订单信息
     */
    SubscriptionOrderResponse createOrder(String userId, String planId);
    
    /**
     * 🔥 处理支付回调（幂等性保证）
     * 
     * 风险说明：
     * 微信/支付宝支付成功后，会向后端发送支付成功通知（Webhook）。
     * 由于网络不稳定，支付平台可能会重试发送多次通知。
     * 如果不做幂等处理，可能导致：
     * 1. 用户被重复扣费
     * 2. 订单状态异常
     * 3. 订阅记录重复创建
     * 
     * 幂等策略：
     * 1. 使用分布式锁（Redis）锁定订单 ID
     * 2. 检查订单状态，如果已是 PAID，直接返回成功
     * 3. 更新订单状态时使用乐观锁（版本号）
     * 4. 记录每次回调日志，用于审计和排查
     * 
     * @param orderId 订单ID
     * @param paymentResult 支付结果
     * @return 处理结果（成功/失败/重复处理）
     */
    PaymentCallbackResult handlePaymentCallback(String orderId, PaymentResult paymentResult);
    
    /**
     * 获取用户订阅状态
     * @param userId 用户ID
     * @return 订阅状态
     */
    SubscriptionStatusResponse getSubscriptionStatus(String userId);
    
    /**
     * 检查用户是否有订阅权益
     * @param userId 用户ID
     * @param serviceType 服务类型
     * @return 是否有权益
     */
    boolean checkSubscriptionAccess(String userId, String serviceType);
    
    /**
     * 取消订阅
     * @param userId 用户ID
     */
    void cancelSubscription(String userId);
}
```

**SubscriptionServiceImpl.java**
```java
package com.tangxiaonuan.subscription.service.impl;

/**
 * 订阅服务实现
 */
@Service
@Slf4j
public class SubscriptionServiceImpl implements SubscriptionService {
    
    @Autowired
    private SubscriptionOrderMapper orderMapper;
    
    @Autowired
    private UserSubscriptionMapper userSubscriptionMapper;
    
    @Autowired
    private RedisTemplate<String, Object> redisTemplate;
    
    private static final String PAYMENT_CALLBACK_LOCK_PREFIX = "payment:callback:lock:";
    private static final int LOCK_EXPIRE_SECONDS = 30;
    
    @Override
    @Transactional(rollbackFor = Exception.class)
    public PaymentCallbackResult handlePaymentCallback(String orderId, PaymentResult paymentResult) {
        /**
         * 🔥 幂等性保证的完整实现
         */
        
        String lockKey = PAYMENT_CALLBACK_LOCK_PREFIX + orderId;
        
        // 1. 获取分布式锁（防止并发重复处理）
        Boolean lockAcquired = redisTemplate.opsForValue().setIfAbsent(
            lockKey, 
            "locked", 
            LOCK_EXPIRE_SECONDS, 
            TimeUnit.SECONDS
        );
        
        if (Boolean.FALSE.equals(lockAcquired)) {
            log.warn("支付回调处理中，拒绝重复请求：orderId={}", orderId);
            return PaymentCallbackResult.duplicate("处理中，请勿重复提交");
        }
        
        try {
            // 2. 查询订单
            SubscriptionOrder order = orderMapper.selectByOrderId(orderId);
            if (order == null) {
                log.error("订单不存在：orderId={}", orderId);
                return PaymentCallbackResult.failure("订单不存在");
            }
            
            // 3. 幂等性检查：如果订单已是 PAID 状态，直接返回成功
            if ("paid".equals(order.getStatus())) {
                log.info("订单已支付，跳过重复处理：orderId={}", orderId);
                return PaymentCallbackResult.success("订单已处理");
            }
            
            // 4. 验证支付结果
            if (!validatePaymentResult(order, paymentResult)) {
                log.error("支付结果验证失败：orderId={}, result={}", orderId, paymentResult);
                return PaymentCallbackResult.failure("支付验证失败");
            }
            
            // 5. 更新订单状态（使用乐观锁）
            order.setStatus("paid");
            order.setPaymentTransactionId(paymentResult.getTransactionId());
            order.setPaidAt(LocalDateTime.now());
            order.setVersion(order.getVersion() + 1); // 乐观锁版本号
            
            int updated = orderMapper.updateByOrderIdWithVersion(order);
            if (updated == 0) {
                // 版本号冲突，说明有并发更新，抛出异常回滚
                throw new ConcurrentModificationException("订单状态已被其他线程更新");
            }
            
            // 6. 创建用户订阅记录
            createUserSubscription(order);
            
            // 7. 记录回调日志
            recordPaymentCallbackLog(orderId, paymentResult, "success");
            
            log.info("支付回调处理成功：orderId={}, transactionId={}", orderId, paymentResult.getTransactionId());
            return PaymentCallbackResult.success("支付成功");
            
        } catch (Exception e) {
            log.error("支付回调处理异常：orderId=" + orderId, e);
            recordPaymentCallbackLog(orderId, paymentResult, "failure");
            throw e;
            
        } finally {
            // 8. 释放锁
            redisTemplate.delete(lockKey);
        }
    }
    
    /**
     * 验证支付结果
     * 防止伪造回调
     */
    private boolean validatePaymentResult(SubscriptionOrder order, PaymentResult paymentResult) {
        // 1. 验证订单金额是否一致
        if (!order.getAmount().equals(paymentResult.getAmount())) {
            return false;
        }
        
        // 2. 验证签名（防止篡改）
        // 根据支付平台的签名算法验证
        String expectedSign = calculatePaymentSign(paymentResult);
        if (!expectedSign.equals(paymentResult.getSign())) {
            return false;
        }
        
        return true;
    }
    
    /**
     * 创建用户订阅记录
     */
    private void createUserSubscription(SubscriptionOrder order) {
        UserSubscription subscription = new UserSubscription();
        subscription.setSubscriptionId(UUID.randomUUID().toString());
        subscription.setUserId(order.getUserId());
        subscription.setPlanId(order.getPlanId());
        subscription.setStatus("active");
        subscription.setStartDate(LocalDateTime.now());
        subscription.setEndDate(calculateEndDate(order.getPlanId()));
        subscription.setAutoRenew(false);
        
        userSubscriptionMapper.insert(subscription);
    }
    
    /**
     * 记录支付回调日志
     * 用于审计和排查问题
     */
    private void recordPaymentCallbackLog(String orderId, PaymentResult paymentResult, String result) {
        PaymentCallbackLog log = new PaymentCallbackLog();
        log.setOrderId(orderId);
        log.setTransactionId(paymentResult.getTransactionId());
        log.setCallbackData(JSON.toJSONString(paymentResult));
        log.setResult(result);
        log.setCreatedAt(LocalDateTime.now());
        
        paymentCallbackLogMapper.insert(log);
    }
}

/**
 * 支付回调结果
 */
@Data
public class PaymentCallbackResult {
    private String status;  // success/failure/duplicate
    private String message;
    
    public static PaymentCallbackResult success(String message) {
        PaymentCallbackResult result = new PaymentCallbackResult();
        result.setStatus("success");
        result.setMessage(message);
        return result;
    }
    
    public static PaymentCallbackResult failure(String message) {
        PaymentCallbackResult result = new PaymentCallbackResult();
        result.setStatus("failure");
        result.setMessage(message);
        return result;
    }
    
    public static PaymentCallbackResult duplicate(String message) {
        PaymentCallbackResult result = new PaymentCallbackResult();
        result.setStatus("duplicate");
        result.setMessage(message);
        return result;
    }
}
```

#### 2.7.3 数据模型

**SubscriptionPlan (订阅套餐实体)**
```java
package com.tangxiaonuan.subscription.entity;

@Data
@TableName("tbl_subscription_plan")
public class SubscriptionPlan {
    @TableId(type = IdType.ASSIGN_UUID)
    private String planId;              // 套餐ID
    private String planName;            // 套餐名称
    private String description;         // 套餐描述
    private BigDecimal price;           // 价格（元）
    private Integer duration;           // 有效期（天）
    private String features;            // 功能列表（JSON）
    private Boolean isActive;           // 是否启用
    private LocalDateTime createdAt;    // 创建时间
}
```

**SubscriptionOrder (订阅订单实体)**
```java
package com.tangxiaonuan.subscription.entity;

@Data
@TableName("tbl_subscription_order")
public class SubscriptionOrder {
    @TableId(type = IdType.ASSIGN_UUID)
    private String orderId;             // 订单ID
    private String userId;              // 用户ID
    private String planId;              // 套餐ID
    private BigDecimal amount;          // 订单金额
    private String status;              // 订单状态：pending/paid/cancelled/refunded
    private String paymentMethod;       // 支付方式：wechat/alipay
    private String paymentTransactionId; // 支付交易号
    private LocalDateTime paidAt;       // 支付时间
    private LocalDateTime createdAt;    // 创建时间
}
```

**UserSubscription (用户订阅实体)**
```java
package com.tangxiaonuan.subscription.entity;

@Data
@TableName("tbl_user_subscription")
public class UserSubscription {
    @TableId(type = IdType.ASSIGN_UUID)
    private String subscriptionId;      // 订阅ID
    private String userId;              // 用户ID
    private String planId;              // 套餐ID
    private String status;              // 状态：active/expired/cancelled
    private LocalDateTime startDate;    // 开始日期
    private LocalDateTime endDate;      // 结束日期
    private Boolean autoRenew;          // 是否自动续费
    private LocalDateTime createdAt;    // 创建时间
}
```

#### 2.7.4 设计理由

1. **套餐灵活配置**: 支持多种订阅套餐，便于运营调整定价策略。

2. **订单状态管理**: 完整的订单状态流转，支持支付回调处理。

3. **权益检查**: 提供统一的权益检查接口，便于各业务模块判断用户是否有访问权限。

---

### 2.8 首页展示模块 (Dashboard Module)

#### 2.8.1 功能描述
聚合展示用户核心数据，包括血糖、今日摄入、今日消耗、数字人入口等。

#### 2.8.2 类设计

**DashboardController.java**
```java
package com.tangxiaonuan.dashboard.controller;

/**
 * 首页控制器
 */
@RestController
@RequestMapping("/api/v1/dashboard")
public class DashboardController {
    
    /**
     * 获取首页数据
     * 包括最新血糖、今日摄入、今日消耗、快捷入口等
     */
    @GetMapping
    public ResponseEntity<DashboardResponse> getDashboard(@RequestParam String userId);
    
    /**
     * 获取血糖趋势图数据
     */
    @GetMapping("/glucose-trend")
    public ResponseEntity<GlucoseTrendResponse> getGlucoseTrend(
        @RequestParam String userId,
        @RequestParam(defaultValue = "7") int days
    );
    
    /**
     * 获取营养摄入趋势图数据
     */
    @GetMapping("/nutrition-trend")
    public ResponseEntity<NutritionTrendResponse> getNutritionTrend(
        @RequestParam String userId,
        @RequestParam(defaultValue = "7") int days
    );
}
```

**DashboardService.java**
```java
package com.tangxiaonuan.dashboard.service;

/**
 * 首页服务接口
 */
public interface DashboardService {
    
    /**
     * 聚合首页数据
     * @param userId 用户ID
     * @return 首页数据
     */
    DashboardResponse aggregateDashboardData(String userId);
    
    /**
     * 获取血糖趋势数据
     * @param userId 用户ID
     * @param days 天数
     * @return 趋势数据
     */
    GlucoseTrendResponse getGlucoseTrend(String userId, int days);
    
    /**
     * 获取营养摄入趋势数据
     * @param userId 用户ID
     * @param days 天数
     * @return 趋势数据
     */
    NutritionTrendResponse getNutritionTrend(String userId, int days);
}
```

#### 2.8.3 设计理由

1. **数据聚合**: 一次请求获取首页所需的所有数据，减少前端请求次数，提升加载速度。

2. **缓存优化**: 首页数据使用 Redis 缓存，TTL 设置为 5 分钟，平衡实时性和性能。

3. **趋势图**: 提供血糖和营养趋势图数据，帮助用户直观了解健康状态变化。

---

### 2.9 安全模块 (Security Module)

#### 2.9.1 功能描述
提供数据加密、解密、隐私协议管理等安全功能。

#### 2.9.2 类设计

**EncryptionService.java**
```java
package com.tangxiaonuan.security.service;

/**
 * 加密服务
 * 提供数据加密和解密功能
 */
@Service
public class EncryptionService {
    
    @Value("${security.encryption.key}")
    private String encryptionKey;
    
    /**
     * 加密字符串
     * 使用 AES-256-GCM 算法
     * @param plainText 明文
     * @return 密文（Base64编码）
     */
    public String encrypt(String plainText) {
        // AES-256-GCM 加密
        // Base64 编码
        // 返回密文
    }
    
    /**
     * 解密字符串
     * @param cipherText 密文（Base64编码）
     * @return 明文
     */
    public String decrypt(String cipherText) {
        // Base64 解码
        // AES-256-GCM 解密
        // 返回明文
    }
    
    /**
     * 生成加密密钥
     * @return 密钥（Base64编码）
     */
    public String generateKey() {
        // 生成 256 位随机密钥
        // Base64 编码
        // 返回密钥
    }
    
    /**
     * 数据脱敏
     * 用于日志和展示
     * @param sensitiveData 敏感数据
     * @param maskType 脱敏类型：name/phone/idcard
     * @return 脱敏后的数据
     */
    public String mask(String sensitiveData, String maskType) {
        // 根据类型进行脱敏
        // 例如：张三 -> 张*，13812345678 -> 138****5678
        // 返回脱敏数据
    }
}
```

**PrivacyPolicyService.java**
```java
package com.tangxiaonuan.security.service;

/**
 * 隐私政策服务
 */
@Service
public class PrivacyPolicyService {
    
    /**
     * 获取最新隐私政策
     * @return 隐私政策内容和版本
     */
    public PrivacyPolicyResponse getLatestPolicy();
    
    /**
     * 检查用户是否同意最新隐私政策
     * @param userId 用户ID
     * @return 是否同意
     */
    public boolean hasAgreedToLatestPolicy(String userId);
    
    /**
     * 获取用户隐私政策同意记录
     * @param userId 用户ID
     * @return 同意记录列表
     */
    public List<PrivacyAgreementResponse> getUserAgreements(String userId);
}
```

#### 2.9.3 设计理由

1. **AES-256-GCM 加密**: 使用业界标准的加密算法，保证数据安全性和完整性。

2. **密钥管理**: 加密密钥存储在配置中心，定期轮换，避免硬编码。

3. **数据脱敏**: 对敏感数据进行脱敏处理，保护用户隐私，满足日志审计需求。

4. **隐私协议版本化**: 支持隐私协议版本管理，当协议更新时提示用户重新同意。

---

### 2.10 文件上传模块 (File Upload Module)

#### 2.10.1 功能描述
提供文件上传服务，支持阿里云 OSS 直传和后端中转两种模式。

#### 2.10.2 类设计

**FileController.java**
```java
package com.tangxiaonuan.file.controller;

/**
 * 文件上传控制器
 */
@RestController
@RequestMapping("/api/v1/files")
public class FileController {
    
    /**
     * 🔥 获取 OSS 直传凭证（推荐方式）
     * 前端使用此凭证直接上传到 OSS，减少服务器带宽消耗
     * 
     * 适用场景：
     * - 饮食拍照（通常 2-5MB）
     * - 用户头像上传
     * - 问诊图片上传
     * 
     * 优点：
     * - 上传速度快（直接到 OSS）
     * - 服务器无带宽压力
     * - 支持断点续传
     * 
     * @param request 上传请求（文件类型、大小、业务场景）
     * @return STS 临时凭证 + 上传策略
     */
    @PostMapping("/oss-token")
    public ResponseEntity<OssUploadTokenResponse> getOssUploadToken(@RequestBody OssUploadTokenRequest request);
    
    /**
     * 后端中转上传（降级方式）
     * 前端直接传文件给后端，后端转存 OSS
     * 
     * 适用场景：
     * - P0 阶段快速实现
     * - 小文件上传（<1MB）
     * - 客户端不支持 OSS SDK
     * 
     * 缺点：
     * - 消耗服务器带宽
     * - 上传速度受服务器限制
     * 
     * @param file 文件
     * @param bizType 业务类型：avatar/diet_photo/consultation_image
     * @return 文件 URL
     */
    @PostMapping("/upload")
    public ResponseEntity<FileUploadResponse> uploadFile(
        @RequestParam("file") MultipartFile file,
        @RequestParam("bizType") String bizType
    );
    
    /**
     * 获取文件访问 URL
     * 对于私有文件，生成带签名的临时访问 URL
     * 
     * @param fileKey 文件 Key
     * @param expireSeconds 过期时间（秒）
     * @return 访问 URL
     */
    @GetMapping("/access-url")
    public ResponseEntity<FileAccessUrlResponse> getFileAccessUrl(
        @RequestParam String fileKey,
        @RequestParam(defaultValue = "3600") int expireSeconds
    );
}
```

**OssService.java**
```java
package com.tangxiaonuan.file.service;

/**
 * OSS 服务
 */
@Service
public class OssService {
    
    @Value("${aliyun.oss.endpoint}")
    private String endpoint;
    
    @Value("${aliyun.oss.bucket}")
    private String bucket;
    
    @Value("${aliyun.oss.access-key-id}")
    private String accessKeyId;
    
    @Value("${aliyun.oss.access-key-secret}")
    private String accessKeySecret;
    
    @Autowired
    private OSS ossClient;
    
    /**
     * 生成 STS 临时凭证
     * 用于前端直传 OSS
     * 
     * @param bizType 业务类型
     * @param expireSeconds 凭证有效期（秒）
     * @return STS 凭证
     */
    public OssUploadTokenResponse generateUploadToken(String bizType, int expireSeconds) {
        // 1. 生成唯一文件名
        String fileKey = generateFileKey(bizType);
        
        // 2. 调用 STS 服务生成临时凭证
        // 限制上传路径、文件大小、文件类型
        
        // 3. 生成 PostObject 签名
        PolicyConditions conditions = new PolicyConditions();
        conditions.addConditionItem("bucket", bucket);
        conditions.addConditionItem("key", fileKey);
        conditions.addConditionItem(PolicyConditions.COND_CONTENT_LENGTH_RANGE, 0, 10 * 1024 * 1024); // 最大 10MB
        
        String postPolicy = ossClient.generatePostPolicy(expireSeconds, conditions);
        String signature = ossClient.calculatePostSignature(postPolicy);
        
        // 4. 返回凭证
        OssUploadTokenResponse response = new OssUploadTokenResponse();
        response.setAccessKeyId(accessKeyId);
        response.setPolicy(postPolicy);
        response.setSignature(signature);
        response.setFileKey(fileKey);
        response.setHost("https://" + bucket + "." + endpoint);
        response.setExpireAt(LocalDateTime.now().plusSeconds(expireSeconds));
        
        return response;
    }
    
    /**
     * 后端中转上传文件
     * @param file 文件
     * @param bizType 业务类型
     * @return 文件 URL
     */
    public String uploadFile(MultipartFile file, String bizType) {
        try {
            // 1. 验证文件
            validateFile(file, bizType);
            
            // 2. 生成文件 Key
            String fileKey = generateFileKey(bizType);
            
            // 3. 上传到 OSS
            PutObjectRequest putRequest = new PutObjectRequest(bucket, fileKey, file.getInputStream());
            ossClient.putObject(putRequest);
            
            // 4. 返回 URL
            return "https://" + bucket + "." + endpoint + "/" + fileKey;
            
        } catch (IOException e) {
            throw new BusinessException("文件上传失败", e);
        }
    }
    
    /**
     * 生成带签名的访问 URL（用于私有文件）
     * @param fileKey 文件 Key
     * @param expireSeconds 过期时间
     * @return 访问 URL
     */
    public String generateAccessUrl(String fileKey, int expireSeconds) {
        Date expiration = new Date(System.currentTimeMillis() + expireSeconds * 1000L);
        URL url = ossClient.generatePresignedUrl(bucket, fileKey, expiration);
        return url.toString();
    }
    
    /**
     * 删除文件
     * @param fileKey 文件 Key
     */
    public void deleteFile(String fileKey) {
        ossClient.deleteObject(bucket, fileKey);
    }
    
    /**
     * 生成文件 Key
     * 格式：{bizType}/{date}/{uuid}.{ext}
     * 例如：diet_photo/2026/01/29/abc123.jpg
     */
    private String generateFileKey(String bizType) {
        LocalDate now = LocalDate.now();
        String uuid = UUID.randomUUID().toString().replace("-", "");
        return String.format("%s/%d/%02d/%02d/%s", 
            bizType, now.getYear(), now.getMonthValue(), now.getDayOfMonth(), uuid);
    }
    
    /**
     * 验证文件
     */
    private void validateFile(MultipartFile file, String bizType) {
        // 检查文件大小
        // 检查文件类型
        // 检查文件内容（防止恶意文件）
    }
}
```

#### 2.10.3 数据模型

**OssUploadTokenResponse (OSS 上传凭证)**
```java
@Data
public class OssUploadTokenResponse {
    private String accessKeyId;         // AccessKeyId
    private String policy;              // 上传策略（Base64 编码）
    private String signature;           // 签名
    private String fileKey;             // 文件 Key（路径）
    private String host;                // OSS Endpoint
    private LocalDateTime expireAt;     // 凭证过期时间
    private String callbackUrl;         // 回调 URL（可选，用于上传成功后通知后端）
}
```

**FileUploadResponse (文件上传响应)**
```java
@Data
public class FileUploadResponse {
    private String fileKey;             // 文件 Key
    private String fileUrl;             // 文件访问 URL
    private Long fileSize;              // 文件大小（字节）
    private String contentType;         // 文件类型
    private LocalDateTime uploadAt;     // 上传时间
}
```

#### 2.10.4 配置示例

**application.yml**
```yaml
aliyun:
  oss:
    endpoint: oss-cn-hangzhou.aliyuncs.com
    bucket: tangxiaonuan-prod
    access-key-id: ${ALIYUN_ACCESS_KEY_ID}
    access-key-secret: ${ALIYUN_ACCESS_KEY_SECRET}
    # 文件大小限制（字节）
    max-size:
      avatar: 2097152        # 2MB
      diet-photo: 10485760   # 10MB
      consultation-image: 5242880  # 5MB
    # 允许的文件类型
    allowed-types:
      image: image/jpeg,image/png,image/webp
      audio: audio/mp3,audio/wav
```

#### 2.10.5 P0 阶段建议

**推荐策略：后端中转（快速上线）**

理由：
1. **实现简单**：无需前端集成 OSS SDK，减少联调时间
2. **安全可控**：后端统一校验文件类型、大小，防止恶意上传
3. **适合 P0**：初期用户量小，带宽压力可接受

**P1 阶段优化：前端直传**

升级路径：
1. 前端集成阿里云 OSS SDK（微信小程序版）
2. 前端调用 `/api/v1/files/oss-token` 获取凭证
3. 前端使用凭证直传 OSS
4. 上传成功后，OSS 回调后端接口（可选，用于记录上传日志）

带宽节省估算：
- 假设日均 1000 次拍照记录，每张图 3MB
- 后端中转：3GB/天 × 30天 = 90GB/月
- 前端直传：0GB/月（OSS 流量费用更低）

#### 2.10.6 设计理由

1. **双模式支持**：提供直传和中转两种方式，兼顾开发速度和长期性能。

2. **STS 临时凭证**：不在前端暴露永久 AccessKey，提升安全性。

3. **文件 Key 设计**：按业务类型和日期分目录，便于管理和定期清理过期文件。

4. **回调机制**：OSS 上传成功后回调后端，后端可记录上传日志、触发后续业务（如图片识别）。

---

## 3. 公共 API 接口设计

### 3.1 RESTful API 规范

#### 3.1.1 基础规范
- **Base URL**: `https://api.tangxiaonuan.com/api/v1`
- **协议**: HTTPS
- **数据格式**: JSON
- **字符编码**: UTF-8
- **时间格式**: ISO 8601 (例: `2026-01-29T10:30:00+08:00`)

#### 3.1.2 通用请求头
```
Content-Type: application/json
Authorization: Bearer {access_token}
X-Request-ID: {unique_request_id}
X-User-Agent: {client_info}
```

#### 3.1.3 通用响应格式

**成功响应**
```json
{
  "code": 200,
  "message": "Success",
  "data": {
    // 业务数据
  },
  "requestId": "req-1234567890",
  "timestamp": "2026-01-29T10:30:00+08:00"
}
```

**错误响应**
```json
{
  "code": 400,
  "message": "Invalid parameter",
  "error": {
    "type": "ValidationError",
    "details": [
      {
        "field": "userId",
        "message": "userId is required"
      }
    ]
  },
  "requestId": "req-1234567890",
  "timestamp": "2026-01-29T10:30:00+08:00"
}
```

#### 3.1.4 HTTP 状态码使用
- `200 OK`: 请求成功
- `201 Created`: 资源创建成功
- `204 No Content`: 请求成功但无返回内容
- `400 Bad Request`: 请求参数错误
- `401 Unauthorized`: 未授权
- `403 Forbidden`: 无权限
- `404 Not Found`: 资源不存在
- `409 Conflict`: 资源冲突
- `429 Too Many Requests`: 请求过于频繁
- `500 Internal Server Error`: 服务器内部错误
- `503 Service Unavailable`: 服务不可用

### 3.2 核心 API 列表

#### 3.2.1 用户相关 API
| 方法 | 路径 | 描述 | 是否需要认证 |
|------|------|------|------------|
| POST | /users/guest/register | 访客注册 | 否 |
| POST | /users/guest/convert | 访客转正 | 是 |
| GET | /users/{userId} | 获取用户信息 | 是 |
| PUT | /users/{userId}/profile | 更新用户信息 | 是 |
| PUT | /users/{userId}/medical-history | 更新病例信息 | 是 |
| GET | /users/{userId}/medical-history | 获取病例信息 | 是 |
| POST | /users/{userId}/privacy-agreement | 同意隐私协议 | 是 |

#### 3.2.2 数字人相关 API
| 方法 | 路径 | 描述 | 是否需要认证 |
|------|------|------|------------|
| POST | /digital-human/sessions | 创建对话会话 | 是 |
| POST | /digital-human/sessions/{sessionId}/messages/stream | 🔥 流式发送消息（推荐） | 是 |
| POST | /digital-human/sessions/{sessionId}/messages | 同步发送消息（降级） | 是 |
| GET | /digital-human/sessions/{sessionId}/messages | 获取消息历史 | 是 |
| GET | /digital-human/sessions/{sessionId}/guidance-status | 获取引导状态 | 是 |
| DELETE | /digital-human/sessions/{sessionId} | 结束会话 | 是 |

#### 3.2.3 血糖相关 API
| 方法 | 路径 | 描述 | 是否需要认证 |
|------|------|------|------------|
| POST | /glucose/records | 创建血糖记录 | 是 |
| POST | /glucose/records/batch | 批量创建记录 | 是 |
| GET | /glucose/records | 获取血糖记录列表 | 是 |
| GET | /glucose/records/latest | 获取最新血糖 | 是 |
| GET | /glucose/statistics | 获取血糖统计 | 是 |
| DELETE | /glucose/records/{recordId} | 删除记录 | 是 |

#### 3.2.4 饮食相关 API
| 方法 | 路径 | 描述 | 是否需要认证 |
|------|------|------|------------|
| POST | /diet/records | 创建饮食记录 | 是 |
| POST | /diet/records/photo | 拍照识别饮食 | 是 |
| GET | /diet/records | 获取饮食记录列表 | 是 |
| GET | /diet/today-intake | 获取今日摄入 | 是 |
| PUT | /diet/records/{recordId} | 更新饮食记录 | 是 |
| DELETE | /diet/records/{recordId} | 删除记录 | 是 |

#### 3.2.5 运动相关 API
| 方法 | 路径 | 描述 | 是否需要认证 |
|------|------|------|------------|
| POST | /exercise/records | 创建运动记录 | 是 |
| GET | /exercise/records | 获取运动记录列表 | 是 |
| GET | /exercise/today-expenditure | 获取今日消耗 | 是 |
| DELETE | /exercise/records/{recordId} | 删除记录 | 是 |

#### 3.2.6 问诊相关 API
| 方法 | 路径 | 描述 | 是否需要认证 |
|------|------|------|------------|
| POST | /consultations | 创建问诊 | 是 |
| POST | /consultations/{consultationId}/messages | 发送问诊消息 | 是 |
| GET | /consultations/{consultationId}/messages | 获取问诊消息 | 是 |
| GET | /consultations | 获取问诊历史 | 是 |
| POST | /consultations/{consultationId}/complete | 结束问诊 | 是 |
| POST | /consultations/{consultationId}/rating | 问诊评价 | 是 |

#### 3.2.7 订阅相关 API
| 方法 | 路径 | 描述 | 是否需要认证 |
|------|------|------|------------|
| GET | /subscriptions/plans | 获取套餐列表 | 否 |
| POST | /subscriptions/orders | 创建订阅订单 | 是 |
| GET | /subscriptions/status | 获取订阅状态 | 是 |
| POST | /subscriptions/cancel | 取消订阅 | 是 |

#### 3.2.8 首页相关 API
| 方法 | 路径 | 描述 | 是否需要认证 |
|------|------|------|------------|
| GET | /dashboard | 获取首页数据 | 是 |
| GET | /dashboard/glucose-trend | 获取血糖趋势 | 是 |
| GET | /dashboard/nutrition-trend | 获取营养趋势 | 是 |

#### 3.2.9 文件上传相关 API
| 方法 | 路径 | 描述 | 是否需要认证 |
|------|------|------|------------|
| POST | /files/oss-token | 获取 OSS 直传凭证（推荐） | 是 |
| POST | /files/upload | 后端中转上传（降级） | 是 |
| GET | /files/access-url | 获取文件访问 URL | 是 |

---

## 4. 数据库设计

### 4.1 数据库选型
- **数据库**: PostgreSQL 15+
- **字符集**: UTF8
- **时区**: Asia/Shanghai

### 4.2 数据表设计

#### 4.2.1 用户表 (tbl_user)

```sql
CREATE TABLE tbl_user (
    user_id VARCHAR(36) PRIMARY KEY,
    user_type VARCHAR(20) NOT NULL,  -- guest/normal
    encrypted_name TEXT,
    gender VARCHAR(10),
    age INTEGER,
    encrypted_phone TEXT,
    encrypted_id_card TEXT,
    phone_hash VARCHAR(64),  -- 🔥 SHA256(phone + global_salt) 用于检索
    id_card_hash VARCHAR(64),  -- 🔥 SHA256(id_card + global_salt) 用于检索
    wechat_openid VARCHAR(100),  -- 微信 OpenID（用于登录）
    wechat_unionid VARCHAR(100),  -- 微信 UnionID（用于跨应用识别）
    avatar VARCHAR(500),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    is_deleted BOOLEAN DEFAULT FALSE
);

CREATE INDEX idx_user_type ON tbl_user(user_type);
CREATE INDEX idx_user_created_at ON tbl_user(created_at);
CREATE UNIQUE INDEX idx_user_phone_hash ON tbl_user(phone_hash) WHERE phone_hash IS NOT NULL;  -- 🔥 用于手机号查询
CREATE UNIQUE INDEX idx_user_id_card_hash ON tbl_user(id_card_hash) WHERE id_card_hash IS NOT NULL;  -- 🔥 用于身份证查询
CREATE UNIQUE INDEX idx_user_wechat_openid ON tbl_user(wechat_openid) WHERE wechat_openid IS NOT NULL;

COMMENT ON TABLE tbl_user IS '用户表';
COMMENT ON COLUMN tbl_user.user_id IS '用户ID（UUID）';
COMMENT ON COLUMN tbl_user.user_type IS '用户类型：guest-访客，normal-正式用户';
COMMENT ON COLUMN tbl_user.encrypted_name IS '加密的姓名（AES-256-GCM）';
COMMENT ON COLUMN tbl_user.gender IS '性别：male/female/other';
COMMENT ON COLUMN tbl_user.age IS '年龄';
COMMENT ON COLUMN tbl_user.encrypted_phone IS '加密的手机号（用于展示）';
COMMENT ON COLUMN tbl_user.encrypted_id_card IS '加密的身份证号（用于展示）';
COMMENT ON COLUMN tbl_user.phone_hash IS '🔥 手机号哈希值（用于检索）- SHA256(phone + global_salt)';
COMMENT ON COLUMN tbl_user.id_card_hash IS '🔥 身份证哈希值（用于检索）- SHA256(id_card + global_salt)';
COMMENT ON COLUMN tbl_user.wechat_openid IS '微信 OpenID';
COMMENT ON COLUMN tbl_user.wechat_unionid IS '微信 UnionID';
COMMENT ON COLUMN tbl_user.avatar IS '头像URL';
```

#### 4.2.2 病例信息表 (tbl_medical_history)

```sql
CREATE TABLE tbl_medical_history (
    history_id VARCHAR(36) PRIMARY KEY,
    user_id VARCHAR(36) NOT NULL REFERENCES tbl_user(user_id),
    diagnosis_date DATE,
    diabetes_type VARCHAR(20),  -- type1/type2
    systolic_pressure INTEGER,
    diastolic_pressure INTEGER,
    complications JSONB,
    is_on_medication BOOLEAN,
    medication_details JSONB,
    is_using_insulin BOOLEAN,
    insulin_details JSONB,
    other_diseases JSONB,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_medical_history_user_id ON tbl_medical_history(user_id);

COMMENT ON TABLE tbl_medical_history IS '病例信息表';
COMMENT ON COLUMN tbl_medical_history.diagnosis_date IS '确诊日期';
COMMENT ON COLUMN tbl_medical_history.diabetes_type IS '糖尿病类型：type1-1型，type2-2型';
COMMENT ON COLUMN tbl_medical_history.systolic_pressure IS '收缩压（mmHg）';
COMMENT ON COLUMN tbl_medical_history.diastolic_pressure IS '舒张压（mmHg）';
COMMENT ON COLUMN tbl_medical_history.complications IS '并发症列表（JSON）';
COMMENT ON COLUMN tbl_medical_history.medication_details IS '用药详情（JSON）';
COMMENT ON COLUMN tbl_medical_history.insulin_details IS '胰岛素使用详情（JSON）';
```

#### 4.2.3 隐私协议同意记录表 (tbl_privacy_agreement)

```sql
CREATE TABLE tbl_privacy_agreement (
    agreement_id VARCHAR(36) PRIMARY KEY,
    user_id VARCHAR(36) NOT NULL REFERENCES tbl_user(user_id),
    agreement_version VARCHAR(20) NOT NULL,
    agreed_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    ip_address VARCHAR(50),
    device_info VARCHAR(500)
);

CREATE INDEX idx_privacy_agreement_user_id ON tbl_privacy_agreement(user_id);
CREATE INDEX idx_privacy_agreement_version ON tbl_privacy_agreement(agreement_version);

COMMENT ON TABLE tbl_privacy_agreement IS '隐私协议同意记录表';
COMMENT ON COLUMN tbl_privacy_agreement.agreement_version IS '协议版本号';
COMMENT ON COLUMN tbl_privacy_agreement.agreed_at IS '同意时间';
COMMENT ON COLUMN tbl_privacy_agreement.ip_address IS 'IP地址';
COMMENT ON COLUMN tbl_privacy_agreement.device_info IS '设备信息';
```

#### 4.2.4 对话会话表 (tbl_conversation_session)

```sql
CREATE TABLE tbl_conversation_session (
    session_id VARCHAR(36) PRIMARY KEY,
    user_id VARCHAR(36) NOT NULL REFERENCES tbl_user(user_id),
    scene_type VARCHAR(50) NOT NULL,  -- profile_input/medical_input/glucose_record/diet_record/consultation
    status VARCHAR(20) DEFAULT 'active',  -- active/completed/expired
    start_time TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    end_time TIMESTAMP WITH TIME ZONE,
    message_count INTEGER DEFAULT 0
);

CREATE INDEX idx_session_user_id ON tbl_conversation_session(user_id);
CREATE INDEX idx_session_status ON tbl_conversation_session(status);
CREATE INDEX idx_session_start_time ON tbl_conversation_session(start_time);

COMMENT ON TABLE tbl_conversation_session IS '对话会话表';
COMMENT ON COLUMN tbl_conversation_session.scene_type IS '场景类型';
COMMENT ON COLUMN tbl_conversation_session.status IS '会话状态';
```

#### 4.2.5 对话消息表 (tbl_conversation_message)

```sql
CREATE TABLE tbl_conversation_message (
    message_id VARCHAR(36) PRIMARY KEY,
    session_id VARCHAR(36) NOT NULL REFERENCES tbl_conversation_session(session_id),
    role VARCHAR(20) NOT NULL,  -- user/assistant
    content_type VARCHAR(20) NOT NULL,  -- text/audio/image
    content TEXT,
    audio_url VARCHAR(500),
    metadata JSONB,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_message_session_id ON tbl_conversation_message(session_id);
CREATE INDEX idx_message_created_at ON tbl_conversation_message(created_at);

COMMENT ON TABLE tbl_conversation_message IS '对话消息表';
COMMENT ON COLUMN tbl_conversation_message.role IS '角色：user-用户，assistant-AI助手';
COMMENT ON COLUMN tbl_conversation_message.content_type IS '内容类型';
COMMENT ON COLUMN tbl_conversation_message.metadata IS '元数据（JSON）';
```

#### 4.2.6 引导进度表 (tbl_guidance_progress)

```sql
CREATE TABLE tbl_guidance_progress (
    progress_id VARCHAR(36) PRIMARY KEY,
    session_id VARCHAR(36) NOT NULL REFERENCES tbl_conversation_session(session_id),
    user_id VARCHAR(36) NOT NULL REFERENCES tbl_user(user_id),
    scene_type VARCHAR(50) NOT NULL,
    current_step VARCHAR(50),
    collected_data JSONB,
    completion_rate INTEGER DEFAULT 0,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_guidance_progress_session_id ON tbl_guidance_progress(session_id);
CREATE INDEX idx_guidance_progress_user_id ON tbl_guidance_progress(user_id);

COMMENT ON TABLE tbl_guidance_progress IS '引导进度表';
COMMENT ON COLUMN tbl_guidance_progress.current_step IS '当前步骤';
COMMENT ON COLUMN tbl_guidance_progress.collected_data IS '已收集数据（JSON）';
COMMENT ON COLUMN tbl_guidance_progress.completion_rate IS '完成百分比（0-100）';
```

#### 4.2.7 血糖记录表 (tbl_glucose_record)

```sql
CREATE TABLE tbl_glucose_record (
    record_id VARCHAR(36) PRIMARY KEY,
    user_id VARCHAR(36) NOT NULL REFERENCES tbl_user(user_id),
    glucose_value NUMERIC(5, 2) NOT NULL,
    measurement_type VARCHAR(30) NOT NULL,  -- fasting/before_meal/after_meal/before_sleep/random
    measurement_time TIMESTAMP WITH TIME ZONE NOT NULL,
    device_info VARCHAR(200),
    notes TEXT,
    session_id VARCHAR(36),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_glucose_user_id ON tbl_glucose_record(user_id);
CREATE INDEX idx_glucose_measurement_time ON tbl_glucose_record(measurement_time);
CREATE INDEX idx_glucose_measurement_type ON tbl_glucose_record(measurement_type);

COMMENT ON TABLE tbl_glucose_record IS '血糖记录表';
COMMENT ON COLUMN tbl_glucose_record.glucose_value IS '血糖值（mmol/L）';
COMMENT ON COLUMN tbl_glucose_record.measurement_type IS '测量类型';
COMMENT ON COLUMN tbl_glucose_record.measurement_time IS '测量时间';
COMMENT ON COLUMN tbl_glucose_record.session_id IS '关联的对话会话ID（如果通过数字人录入）';
```

#### 4.2.8 饮食记录表 (tbl_diet_record)

```sql
CREATE TABLE tbl_diet_record (
    record_id VARCHAR(36) PRIMARY KEY,
    user_id VARCHAR(36) NOT NULL REFERENCES tbl_user(user_id),
    meal_type VARCHAR(20) NOT NULL,  -- breakfast/lunch/dinner/snack
    meal_time TIMESTAMP WITH TIME ZONE NOT NULL,
    food_items JSONB NOT NULL,
    photo_url VARCHAR(500),
    total_calories NUMERIC(8, 2),
    carbohydrate NUMERIC(8, 2),
    protein NUMERIC(8, 2),
    fat NUMERIC(8, 2),
    fiber NUMERIC(8, 2),
    notes TEXT,
    session_id VARCHAR(36),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_diet_user_id ON tbl_diet_record(user_id);
CREATE INDEX idx_diet_meal_time ON tbl_diet_record(meal_time);
CREATE INDEX idx_diet_meal_type ON tbl_diet_record(meal_type);

COMMENT ON TABLE tbl_diet_record IS '饮食记录表';
COMMENT ON COLUMN tbl_diet_record.meal_type IS '餐次类型';
COMMENT ON COLUMN tbl_diet_record.food_items IS '食物清单（JSON数组）';
COMMENT ON COLUMN tbl_diet_record.total_calories IS '总热量（千卡）';
COMMENT ON COLUMN tbl_diet_record.carbohydrate IS '碳水化合物（克）';
COMMENT ON COLUMN tbl_diet_record.protein IS '蛋白质（克）';
COMMENT ON COLUMN tbl_diet_record.fat IS '脂肪（克）';
COMMENT ON COLUMN tbl_diet_record.fiber IS '膳食纤维（克）';
```

#### 4.2.9 营养成分数据库表 (tbl_nutrition_database)

```sql
CREATE TABLE tbl_nutrition_database (
    nutrition_id VARCHAR(36) PRIMARY KEY,
    food_name VARCHAR(200) NOT NULL,
    category VARCHAR(50),
    calories_per_100g NUMERIC(8, 2),
    carbohydrate_per_100g NUMERIC(8, 2),
    protein_per_100g NUMERIC(8, 2),
    fat_per_100g NUMERIC(8, 2),
    fiber_per_100g NUMERIC(8, 2),
    source VARCHAR(100),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_nutrition_food_name ON tbl_nutrition_database(food_name);
CREATE INDEX idx_nutrition_category ON tbl_nutrition_database(category);

COMMENT ON TABLE tbl_nutrition_database IS '营养成分数据库';
COMMENT ON COLUMN tbl_nutrition_database.food_name IS '食物名称';
COMMENT ON COLUMN tbl_nutrition_database.category IS '食物分类';
COMMENT ON COLUMN tbl_nutrition_database.calories_per_100g IS '每100克热量（千卡）';
COMMENT ON COLUMN tbl_nutrition_database.source IS '数据来源';
```

#### 4.2.10 运动记录表 (tbl_exercise_record)

```sql
CREATE TABLE tbl_exercise_record (
    record_id VARCHAR(36) PRIMARY KEY,
    user_id VARCHAR(36) NOT NULL REFERENCES tbl_user(user_id),
    exercise_type VARCHAR(50) NOT NULL,  -- walking/running/cycling/swimming/yoga/...
    duration INTEGER NOT NULL,  -- 分钟
    distance NUMERIC(8, 2),  -- 公里
    calories_burned NUMERIC(8, 2),
    start_time TIMESTAMP WITH TIME ZONE,
    end_time TIMESTAMP WITH TIME ZONE,
    notes TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_exercise_user_id ON tbl_exercise_record(user_id);
CREATE INDEX idx_exercise_start_time ON tbl_exercise_record(start_time);
CREATE INDEX idx_exercise_type ON tbl_exercise_record(exercise_type);

COMMENT ON TABLE tbl_exercise_record IS '运动记录表';
COMMENT ON COLUMN tbl_exercise_record.exercise_type IS '运动类型';
COMMENT ON COLUMN tbl_exercise_record.duration IS '持续时间（分钟）';
COMMENT ON COLUMN tbl_exercise_record.distance IS '距离（公里）';
COMMENT ON COLUMN tbl_exercise_record.calories_burned IS '消耗热量（千卡）';
```

#### 4.2.11 问诊记录表 (tbl_consultation)

```sql
CREATE TABLE tbl_consultation (
    consultation_id VARCHAR(36) PRIMARY KEY,
    user_id VARCHAR(36) NOT NULL REFERENCES tbl_user(user_id),
    consultation_type VARCHAR(30) NOT NULL,  -- digital_human/text_image
    status VARCHAR(20) DEFAULT 'ongoing',  -- ongoing/completed/cancelled
    chief_complaint TEXT,
    start_time TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    end_time TIMESTAMP WITH TIME ZONE,
    message_count INTEGER DEFAULT 0,
    rating INTEGER,  -- 1-5
    comment TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_consultation_user_id ON tbl_consultation(user_id);
CREATE INDEX idx_consultation_status ON tbl_consultation(status);
CREATE INDEX idx_consultation_start_time ON tbl_consultation(start_time);

COMMENT ON TABLE tbl_consultation IS '问诊记录表';
COMMENT ON COLUMN tbl_consultation.consultation_type IS '问诊类型';
COMMENT ON COLUMN tbl_consultation.chief_complaint IS '主诉';
COMMENT ON COLUMN tbl_consultation.rating IS '评分（1-5星）';
```

#### 4.2.12 问诊消息表 (tbl_consultation_message)

```sql
CREATE TABLE tbl_consultation_message (
    message_id VARCHAR(36) PRIMARY KEY,
    consultation_id VARCHAR(36) NOT NULL REFERENCES tbl_consultation(consultation_id),
    sender_id VARCHAR(36) NOT NULL,
    sender_type VARCHAR(20) NOT NULL,  -- user/doctor/digital_human
    message_type VARCHAR(20) NOT NULL,  -- text/image/audio
    content TEXT,
    attachment_url VARCHAR(500),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_consultation_message_consultation_id ON tbl_consultation_message(consultation_id);
CREATE INDEX idx_consultation_message_created_at ON tbl_consultation_message(created_at);

COMMENT ON TABLE tbl_consultation_message IS '问诊消息表';
COMMENT ON COLUMN tbl_consultation_message.sender_type IS '发送者类型';
COMMENT ON COLUMN tbl_consultation_message.message_type IS '消息类型';
```

#### 4.2.13 订阅套餐表 (tbl_subscription_plan)

```sql
CREATE TABLE tbl_subscription_plan (
    plan_id VARCHAR(36) PRIMARY KEY,
    plan_name VARCHAR(100) NOT NULL,
    description TEXT,
    price NUMERIC(10, 2) NOT NULL,
    duration INTEGER NOT NULL,  -- 天
    features JSONB,
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_subscription_plan_is_active ON tbl_subscription_plan(is_active);

COMMENT ON TABLE tbl_subscription_plan IS '订阅套餐表';
COMMENT ON COLUMN tbl_subscription_plan.price IS '价格（元）';
COMMENT ON COLUMN tbl_subscription_plan.duration IS '有效期（天）';
COMMENT ON COLUMN tbl_subscription_plan.features IS '功能列表（JSON）';
```

#### 4.2.14 订阅订单表 (tbl_subscription_order)

```sql
CREATE TABLE tbl_subscription_order (
    order_id VARCHAR(36) PRIMARY KEY,
    user_id VARCHAR(36) NOT NULL REFERENCES tbl_user(user_id),
    plan_id VARCHAR(36) NOT NULL REFERENCES tbl_subscription_plan(plan_id),
    amount NUMERIC(10, 2) NOT NULL,
    status VARCHAR(20) DEFAULT 'pending',  -- pending/paid/cancelled/refunded
    payment_method VARCHAR(20),  -- wechat/alipay
    payment_transaction_id VARCHAR(100),
    paid_at TIMESTAMP WITH TIME ZONE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_subscription_order_user_id ON tbl_subscription_order(user_id);
CREATE INDEX idx_subscription_order_status ON tbl_subscription_order(status);
CREATE INDEX idx_subscription_order_created_at ON tbl_subscription_order(created_at);

COMMENT ON TABLE tbl_subscription_order IS '订阅订单表';
COMMENT ON COLUMN tbl_subscription_order.payment_transaction_id IS '支付交易号';
```

#### 4.2.15 用户订阅表 (tbl_user_subscription)

```sql
CREATE TABLE tbl_user_subscription (
    subscription_id VARCHAR(36) PRIMARY KEY,
    user_id VARCHAR(36) NOT NULL REFERENCES tbl_user(user_id),
    plan_id VARCHAR(36) NOT NULL REFERENCES tbl_subscription_plan(plan_id),
    status VARCHAR(20) DEFAULT 'active',  -- active/expired/cancelled
    start_date TIMESTAMP WITH TIME ZONE NOT NULL,
    end_date TIMESTAMP WITH TIME ZONE NOT NULL,
    auto_renew BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_user_subscription_user_id ON tbl_user_subscription(user_id);
CREATE INDEX idx_user_subscription_status ON tbl_user_subscription(status);
CREATE INDEX idx_user_subscription_end_date ON tbl_user_subscription(end_date);

COMMENT ON TABLE tbl_user_subscription IS '用户订阅表';
COMMENT ON COLUMN tbl_user_subscription.auto_renew IS '是否自动续费';
```

#### 4.2.16 支付回调日志表 (tbl_payment_callback_log)

```sql
CREATE TABLE tbl_payment_callback_log (
    log_id VARCHAR(36) PRIMARY KEY,
    order_id VARCHAR(36) NOT NULL,
    transaction_id VARCHAR(100),
    callback_data TEXT NOT NULL,  -- 完整的回调数据（JSON）
    result VARCHAR(20) NOT NULL,  -- success/failure/duplicate
    error_message TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_payment_callback_log_order_id ON tbl_payment_callback_log(order_id);
CREATE INDEX idx_payment_callback_log_transaction_id ON tbl_payment_callback_log(transaction_id);
CREATE INDEX idx_payment_callback_log_created_at ON tbl_payment_callback_log(created_at);

COMMENT ON TABLE tbl_payment_callback_log IS '🔥 支付回调日志表 - 用于审计和排查支付问题';
COMMENT ON COLUMN tbl_payment_callback_log.callback_data IS '完整的回调数据（JSON）';
COMMENT ON COLUMN tbl_payment_callback_log.result IS '处理结果：success-成功，failure-失败，duplicate-重复';
```

#### 4.2.17 订单表增加乐观锁版本号

```sql
-- 为订阅订单表增加版本号字段（用于乐观锁）
ALTER TABLE tbl_subscription_order ADD COLUMN version INTEGER DEFAULT 0;

COMMENT ON COLUMN tbl_subscription_order.version IS '🔥 乐观锁版本号 - 防止并发更新订单状态';
```

### 4.3 数据库初始化脚本

```sql
-- 创建数据库
CREATE DATABASE tangxiaonuan
    WITH
    ENCODING = 'UTF8'
    LC_COLLATE = 'zh_CN.UTF-8'
    LC_CTYPE = 'zh_CN.UTF-8'
    TEMPLATE = template0;

-- 连接到数据库
\c tangxiaonuan;

-- 创建扩展
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";  -- UUID生成
CREATE EXTENSION IF NOT EXISTS "pg_trgm";    -- 模糊搜索

-- 执行上述所有建表语句...

-- 初始化订阅套餐数据
INSERT INTO tbl_subscription_plan (plan_id, plan_name, description, price, duration, features, is_active)
VALUES
    (uuid_generate_v4(), '月度会员', '享受图文问诊等增值服务', 29.90, 30, 
     '{"text_consultation": true, "priority_support": true}', true),
    (uuid_generate_v4(), '季度会员', '享受图文问诊等增值服务，更优惠', 79.90, 90, 
     '{"text_consultation": true, "priority_support": true, "health_report": true}', true),
    (uuid_generate_v4(), '年度会员', '享受全年图文问诊服务，最划算', 299.00, 365, 
     '{"text_consultation": true, "priority_support": true, "health_report": true, "vip_badge": true}', true);
```

### 4.4 数据库性能优化建议

1. **分区表**: 对 `tbl_glucose_record`、`tbl_diet_record`、`tbl_exercise_record` 等大表按时间分区，提升查询性能。

2. **物化视图**: 创建物化视图存储用户统计数据，定期刷新，避免实时计算。

3. **连接池**: 使用 HikariCP 连接池，配置合理的最大连接数和超时时间。

4. **慢查询监控**: 开启 PostgreSQL 慢查询日志，分析并优化慢查询。

5. **定期清理**: 定期归档或删除过期的对话消息、日志等数据。

---

## 5. 阿里百炼集成方案

### 5.1 百炼服务使用场景

| 场景 | 使用的百炼能力 | 说明 |
|------|--------------|------|
| 数字人对话 | 对话模型 + TTS/ASR | 实现徐医生数字人的文字和语音交互 |
| 信息引导录入 | 对话模型 + 信息抽取 | 通过对话引导用户录入信息，并提取结构化数据 |
| 食物识别 | 视觉模型 | 识别食物照片中的食物种类和估算重量 |
| 问诊咨询 | 对话模型 + 知识库 | 基于医学知识库回答用户问题 |

### 5.2 百炼配置

**application.yml**
```yaml
bailianai:
  api:
    key: ${BAILIANAI_API_KEY}
    endpoint: https://dashscope.aliyuncs.com/api/v1
  models:
    chat: qwen-plus  # 对话模型
    vision: qwen-vl-plus  # 视觉模型
  tts:
    voice-id: xiaoyun  # 徐医生声音ID（需要定制）
  knowledge-base:
    medical: kb-medical-diabetes  # 糖尿病医学知识库ID
```

### 5.3 知识库构建

#### 5.3.1 知识库内容
- 糖尿病基础知识
- 血糖管理指南
- 饮食营养建议
- 运动锻炼方案
- 常见问题解答
- 用药指导（需要徐医生提供专业内容）

#### 5.3.2 知识库更新流程
1. 徐医生提供医学内容文档
2. 整理为结构化数据（Markdown 格式）
3. 上传到阿里百炼知识库
4. 配置知识库检索策略
5. 测试问答效果并优化

---

## 6. 部署架构

### 6.1 服务器配置建议

**生产环境**
- **应用服务器**: 4核8G × 2台（主备）
- **数据库服务器**: 8核16G × 1台（主） + 4核8G × 1台（从，只读）
- **Redis 服务器**: 2核4G × 1台
- **负载均衡**: SLB / Nginx
- **对象存储**: 阿里云 OSS

**Showcase 演示环境**
- **单机部署**: 4核8G × 1台（应用+数据库）
- **Redis**: 内置或单独 2核2G
- **对象存储**: 阿里云 OSS

### 6.2 技术栈清单

**后端**
- JDK 21
- Spring Boot 3.2.x
- MyBatis-Plus 3.5.x
- PostgreSQL JDBC Driver
- Redis Lettuce Client
- Alibaba Cloud SDK (百炼、OSS)
- Hutool (工具类)
- Lombok
- Jackson (JSON 处理)

**中间件**
- PostgreSQL 15+
- Redis 7.x
- Nginx / SLB

**部署**
- Docker
- Docker Compose
- Kubernetes (可选)

---

## 7. 开发计划建议

### 7.1 迭代规划（按优先级）

**第一阶段（2周）：基础功能**
- 用户模块：访客注册、用户信息管理
- 安全模块：数据加密、隐私协议
- 血糖模块：记录和展示
- 数据库初始化

**第二阶段（3周）：数字人交互**
- 数字人模块：会话管理、消息处理
- 阿里百炼集成
- 信息引导录入（个人信息、病例信息）
- 数字人引导记录血糖

**第三阶段（3周）：饮食与运动**
- 饮食模块：记录、拍照识别
- 运动模块：记录和消耗计算
- 首页模块：数据聚合展示
- 数字人引导记录饮食

**第四阶段（2周）：问诊与订阅**
- 问诊模块：数字人问诊
- 订阅模块：套餐管理、订单处理
- 图文问诊（收费）

**第五阶段（1周）：测试与优化**
- 功能测试
- 性能优化
- 安全加固
- 文档完善

### 7.2 技术风险点

1. **阿里百炼集成复杂度**: 需要充分测试对话效果和知识库召回准确性。
   - **缓解措施**: 提前进行技术预研，准备 Plan B（如果百炼效果不佳，可降级为模板对话）。

2. **数据加密性能**: 大量加密解密操作可能影响性能。
   - **缓解措施**: 使用缓存减少解密次数，敏感字段按需解密。

3. **食物识别准确率**: AI 识别可能不准确。
   - **缓解措施**: 允许用户手动修改识别结果，收集反馈数据优化模型。

4. **并发压力**: Showcase 期间可能有大量用户同时访问。
   - **缓解措施**: 使用 Redis 缓存热点数据，限流保护核心接口。

---

## 8. 附录

### 8.1 术语表

| 术语 | 英文 | 说明 |
|------|------|------|
| 数字人 | Digital Human | AI 驱动的虚拟医生助手 |
| 访客模式 | Guest Mode | 免登录体验模式 |
| 徐医生 | Dr. Xu | 数字人医生角色名称 |
| 百炼 | Bailian | 阿里云百炼 AI 平台 |
| TTS | Text-to-Speech | 文字转语音 |
| ASR | Automatic Speech Recognition | 语音识别 |

### 8.2 参考资料

- [Spring Boot 官方文档](https://spring.io/projects/spring-boot)
- [PostgreSQL 官方文档](https://www.postgresql.org/docs/)
- [阿里百炼开发文档](https://help.aliyun.com/product/2400256.html)
- [MyBatis-Plus 官方文档](https://baomidou.com/)

---

## 9. 总结

本文档详细设计了"糖小暖"应用的所有 P0 需求，包括：

✅ **9 个核心模块**：用户、数字人、血糖、饮食、运动、问诊、订阅、首页、安全
✅ **50+ 个 Java 类**：Controller、Service、Mapper 等
✅ **40+ 个 RESTful API**：覆盖所有业务场景
✅ **15 张数据表**：完整的数据库设计
✅ **阿里百炼集成方案**：数字人对话、食物识别
✅ **安全合规设计**：数据加密、隐私协议管理
✅ **部署方案**：生产环境和演示环境配置

所有设计遵循以下原则：
- **模块化**：高内聚低耦合
- **可扩展**：支持后续功能扩展
- **安全性**：敏感数据加密存储
- **合规性**：满足隐私保护法律要求
- **性能**：缓存、索引、分页优化
- **可维护**：清晰的代码结构和文档

---

## 10. 技术优化总结（v1.1 新增）

### 10.1 核心优化点对比

| 优化项 | 原设计 | 优化后 | 收益 |
|--------|--------|--------|------|
| **数字人交互协议** | REST 同步请求 | SSE 流式推送 | ✅ 延迟从 5-8 秒降低至 0.5 秒首字响应<br>✅ 支持打字机效果<br>✅ 用户体验大幅提升 |
| **AI Agent 集成** | 仅返回文本 | Function Calling 工具执行 | ✅ AI 可直接调用业务接口<br>✅ 自动保存数据到数据库<br>✅ 减少用户手动操作 |
| **敏感数据检索** | 仅加密存储 | 加密 + 哈希索引 | ✅ 支持手机号/身份证查询<br>✅ 保持数据安全性<br>✅ 查询性能无损失 |
| **访客转正逻辑** | 未明确 | 双策略数据迁移 | ✅ 新用户直接升级<br>✅ 老用户数据合并<br>✅ 无数据丢失风险 |
| **文件上传** | 未定义 | OSS 直传 + 后端中转 | ✅ P0 快速上线（中转）<br>✅ P1 性能优化（直传）<br>✅ 节省带宽成本 90%+ |
| **支付回调** | 简单处理 | 幂等性保证 | ✅ 防止重复扣费<br>✅ 分布式锁 + 乐观锁<br>✅ 完整审计日志 |

### 10.2 SSE 流式交互详解

#### 10.2.1 前端集成示例（微信小程序）

```javascript
// 微信小程序调用 SSE 接口
wx.request({
  url: 'https://api.tangxiaonuan.com/api/v1/digital-human/sessions/{sessionId}/messages/stream',
  method: 'POST',
  data: {
    content: '我刚测了血糖6.5',
    contentType: 'text'
  },
  enableChunked: true,  // 🔥 关键：启用分块传输
  success(res) {
    // SSE 事件处理
    res.on('data', (chunk) => {
      const lines = chunk.toString().split('\n');
      lines.forEach(line => {
        if (line.startsWith('data: ')) {
          const data = JSON.parse(line.substring(6));
          handleSseEvent(data);
        }
      });
    });
  }
});

function handleSseEvent(data) {
  switch(data.event) {
    case 'thinking':
      // 显示"徐医生正在思考..."
      showThinking();
      break;
    case 'text_chunk':
      // 逐字追加到对话框（打字机效果）
      appendText(data.text);
      break;
    case 'audio_chunk':
      // 播放语音片段
      playAudio(data.audioUrl);
      break;
    case 'tool_call':
      // 显示"正在为您记录..."
      showToolExecution(data.toolName);
      break;
    case 'done':
      // 对话完成
      hideThinking();
      break;
  }
}
```

#### 10.2.2 后端 SSE 实现示例

```java
@PostMapping(value = "/sessions/{sessionId}/messages/stream", 
             produces = MediaType.TEXT_EVENT_STREAM_VALUE)
public SseEmitter streamMessage(@PathVariable String sessionId,
                                @RequestBody SendMessageRequest request) {
    SseEmitter emitter = new SseEmitter(60000L); // 60秒超时
    
    // 异步处理（不阻塞主线程）
    CompletableFuture.runAsync(() -> {
        try {
            // 1. 发送 thinking 事件
            emitter.send(SseEmitter.event()
                .name("thinking")
                .data("{\"status\":\"thinking\"}"));
            
            // 2. 调用百炼流式 API
            bailianAIClient.chatStream(request.getContent(), response -> {
                // 3. 逐块推送文本
                emitter.send(SseEmitter.event()
                    .name("text_chunk")
                    .data("{\"text\":\"" + response.getText() + "\"}"));
                
                // 4. 检测 Function Call
                if (response.hasFunctionCall()) {
                    emitter.send(SseEmitter.event()
                        .name("tool_call")
                        .data("{\"toolName\":\"" + response.getFunctionName() + "\"}"));
                    
                    // 执行工具
                    String toolResult = agentToolExecutor.executeTool(
                        response.getFunctionName(),
                        response.getFunctionParams(),
                        request.getUserId()
                    );
                    
                    emitter.send(SseEmitter.event()
                        .name("tool_result")
                        .data("{\"result\":\"" + toolResult + "\"}"));
                }
            });
            
            // 5. 生成并推送音频
            String audioUrl = bailianAIClient.textToSpeech(fullText, "xiaoyun");
            emitter.send(SseEmitter.event()
                .name("audio_chunk")
                .data("{\"audioUrl\":\"" + audioUrl + "\"}"));
            
            // 6. 发送完成事件
            emitter.send(SseEmitter.event()
                .name("done")
                .data("{\"messageId\":\"msg-123\"}"));
            
            emitter.complete();
            
        } catch (Exception e) {
            emitter.completeWithError(e);
        }
    });
    
    return emitter;
}
```

### 10.3 阿里百炼 Function Calling 配置

#### 10.3.1 百炼控制台工具定义

在阿里百炼控制台，需要为 Agent 配置以下工具：

```json
{
  "tools": [
    {
      "name": "save_blood_sugar",
      "description": "保存用户的血糖测量记录。当用户提到血糖值、测血糖时调用此工具。",
      "parameters": {
        "type": "object",
        "properties": {
          "userId": {
            "type": "string",
            "description": "用户ID"
          },
          "value": {
            "type": "number",
            "description": "血糖值（mmol/L）"
          },
          "measurementType": {
            "type": "string",
            "enum": ["fasting", "before_meal", "after_meal", "before_sleep", "random"],
            "description": "测量类型：fasting-空腹，before_meal-餐前，after_meal-餐后，before_sleep-睡前，random-随机"
          },
          "measurementTime": {
            "type": "string",
            "description": "测量时间，支持自然语言表达，如'刚才'、'早上8点'、'昨天晚上'"
          }
        },
        "required": ["userId", "value", "measurementType"]
      }
    },
    {
      "name": "save_diet",
      "description": "保存用户的饮食记录。当用户描述吃了什么食物时调用此工具。",
      "parameters": {
        "type": "object",
        "properties": {
          "userId": {
            "type": "string",
            "description": "用户ID"
          },
          "mealType": {
            "type": "string",
            "enum": ["breakfast", "lunch", "dinner", "snack"],
            "description": "餐次类型"
          },
          "foodItems": {
            "type": "array",
            "items": {
              "type": "object",
              "properties": {
                "foodName": {"type": "string"},
                "weight": {"type": "number", "description": "重量（克）"}
              }
            },
            "description": "食物清单"
          },
          "mealTime": {
            "type": "string",
            "description": "用餐时间"
          }
        },
        "required": ["userId", "mealType", "foodItems"]
      }
    },
    {
      "name": "query_today_glucose",
      "description": "查询用户今日的血糖记录。当用户询问今天血糖情况时调用此工具。",
      "parameters": {
        "type": "object",
        "properties": {
          "userId": {"type": "string", "description": "用户ID"}
        },
        "required": ["userId"]
      }
    }
  ]
}
```

#### 10.3.2 后端工具执行器注册

```java
@Configuration
public class AgentToolConfig {
    
    @Autowired
    private AgentToolExecutor toolExecutor;
    
    /**
     * 注册所有工具到路由表
     */
    @PostConstruct
    public void registerTools() {
        ToolRegistry.register("save_blood_sugar", 
            (params, userId) -> toolExecutor.saveBloodSugar(
                userId,
                (Double) params.get("value"),
                (String) params.get("measurementType"),
                (String) params.get("measurementTime")
            )
        );
        
        ToolRegistry.register("save_diet",
            (params, userId) -> toolExecutor.saveDiet(
                userId,
                (String) params.get("mealType"),
                (List<Map<String, Object>>) params.get("foodItems"),
                (String) params.get("mealTime")
            )
        );
        
        ToolRegistry.register("query_today_glucose",
            (params, userId) -> toolExecutor.queryTodayGlucose(userId)
        );
    }
}
```

### 10.4 安全检索策略详解

#### 10.4.1 加密与哈希双轨存储

```java
/**
 * 保存用户手机号（双轨存储）
 */
public void saveUserPhone(String userId, String phone) {
    User user = userMapper.selectByUserId(userId);
    
    // 1. 加密存储（用于展示和解密）
    String encryptedPhone = encryptionService.encrypt(phone);
    user.setEncryptedPhone(encryptedPhone);
    
    // 2. 哈希存储（用于检索）
    String phoneHash = DigestUtils.sha256Hex(phone + globalSalt);
    user.setPhoneHash(phoneHash);
    
    userMapper.updateByUserId(user);
}

/**
 * 通过手机号查询用户
 */
public User findUserByPhone(String phone) {
    // 1. 计算哈希值
    String phoneHash = DigestUtils.sha256Hex(phone + globalSalt);
    
    // 2. 通过哈希值查询
    User user = userMapper.selectByPhoneHash(phoneHash);
    
    // 3. 解密手机号用于展示
    if (user != null) {
        String decryptedPhone = encryptionService.decrypt(user.getEncryptedPhone());
        user.setPhone(decryptedPhone); // 设置到临时字段（不持久化）
    }
    
    return user;
}
```

#### 10.4.2 全局盐值管理

```yaml
# application.yml
security:
  encryption:
    key: ${ENCRYPTION_KEY}  # AES 密钥（32字节，从配置中心读取）
    global-salt: ${GLOBAL_SALT}  # 全局盐值（用于哈希，定期轮换）
```

**盐值轮换策略**：
1. 初始盐值：`GLOBAL_SALT_V1`
2. 6个月后轮换：`GLOBAL_SALT_V2`
3. 轮换时保留旧盐值，支持双版本查询
4. 后台任务逐步重新计算所有哈希值

### 10.5 性能优化建议

#### 10.5.1 Redis 缓存策略

```java
/**
 * 用户信息缓存
 * TTL: 30分钟
 */
@Cacheable(value = "user", key = "#userId", unless = "#result == null")
public UserResponse getUserById(String userId) {
    // 查询数据库
}

/**
 * 首页数据缓存
 * TTL: 5分钟
 */
@Cacheable(value = "dashboard", key = "#userId")
public DashboardResponse getDashboard(String userId) {
    // 聚合查询
}

/**
 * 订阅状态缓存
 * TTL: 10分钟
 */
@Cacheable(value = "subscription", key = "#userId")
public SubscriptionStatusResponse getSubscriptionStatus(String userId) {
    // 查询订阅
}
```

#### 10.5.2 数据库查询优化

```sql
-- 1. 复合索引优化（血糖查询）
CREATE INDEX idx_glucose_user_time ON tbl_glucose_record(user_id, measurement_time DESC);

-- 2. 部分索引（仅索引活跃用户）
CREATE INDEX idx_active_user ON tbl_user(user_id) 
WHERE user_type = 'normal' AND is_deleted = FALSE;

-- 3. 表达式索引（日期查询）
CREATE INDEX idx_glucose_date ON tbl_glucose_record((measurement_time::date));

-- 4. JSONB 字段索引（食物查询）
CREATE INDEX idx_diet_food_items ON tbl_diet_record USING GIN(food_items);
```

### 10.6 监控与告警

#### 10.6.1 关键指标监控

| 指标 | 阈值 | 告警级别 |
|------|------|----------|
| SSE 连接失败率 | > 5% | P1 |
| Function Calling 执行失败率 | > 1% | P1 |
| 支付回调重复处理率 | > 10% | P2 |
| OSS 上传失败率 | > 3% | P2 |
| 数据库慢查询 | > 2秒 | P1 |
| Redis 命中率 | < 80% | P3 |

#### 10.6.2 日志埋点

```java
// 1. SSE 交互日志
log.info("SSE开始推送：sessionId={}, userId={}, content={}", sessionId, userId, content);
log.info("SSE推送完成：sessionId={}, duration={}ms, chunks={}", sessionId, duration, chunkCount);

// 2. Function Calling 日志
log.info("工具调用：toolName={}, userId={}, params={}", toolName, userId, params);
log.info("工具执行完成：toolName={}, result={}, duration={}ms", toolName, result, duration);

// 3. 支付回调日志（已在代码中实现）
log.info("支付回调：orderId={}, transactionId={}, amount={}", orderId, transactionId, amount);
```

### 10.7 部署架构升级建议

#### 10.7.1 生产环境架构（优化后）

```
                        ┌─────────────┐
                        │   SLB/CDN   │
                        └──────┬──────┘
                               │
                ┌──────────────┼──────────────┐
                │              │              │
         ┌──────▼──────┐┌──────▼──────┐┌──────▼──────┐
         │  应用节点1   ││  应用节点2   ││  应用节点3   │
         │ (SSE支持)   ││ (SSE支持)   ││ (SSE支持)   │
         └──────┬──────┘└──────┬──────┘└──────┬──────┘
                │              │              │
                └──────────────┼──────────────┘
                               │
                ┌──────────────┼──────────────┐
                │              │              │
         ┌──────▼──────┐┌──────▼──────┐┌──────▼──────┐
         │   Redis     ││  PostgreSQL ││  RocketMQ   │
         │  (缓存/锁)   ││  (主从分离)  ││  (异步任务)  │
         └─────────────┘└─────────────┘└─────────────┘
                               │
                ┌──────────────┼──────────────┐
                │              │              │
         ┌──────▼──────┐┌──────▼──────┐┌──────▼──────┐
         │  阿里百炼    ││  阿里云OSS  ││  微信支付    │
         │  (AI服务)   ││  (文件存储)  ││  (支付网关)  │
         └─────────────┘└─────────────┘└─────────────┘
```

#### 10.7.2 资源配置建议（优化后）

| 服务 | P0（演示） | P1（生产 1000 DAU） | P2（生产 10000 DAU） |
|------|-----------|-------------------|---------------------|
| 应用服务器 | 4C8G × 1 | 4C8G × 2 | 8C16G × 3 |
| PostgreSQL | 4C8G × 1 | 8C16G × 1（主）+ 4C8G × 1（从） | 16C32G × 1（主）+ 8C16G × 2（从） |
| Redis | 2C4G × 1 | 4C8G × 1（主从） | 8C16G × 1（哨兵集群） |
| 阿里百炼 QPS | 10 QPS | 50 QPS | 200 QPS |
| OSS 流量 | 10GB/月 | 100GB/月 | 500GB/月 |

---

### 10.8 工程落地最佳实践（🔥 避坑指南）

#### 10.8.1 SSE 连接管理优化

##### 问题 1：客户端断连导致的成本浪费

**场景**：用户在数字人说话说到一半时退出小程序，但后端线程仍在调用百炼 API 和 TTS，浪费 Token 和流量。

**影响**：
- 阿里百炼按 Token 计费，无效推理浪费成本
- TTS 音频生成浪费流量和时间
- 服务器资源占用

**解决方案**：注册 SseEmitter 回调，感知断连并取消异步任务

```java
@PostMapping(value = "/sessions/{sessionId}/messages/stream", 
             produces = MediaType.TEXT_EVENT_STREAM_VALUE)
public SseEmitter streamMessage(@PathVariable String sessionId,
                                @RequestBody SendMessageRequest request) {
    SseEmitter emitter = new SseEmitter(180000L); // 180秒超时
    
    // 🔥 使用 AtomicReference 存储 Future，便于在回调中取消
    AtomicReference<CompletableFuture<Void>> runningFutureRef = new AtomicReference<>();
    
    // 🔥 注册断连回调
    emitter.onCompletion(() -> {
        log.info("SSE连接正常结束：sessionId={}", sessionId);
        // 清理资源
    });
    
    emitter.onTimeout(() -> {
        log.warn("SSE连接超时：sessionId={}", sessionId);
        CompletableFuture<Void> future = runningFutureRef.get();
        if (future != null && !future.isDone()) {
            future.cancel(true);
            log.info("已取消超时的异步任务");
        }
    });
    
    emitter.onError((ex) -> {
        log.error("SSE连接异常：sessionId=" + sessionId, ex);
        // 🔥 关键：取消正在运行的 AI 推理任务
        CompletableFuture<Void> future = runningFutureRef.get();
        if (future != null && !future.isDone()) {
            future.cancel(true);
            log.info("已取消异常的异步任务，节省 AI Token 消耗");
        }
    });
    
    // 异步处理
    CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
        try {
            // 检查任务是否已被取消
            if (Thread.currentThread().isInterrupted()) {
                log.info("任务已取消，停止执行");
                return;
            }
            
            // 1. 发送 thinking 事件
            emitter.send(SseEmitter.event()
                .name("thinking")
                .data("{\"status\":\"thinking\"}"));
            
            // 2. 调用百炼流式 API
            bailianAIClient.chatStream(request.getContent(), response -> {
                // 每次推送前检查是否已取消
                if (Thread.currentThread().isInterrupted()) {
                    throw new InterruptedException("任务已取消");
                }
                
                // 推送文本片段
                emitter.send(SseEmitter.event()
                    .name("text_chunk")
                    .data("{\"text\":\"" + response.getText() + "\"}"));
            });
            
            // 3. 生成 TTS（检查是否已取消）
            if (!Thread.currentThread().isInterrupted()) {
                String audioUrl = bailianAIClient.textToSpeech(fullText, "xiaoyun");
                emitter.send(SseEmitter.event()
                    .name("audio_chunk")
                    .data("{\"audioUrl\":\"" + audioUrl + "\"}"));
            }
            
            emitter.send(SseEmitter.event().name("done").data("{}"));
            emitter.complete();
            
        } catch (InterruptedException e) {
            log.info("任务被中断，已取消");
            emitter.completeWithError(e);
        } catch (Exception e) {
            log.error("处理异常", e);
            emitter.completeWithError(e);
        }
    });
    
    // 保存 Future 引用
    runningFutureRef.set(future);
    
    return emitter;
}
```

**成本节省估算**：
- 假设 10% 的用户会中途退出
- 平均每次对话消耗 1000 Token（输入 + 输出）
- 每日 1000 次对话，100 次中途退出
- 节省：100 × 1000 Token × ¥0.0002/Token ≈ **¥20/天** ≈ **¥600/月**

##### 问题 2：SSE 超时设置与心跳保活

**问题**：复杂的 AI 推理（RAG + Tool Calling + TTS）可能超过 60 秒。

**解决方案 A**：延长超时时间

```java
// 方案 1：延长超时到 180 秒（3分钟）
SseEmitter emitter = new SseEmitter(180000L);

// 方案 2：永不超时（依赖客户端主动断开）
SseEmitter emitter = new SseEmitter(0L);  // 0 表示永不超时
```

**解决方案 B**：心跳保活（推荐）

```java
// 后端每隔 30 秒发送心跳
ScheduledExecutorService heartbeatExecutor = Executors.newScheduledThreadPool(1);
heartbeatExecutor.scheduleAtFixedRate(() -> {
    try {
        if (!emitter.isClosed()) {
            // 发送空事件（注释行，不会被解析为数据）
            emitter.send(SseEmitter.event().comment("keepalive"));
        }
    } catch (Exception e) {
        heartbeatExecutor.shutdown();
    }
}, 0, 30, TimeUnit.SECONDS);
```

**前端接收心跳**：

```javascript
// 微信小程序无需特殊处理，心跳会自动保持连接
// 如果是 Web 端，可以监听 comment 事件
eventSource.addEventListener('message', (e) => {
  if (e.data === ':keepalive') {
    console.log('收到心跳');
  }
});
```

---

#### 10.8.2 AI 上下文 Token 优化

##### 问题：对话轮数增加导致 Token 消耗爆炸

**场景**：用户与数字人对话 20 轮后，每次请求都要传历史 20 轮的上下文给百炼。

**影响**：
- **费用爆炸**：Input Token 从 200 增长到 5000+
- **超限报错**：超过模型上下文限制（如 8k/32k Tokens）
- **响应变慢**：大量 Input Token 增加推理时间

**解决方案 1：滑动窗口（Sliding Window）**

```java
@Service
public class ConversationContextManager {
    
    private static final int MAX_CONTEXT_ROUNDS = 10;  // 最多保留 10 轮对话
    
    /**
     * 获取对话上下文（应用滑动窗口）
     */
    public List<Message> getContextWithSlidingWindow(String sessionId) {
        // 1. 从 Redis 获取完整历史
        List<Message> allMessages = getFullHistory(sessionId);
        
        // 2. 应用滑动窗口：只保留最近 N 轮
        List<Message> recentMessages = allMessages.stream()
            .skip(Math.max(0, allMessages.size() - MAX_CONTEXT_ROUNDS * 2)) // 每轮2条消息（用户+助手）
            .collect(Collectors.toList());
        
        // 3. 始终保留 System Prompt（第一条消息）
        if (!allMessages.isEmpty() && !recentMessages.contains(allMessages.get(0))) {
            List<Message> result = new ArrayList<>();
            result.add(allMessages.get(0)); // System Prompt
            result.addAll(recentMessages);
            return result;
        }
        
        return recentMessages;
    }
}
```

**解决方案 2：智能修剪（Function Call 中间结果）**

```java
/**
 * 修剪上下文：移除冗余的 Function Call 中间过程
 */
public List<Message> trimContext(List<Message> messages) {
    List<Message> trimmed = new ArrayList<>();
    
    for (int i = 0; i < messages.size(); i++) {
        Message msg = messages.get(i);
        
        // 保留：用户消息、助手的最终回复
        if (msg.getRole().equals("user") || 
            msg.getRole().equals("assistant") && !msg.isFunctionCall()) {
            trimmed.add(msg);
        }
        
        // 移除：Function Call 的中间参数和结果（已执行完毕）
        // 例如：{"role": "function", "name": "save_blood_sugar", "content": "..."}
        if (msg.getRole().equals("function")) {
            // 跳过，不加入 trimmed
            log.debug("修剪掉 Function Call 中间结果：{}", msg.getContent());
        }
    }
    
    return trimmed;
}
```

**Token 节省估算**：
- 原始：20 轮对话 × 250 Token/轮 = **5000 Input Tokens**
- 优化后：10 轮对话 × 250 Token/轮 = **2500 Input Tokens**
- 节省：**50% Input Token 成本**

**配置建议**：

```yaml
bailianai:
  context:
    max-rounds: 10          # 最多保留 10 轮对话
    max-input-tokens: 4000  # 输入 Token 上限（超过则强制修剪）
    trim-function-calls: true  # 是否修剪 Function Call 中间结果
```

---

#### 10.8.3 数据迁移事务与边界处理

##### 问题 1：事务注解失效

**常见原因**：
1. `@Transactional` 注解在 private 方法上（无效）
2. 同类方法内部调用（AOP 代理失效）
3. 异常被 try-catch 吞掉，未抛出

**正确姿势**：

```java
@Service
public class UserServiceImpl implements UserService {
    
    // ✅ 正确：public 方法，事务生效
    @Override
    @Transactional(rollbackFor = Exception.class)
    public UserResponse convertGuestToUser(String guestUserId, ConvertUserRequest request) {
        // ... 业务逻辑 ...
        
        try {
            // 迁移数据
            migrateUserData(guestUserId, existingUserId);
        } catch (DataMigrationException e) {
            // ✅ 正确：重新抛出异常，触发回滚
            throw new BusinessException("数据迁移失败", e);
        }
        
        return response;
    }
    
    // ❌ 错误：private 方法的 @Transactional 无效
    // @Transactional
    private void migrateUserData(String fromUserId, String toUserId) {
        // ...
    }
}
```

##### 问题 2：大数据量迁移可能锁表

**风险**：如果访客记录了 1000 条血糖数据，一次性 UPDATE 可能锁表。

**解决方案**：分批迁移

```java
/**
 * 分批迁移血糖记录
 */
private void migrateGlucoseRecords(String fromUserId, String toUserId) {
    int batchSize = 100;
    int offset = 0;
    int migratedCount = 0;
    
    while (true) {
        // 分批查询
        List<GlucoseRecord> batch = glucoseRecordMapper.selectByUserIdWithLimit(
            fromUserId, offset, batchSize
        );
        
        if (batch.isEmpty()) {
            break;
        }
        
        // 批量更新 user_id
        batch.forEach(record -> record.setUserId(toUserId));
        glucoseRecordMapper.batchUpdate(batch);
        
        migratedCount += batch.size();
        offset += batchSize;
        
        log.info("迁移血糖记录进度：{} 条", migratedCount);
    }
}
```

##### 问题 3：同一时间点的重复记录

**场景**：访客和老账号在 2026-01-29 08:00 都有一条"早餐"记录，合并后如何处理？

**策略 A（简单）**：允许重复

```java
// 直接迁移，不去重
// 前端按时间倒序展示时，用户可以看到两条记录并手动删除
```

**策略 B（智能）**：检测并标记

```java
/**
 * 迁移前检测潜在冲突
 */
private void checkAndMigrateDietRecords(String fromUserId, String toUserId) {
    List<DietRecord> guestRecords = dietRecordMapper.selectByUserId(fromUserId);
    List<DietRecord> existingRecords = dietRecordMapper.selectByUserId(toUserId);
    
    for (DietRecord guestRecord : guestRecords) {
        // 检查是否有时间重叠的记录（前后 10 分钟内）
        boolean hasDuplicate = existingRecords.stream().anyMatch(existing ->
            Math.abs(ChronoUnit.MINUTES.between(
                guestRecord.getMealTime(), 
                existing.getMealTime()
            )) < 10
        );
        
        if (hasDuplicate) {
            // 标记为"待确认"，前端展示时高亮
            guestRecord.setNeedReview(true);
        }
        
        guestRecord.setUserId(toUserId);
        dietRecordMapper.update(guestRecord);
    }
}
```

---

#### 10.8.4 OSS 图片缩略图优化

##### 问题：列表加载大图导致卡顿

**场景**：饮食记录列表展示 10 张照片，每张 5MB，前端需要加载 50MB 数据。

**影响**：
- 加载慢：4G 网络下需要 10+ 秒
- 费流量：用户流量消耗大
- 体验差：列表滑动卡顿

**解决方案**：使用阿里云 OSS 图片处理（x-oss-process）

```java
@Service
public class OssService {
    
    /**
     * 生成缩略图 URL
     * @param originalUrl 原图 URL
     * @param width 缩略图宽度
     * @return 缩略图 URL
     */
    public String generateThumbnailUrl(String originalUrl, int width) {
        if (StringUtils.isBlank(originalUrl)) {
            return null;
        }
        
        // 🔥 使用 OSS 图片处理参数（实时生成，不占用额外存储空间）
        String process = String.format("?x-oss-process=image/resize,w_%d", width);
        return originalUrl + process;
    }
    
    /**
     * 生成多种尺寸 URL
     */
    public ImageUrls generateMultiSizeUrls(String originalUrl) {
        ImageUrls urls = new ImageUrls();
        urls.setOriginal(originalUrl);
        urls.setThumbnail(generateThumbnailUrl(originalUrl, 200));   // 列表缩略图
        urls.setMedium(generateThumbnailUrl(originalUrl, 800));      // 详情页中图
        urls.setLarge(originalUrl);  // 原图（点击放大时加载）
        return urls;
    }
}
```

**Controller 返回多尺寸 URL**：

```java
@GetMapping("/records")
public ResponseEntity<PageResponse<DietRecordResponse>> getRecords(...) {
    List<DietRecord> records = dietService.getRecords(userId, startDate, endDate, page, size);
    
    // 🔥 为每条记录生成缩略图 URL
    List<DietRecordResponse> responses = records.stream().map(record -> {
        DietRecordResponse response = new DietRecordResponse();
        // ... 其他字段映射 ...
        
        // 生成多尺寸 URL
        ImageUrls imageUrls = ossService.generateMultiSizeUrls(record.getPhotoUrl());
        response.setPhotoUrls(imageUrls);
        
        return response;
    }).collect(Collectors.toList());
    
    return ResponseEntity.ok(PageResponse.of(responses, total));
}
```

**前端使用**：

```javascript
// 列表页：加载缩略图（200px 宽）
<image src="{{item.photoUrls.thumbnail}}" mode="aspectFill"></image>

// 详情页：加载中图（800px 宽）
<image src="{{item.photoUrls.medium}}" mode="widthFix"></image>

// 点击放大：加载原图
<image src="{{item.photoUrls.large}}" mode="aspectFit"></image>
```

**效果对比**：

| 场景 | 原始 | 优化后 | 提升 |
|------|------|--------|------|
| 列表加载 10 张图 | 50MB | 500KB | **99% ↓** |
| 加载时间（4G） | 10 秒 | 0.5 秒 | **95% ↓** |
| 用户流量消耗 | 50MB | 500KB | **99% ↓** |

**阿里云 OSS 图片处理支持的操作**：
- `resize` - 缩放
- `crop` - 裁剪
- `quality` - 压缩质量
- `format` - 格式转换（如转 WebP）
- `watermark` - 水印

**示例**：

```java
// 缩略图 + WebP 格式 + 80% 质量
String url = originalUrl + "?x-oss-process=image/resize,w_200/format,webp/quality,q_80";
```

---

#### 10.8.5 PostgreSQL JSONB 查询优化

##### 问题：JSONB 查询慢，索引不生效

**场景**：查询"所有吃过'牛肉'的用户"

```sql
-- ❌ 错误：不会使用 GIN 索引，全表扫描
SELECT * FROM tbl_diet_record 
WHERE food_items::text LIKE '%牛肉%';

-- ✅ 正确：使用 JSONB 操作符，触发 GIN 索引
SELECT * FROM tbl_diet_record 
WHERE food_items @> '[{"foodName": "牛肉"}]';
```

**索引创建**：

```sql
-- 创建 GIN 索引（已在设计中）
CREATE INDEX idx_diet_food_items ON tbl_diet_record USING GIN(food_items);

-- 针对特定字段创建表达式索引
CREATE INDEX idx_diet_food_name ON tbl_diet_record 
USING GIN((food_items -> 'foodName'));
```

**Java 查询示例**（MyBatis）：

```xml
<!-- MyBatis Mapper XML -->
<select id="findByFoodName" resultType="DietRecord">
    SELECT * FROM tbl_diet_record
    WHERE food_items @> CAST(#{foodItemJson} AS jsonb)
    AND user_id = #{userId}
    ORDER BY meal_time DESC
    LIMIT #{limit}
</select>
```

```java
// Service 层
public List<DietRecord> findByFoodName(String userId, String foodName, int limit) {
    // 构造 JSONB 查询条件
    String foodItemJson = String.format("[{\"foodName\": \"%s\"}]", foodName);
    return dietRecordMapper.findByFoodName(userId, foodItemJson, limit);
}
```

**JSONB 常用操作符**：

| 操作符 | 说明 | 示例 | 是否使用索引 |
|--------|------|------|------------|
| `@>` | 包含（左包含右） | `food_items @> '{"foodName":"牛肉"}'` | ✅ GIN |
| `<@` | 被包含（右包含左） | `'{"foodName":"牛肉"}' <@food_items` | ✅ GIN |
| `?` | 存在键 | `food_items ? 'foodName'` | ✅ GIN |
| `?|` | 存在任一键 | `food_items ?| array['foodName','weight']` | ✅ GIN |
| `?&` | 存在所有键 | `food_items ?& array['foodName','weight']` | ✅ GIN |
| `->` | 获取 JSON 字段 | `food_items -> 'foodName'` | ❌ |
| `->>` | 获取 JSON 文本 | `food_items ->> 'foodName'` | ❌ |

**复杂查询示例**：

```sql
-- 查询"早餐吃了牛肉且热量超过 500 千卡"的记录
SELECT * FROM tbl_diet_record
WHERE meal_type = 'breakfast'
  AND food_items @> '[{"foodName": "牛肉"}]'
  AND total_calories > 500
  AND user_id = 'user-123';

-- 查询"食物清单中包含'牛肉'或'鸡肉'"（需要自定义函数）
SELECT * FROM tbl_diet_record
WHERE user_id = 'user-123'
  AND (
    food_items @> '[{"foodName": "牛肉"}]'
    OR food_items @> '[{"foodName": "鸡肉"}]'
  );
```

**性能监控**：

```sql
-- 开启查询分析
EXPLAIN (ANALYZE, BUFFERS) 
SELECT * FROM tbl_diet_record
WHERE food_items @> '[{"foodName": "牛肉"}]';

-- 查看索引使用情况
SELECT 
    schemaname,
    tablename,
    indexname,
    idx_scan,  -- 索引被使用的次数
    idx_tup_read,
    idx_tup_fetch
FROM pg_stat_user_indexes
WHERE tablename = 'tbl_diet_record';
```

**建议**：
1. ✅ JSONB 字段写入时确保格式标准化（统一字段名、数据类型）
2. ✅ 查询时优先使用 `@>`、`?` 等操作符（触发 GIN 索引）
3. ✅ 避免使用 `::text` 转换后 LIKE 查询（不走索引）
4. ✅ 定期使用 `EXPLAIN ANALYZE` 分析查询计划

---

#### 10.8.6 综合避坑清单

| 问题分类 | 具体问题 | 影响 | 解决方案 | 优先级 |
|---------|---------|------|---------|--------|
| **SSE 连接** | 客户端断连未感知 | 浪费 AI Token 成本 | 注册 onError 回调，取消 Future | P0 |
| **SSE 超时** | 60秒超时不够用 | AI 推理被中断 | 延长到 180 秒或使用心跳 | P1 |
| **AI 上下文** | Token 消耗爆炸 | 成本增加 50%+ | 滑动窗口（保留 10 轮） | P0 |
| **数据迁移** | 事务注解失效 | 数据不一致 | 确保 public 方法 + 异常抛出 | P0 |
| **数据迁移** | 大量数据锁表 | 迁移超时 | 分批迁移（100 条/批） | P1 |
| **图片加载** | 列表加载大图 | 用户体验差 | OSS 缩略图（节省 99% 流量） | P1 |
| **JSONB 查询** | 索引不生效 | 查询慢 | 使用 `@>` 操作符 | P2 |

**P0 阶段必须实现**：
- ✅ SSE 断连回调（节省成本）
- ✅ AI 上下文滑动窗口（控制费用）
- ✅ 数据迁移事务保证（数据安全）

**P1 阶段优化**：
- ✅ OSS 缩略图（用户体验）
- ✅ SSE 心跳保活（稳定性）
- ✅ 分批数据迁移（避免锁表）

**P2 阶段增强**：
- ✅ JSONB 查询优化（高级功能）

---

### 10.9 安全与稳定性加固（🛡️ 防弹级）

#### 10.9.1 安全漏洞：越权访问（IDOR）防护

##### 🚨 高危漏洞：不要信任前端传递的 userId

**问题位置**：当前设计中的大量接口接受 `userId` 作为参数

```java
// ❌ 危险：容易被越权攻击
@GetMapping("/dashboard")
public ResponseEntity<DashboardResponse> getDashboard(@RequestParam String userId);

@PostMapping("/glucose/records")
public ResponseEntity<GlucoseRecordResponse> createRecord(@RequestBody CreateGlucoseRecordRequest request);
// CreateGlucoseRecordRequest 中包含 userId 字段
```

**攻击场景**：

```
1. 黑客 A 登录自己的账号，获取合法 Token
   Authorization: Bearer eyJhbGc...（A 的 Token）

2. A 拦截请求，篡改 userId 参数
   GET /api/v1/dashboard?userId=victim-user-123

3. 后端只校验 Token 是否有效（是），未校验 Token 归属
   → A 成功查看受害者 B 的数据 ✅ 越权成功！
```

**影响范围**：
- 查看他人血糖、饮食、病例等隐私数据 ⚠️
- 修改他人数据（如删除记录、修改订阅状态）⚠️⚠️
- 消费他人订阅权益 ⚠️⚠️⚠️

**CVSS 评分**：9.1（严重）- 未授权访问敏感数据

---

##### ✅ 解决方案：从 Token 中强制提取 userId

**方案架构**：

```
请求 → 网关/拦截器 → 解析 JWT → 提取 userId → 注入到请求上下文 → Controller/Service
```

**步骤 1：JWT 工具类**

```java
package com.tangxiaonuan.common.util;

/**
 * JWT 工具类
 */
@Component
public class JwtUtil {
    
    @Value("${jwt.secret}")
    private String secret;
    
    /**
     * 从 Token 中提取 userId
     * @param token JWT Token
     * @return userId
     * @throws JwtException Token 无效或过期
     */
    public String extractUserId(String token) {
        try {
            Claims claims = Jwts.parser()
                .setSigningKey(secret)
                .parseClaimsJws(token)
                .getBody();
            
            return claims.getSubject(); // userId 存储在 subject 中
            
        } catch (ExpiredJwtException e) {
            throw new UnauthorizedException("Token 已过期");
        } catch (JwtException e) {
            throw new UnauthorizedException("Token 无效");
        }
    }
    
    /**
     * 生成 Token
     * @param userId 用户ID
     * @param expirationSeconds 过期时间（秒）
     * @return JWT Token
     */
    public String generateToken(String userId, int expirationSeconds) {
        Date now = new Date();
        Date expiration = new Date(now.getTime() + expirationSeconds * 1000L);
        
        return Jwts.builder()
            .setSubject(userId)  // 🔥 userId 存储在 subject
            .setIssuedAt(now)
            .setExpiration(expiration)
            .signWith(SignatureAlgorithm.HS256, secret)
            .compact();
    }
}
```

**步骤 2：认证拦截器**

```java
package com.tangxiaonuan.common.interceptor;

/**
 * 认证拦截器
 * 从 Token 中提取 userId 并注入到请求上下文
 */
@Component
public class AuthenticationInterceptor implements HandlerInterceptor {
    
    @Autowired
    private JwtUtil jwtUtil;
    
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        // 1. 从 Header 中获取 Token
        String authHeader = request.getHeader("Authorization");
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            throw new UnauthorizedException("缺少 Authorization Header");
        }
        
        String token = authHeader.substring(7); // 去掉 "Bearer " 前缀
        
        // 2. 🔥 从 Token 中提取 userId（会校验签名和过期时间）
        String userId = jwtUtil.extractUserId(token);
        
        // 3. 🔥 将 userId 注入到请求上下文（线程安全）
        request.setAttribute("currentUserId", userId);
        
        // 也可以存到 ThreadLocal（推荐）
        UserContextHolder.setUserId(userId);
        
        log.debug("认证成功：userId={}", userId);
        return true;
    }
    
    @Override
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, 
                                Object handler, Exception ex) {
        // 清理 ThreadLocal，防止内存泄漏
        UserContextHolder.clear();
    }
}
```

**步骤 3：配置拦截器**

```java
@Configuration
public class WebMvcConfig implements WebMvcConfigurer {
    
    @Autowired
    private AuthenticationInterceptor authenticationInterceptor;
    
    @Override
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(authenticationInterceptor)
            .addPathPatterns("/api/v1/**")  // 拦截所有业务接口
            .excludePathPatterns(
                "/api/v1/users/guest/register",  // 访客注册不需要认证
                "/api/v1/subscriptions/plans",   // 套餐列表公开
                "/api/v1/health"                  // 健康检查
            );
    }
}
```

**步骤 4：Controller 改造**

```java
package com.tangxiaonuan.dashboard.controller;

/**
 * 首页控制器（安全版）
 */
@RestController
@RequestMapping("/api/v1/dashboard")
public class DashboardController {
    
    /**
     * 获取首页数据（安全版）
     * 
     * ❌ 错误写法：@RequestParam String userId
     * ✅ 正确写法：从请求上下文获取
     */
    @GetMapping
    public ResponseEntity<DashboardResponse> getDashboard(
        // 方案 A：从 Request Attribute 获取
        @RequestAttribute("currentUserId") String userId
    ) {
        return ResponseEntity.ok(dashboardService.aggregateDashboardData(userId));
    }
    
    // 方案 B：从 ThreadLocal 获取（推荐）
    @GetMapping("/v2")
    public ResponseEntity<DashboardResponse> getDashboardV2() {
        String userId = UserContextHolder.getUserId(); // 从 ThreadLocal 获取
        return ResponseEntity.ok(dashboardService.aggregateDashboardData(userId));
    }
}
```

**步骤 5：DTO 改造**

```java
/**
 * 创建血糖记录请求（安全版）
 */
@Data
public class CreateGlucoseRecordRequest {
    // ❌ 删除 userId 字段（不允许前端传递）
    // private String userId;
    
    private BigDecimal glucoseValue;
    private String measurementType;
    private LocalDateTime measurementTime;
    private String notes;
}

/**
 * Controller 层填充 userId
 */
@PostMapping("/records")
public ResponseEntity<GlucoseRecordResponse> createRecord(
    @RequestAttribute("currentUserId") String userId,
    @RequestBody CreateGlucoseRecordRequest request
) {
    // 🔥 在 Controller 层注入 userId，不信任前端
    return ResponseEntity.ok(glucoseService.createRecord(userId, request));
}

/**
 * Service 层接受 userId 作为独立参数
 */
public interface GlucoseService {
    GlucoseRecordResponse createRecord(String userId, CreateGlucoseRecordRequest request);
}
```

**步骤 6：UserContextHolder 工具类**

```java
package com.tangxiaonuan.common.context;

/**
 * 用户上下文持有者（ThreadLocal）
 */
public class UserContextHolder {
    
    private static final ThreadLocal<String> userIdHolder = new ThreadLocal<>();
    
    public static void setUserId(String userId) {
        userIdHolder.set(userId);
    }
    
    public static String getUserId() {
        String userId = userIdHolder.get();
        if (userId == null) {
            throw new UnauthorizedException("未登录或 Token 已过期");
        }
        return userId;
    }
    
    public static void clear() {
        userIdHolder.remove(); // 🔥 防止内存泄漏
    }
}
```

---

##### 📊 安全效果对比

| 场景 | 原设计 | 加固后 | 是否安全 |
|------|--------|--------|---------|
| 黑客篡改 userId 参数 | 可以查看他人数据 ❌ | Token 中的 userId 优先 ✅ | ✅ |
| 黑客使用他人 Token | Token 泄露后可越权 ❌ | 同样安全（Token 本身需要保护） | ⚠️ |
| 前端 Bug 传错 userId | 可能误操作他人数据 ❌ | 强制使用 Token 中的 ID ✅ | ✅ |

**注意**：Token 本身的安全也很重要：
- ✅ 使用 HTTPS 防止中间人攻击
- ✅ Token 有效期不要太长（建议 2 小时）
- ✅ 支持 Token 刷新机制（Refresh Token）
- ✅ 登出时将 Token 加入黑名单（Redis）

---

#### 10.9.2 并发隐患：SSE 线程池隔离

##### 🚨 问题：默认线程池耗尽导致应用假死

**问题位置**：第 10.2.2 节中的 SSE 实现

```java
// ❌ 危险：使用默认的 ForkJoinPool.commonPool()
CompletableFuture.runAsync(() -> {
    // 数字人对话（IO 密集型，耗时 5-30 秒）
    bailianAIClient.chatStream(...);
}, sseExecutor);  // 如果 sseExecutor 为 null，会使用 commonPool
```

**风险场景**：

```
时间点 0 秒：并发 50 个用户发起数字人对话
时间点 5 秒：ForkJoinPool.commonPool() 的线程全部被占用（默认 CPU 核心数 - 1）
时间点 10 秒：新用户的对话无法获得线程，排队等待
时间点 15 秒：应用中其他使用 CompletableFuture 的业务（异步邮件、异步日志）也被阻塞
时间点 20 秒：应用假死，所有异步任务堆积 ⚠️⚠️⚠️
```

**影响**：
- 数字人对话响应慢或超时
- 其他异步业务被拖累（雪崩效应）
- 线程池满后，新请求被拒绝（用户无法使用）

---

##### ✅ 解决方案：自定义线程池 + 隔离

**配置类**：

```java
package com.tangxiaonuan.common.config;

/**
 * 线程池配置
 * 为不同的异步任务创建独立的线程池，实现资源隔离
 */
@Configuration
public class ThreadPoolConfig {
    
    /**
     * 🔥 数字人 SSE 专用线程池
     * 
     * 特点：IO 密集型，单个任务耗时长（5-30 秒）
     * 策略：线程数多，队列容量适中
     */
    @Bean("sseExecutor")
    public Executor sseExecutor() {
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        
        // 核心线程数：建议为 CPU 核心数的 2-3 倍（IO 密集型）
        executor.setCorePoolSize(20);
        
        // 最大线程数：峰值并发数（根据实际压测调整）
        executor.setMaxPoolSize(100);
        
        // 队列容量：等待队列大小
        // 当核心线程满时，任务先入队；队列满后才创建新线程
        executor.setQueueCapacity(200);
        
        // 线程名称前缀（便于排查问题）
        executor.setThreadNamePrefix("sse-chat-");
        
        // 线程空闲时间（超过核心线程数的线程在空闲 60 秒后回收）
        executor.setKeepAliveSeconds(60);
        
        // 🔥 拒绝策略：当线程池和队列都满时
        // CallerRunsPolicy：由调用线程执行（降级方案，不丢弃任务）
        executor.setRejectedExecutionHandler(new ThreadPoolExecutor.CallerRunsPolicy());
        
        // 关闭应用时等待任务完成
        executor.setWaitForTasksToCompleteOnShutdown(true);
        executor.setAwaitTerminationSeconds(60);
        
        executor.initialize();
        return executor;
    }
    
    /**
     * 通用异步任务线程池
     * 用于异步日志、异步通知等轻量级任务
     */
    @Bean("asyncExecutor")
    public Executor asyncExecutor() {
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(10);
        executor.setMaxPoolSize(20);
        executor.setQueueCapacity(500);
        executor.setThreadNamePrefix("async-");
        executor.setRejectedExecutionHandler(new ThreadPoolExecutor.AbortPolicy()); // 丢弃任务并抛异常
        executor.initialize();
        return executor;
    }
    
    /**
     * 数据迁移专用线程池
     * 用于访客转正时的大量数据迁移
     */
    @Bean("migrationExecutor")
    public Executor migrationExecutor() {
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(5);  // 迁移任务较少，核心线程数少
        executor.setMaxPoolSize(10);
        executor.setQueueCapacity(50);
        executor.setThreadNamePrefix("migration-");
        executor.setRejectedExecutionHandler(new ThreadPoolExecutor.CallerRunsPolicy());
        executor.initialize();
        return executor;
    }
}
```

**Controller 使用**：

```java
@RestController
@RequestMapping("/api/v1/digital-human")
public class DigitalHumanController {
    
    @Autowired
    @Qualifier("sseExecutor")  // 🔥 指定使用 sseExecutor
    private Executor sseExecutor;
    
    @PostMapping(value = "/sessions/{sessionId}/messages/stream", 
                 produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public SseEmitter streamMessage(@PathVariable String sessionId,
                                    @RequestBody SendMessageRequest request) {
        SseEmitter emitter = new SseEmitter(180000L);
        
        // 🔥 使用自定义线程池
        CompletableFuture.runAsync(() -> {
            try {
                // 数字人对话处理
                bailianAIClient.chatStream(...);
            } catch (Exception e) {
                emitter.completeWithError(e);
            }
        }, sseExecutor);  // 🔥 传入自定义线程池
        
        return emitter;
    }
}
```

---

##### 📊 线程池配置对比

| 配置项 | 默认值（commonPool） | 自定义配置（sseExecutor） | 说明 |
|--------|-------------------|---------------------|------|
| 核心线程数 | CPU 核心数 - 1（如 7） | 20 | IO 密集型需要更多线程 |
| 最大线程数 | CPU 核心数 - 1 | 100 | 支持峰值并发 |
| 队列容量 | 无界队列 | 200 | 防止无限堆积 |
| 拒绝策略 | 不支持 | CallerRunsPolicy | 降级方案 |
| 线程名称 | ForkJoinPool.commonPool-worker-X | sse-chat-X | 便于监控排查 |

**效果**：
- ✅ 数字人对话不影响其他业务
- ✅ 线程数可根据压测调优
- ✅ 拒绝策略保证服务不崩溃
- ✅ 监控和排查问题更容易

---

##### 🔍 监控与告警

**线程池监控指标**：

```java
@Component
public class ThreadPoolMonitor {
    
    @Autowired
    @Qualifier("sseExecutor")
    private ThreadPoolTaskExecutor sseExecutor;
    
    /**
     * 定时上报线程池指标（每 30 秒）
     */
    @Scheduled(fixedRate = 30000)
    public void reportMetrics() {
        ThreadPoolExecutor pool = sseExecutor.getThreadPoolExecutor();
        
        int activeCount = pool.getActiveCount();        // 活跃线程数
        int poolSize = pool.getPoolSize();              // 当前线程数
        int corePoolSize = pool.getCorePoolSize();      // 核心线程数
        int maximumPoolSize = pool.getMaximumPoolSize(); // 最大线程数
        long taskCount = pool.getTaskCount();           // 总任务数
        long completedTaskCount = pool.getCompletedTaskCount(); // 完成任务数
        int queueSize = pool.getQueue().size();         // 队列中等待任务数
        
        log.info("SSE线程池监控 - 活跃:{}/{}, 队列:{}, 总任务:{}, 完成:{}", 
            activeCount, maximumPoolSize, queueSize, taskCount, completedTaskCount);
        
        // 🔥 告警：活跃线程数超过 80%
        if (activeCount > maximumPoolSize * 0.8) {
            log.warn("⚠️ SSE线程池负载高：活跃线程 {}/{}", activeCount, maximumPoolSize);
            // 发送告警通知（钉钉、邮件等）
        }
        
        // 🔥 告警：队列堆积超过 80%
        if (queueSize > sseExecutor.getQueueCapacity() * 0.8) {
            log.warn("⚠️ SSE任务队列堆积：{}/{}", queueSize, sseExecutor.getQueueCapacity());
        }
    }
}
```

---

#### 10.9.3 AI 逻辑优化：在 System Prompt 中注入时间

##### 🤔 问题：解析自然语言时间表达式很复杂

**问题位置**：第 2.2.2 节 `AgentToolExecutor.parseTimeExpression()` 方法

```java
// ❌ 复杂：需要 NLP 库或大量规则
private LocalDateTime parseTimeExpression(String timeStr) {
    // "刚才" -> 当前时间
    // "早上8点" -> 今天 08:00
    // "昨天晚上" -> 昨天 20:00
    // "前天中午" -> 前天 12:00
    // "上周三下午3点半" -> ...（非常复杂）
}
```

**困难点**：
- 需要引入 NLP 库（如 HanLP）或时间语义解析库
- 规则复杂，边界情况多（闰年、时区、节假日）
- 不同地区表达习惯不同（"今早"、"今儿个"）
- 代码维护成本高

---

##### ✅ 解决方案：让 AI 自己计算时间

**核心思路**：在 System Prompt 中注入当前时间，让百炼 AI 自己将自然语言转为标准时间。

**实现步骤**：

**步骤 1：动态生成 System Prompt**

```java
@Service
public class PromptBuilder {
    
    /**
     * 构建数字人 System Prompt（动态注入时间）
     */
    public String buildDigitalHumanSystemPrompt(String sceneType) {
        // 🔥 获取当前时间
        LocalDateTime now = LocalDateTime.now();
        String currentTime = now.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss EEEE", Locale.CHINA));
        // 例如："2026-01-29 14:30:00 星期三"
        
        // 基础 Prompt
        StringBuilder prompt = new StringBuilder();
        prompt.append("You are Dr. Xu (徐医生), a professional diabetes management assistant.\n");
        prompt.append("\n");
        
        // 🔥 注入当前时间
        prompt.append("=== SYSTEM INFORMATION ===\n");
        prompt.append("Current System Time: ").append(currentTime).append("\n");
        prompt.append("Timezone: Asia/Shanghai (UTC+8)\n");
        prompt.append("\n");
        
        prompt.append("=== YOUR MISSION ===\n");
        prompt.append("Help users record their blood sugar, diet, and exercise data.\n");
        prompt.append("When user mentions time like '刚才', '昨天晚上', '今早', you MUST calculate the actual datetime based on Current System Time.\n");
        prompt.append("\n");
        
        prompt.append("=== TOOLS AVAILABLE ===\n");
        prompt.append("- save_blood_sugar: Save blood glucose record\n");
        prompt.append("  - Parameters: userId, value, measurementType, measurementTime (ISO 8601 format: YYYY-MM-DD HH:mm:ss)\n");
        prompt.append("- save_diet: Save diet record\n");
        prompt.append("  - Parameters: userId, mealType, foodItems, mealTime (ISO 8601 format)\n");
        prompt.append("\n");
        
        prompt.append("=== IMPORTANT RULES ===\n");
        prompt.append("1. When user says '刚才' or '刚刚', use current time.\n");
        prompt.append("2. When user says '早上8点', calculate today 08:00:00.\n");
        prompt.append("3. When user says '昨天晚上', calculate yesterday 20:00:00 (estimate).\n");
        prompt.append("4. When calling tools, measurementTime/mealTime MUST be in format 'YYYY-MM-DD HH:mm:ss'.\n");
        prompt.append("5. Always ask user to confirm if the time is ambiguous.\n");
        
        return prompt.toString();
    }
}
```

**步骤 2：调用百炼 API 时传入**

```java
@Service
public class BailianAIClient {
    
    @Autowired
    private PromptBuilder promptBuilder;
    
    public void chatStream(String sessionId, String userMessage, Consumer<StreamResponse> callback) {
        // 1. 🔥 构建动态 System Prompt
        String systemPrompt = promptBuilder.buildDigitalHumanSystemPrompt("glucose_record");
        
        // 2. 构建消息列表
        List<Message> messages = new ArrayList<>();
        messages.add(new Message("system", systemPrompt)); // System Prompt
        messages.add(new Message("user", userMessage));    // 用户消息
        
        // 3. 调用百炼流式 API
        BailianRequest request = new BailianRequest();
        request.setModel("qwen-plus");
        request.setMessages(messages);
        request.setTools(getToolDefinitions()); // Function Calling 工具定义
        request.setStream(true);
        
        // 4. 发送请求并处理流式响应
        bailianApiClient.chatCompletionStream(request, callback);
    }
}
```

**步骤 3：AI 自动计算时间**

**对话示例**：

```
用户："我昨天晚上 8 点测的血糖是 6.5"

AI 推理（内部）：
- Current System Time: 2026-01-29 14:30:00 星期三
- "昨天晚上 8 点" → 2026-01-28 20:00:00

AI 返回（Function Call）：
{
  "tool_name": "save_blood_sugar",
  "parameters": {
    "userId": "user-123",
    "value": 6.5,
    "measurementType": "random",
    "measurementTime": "2026-01-28 20:00:00"  // 🔥 AI 已计算好
  }
}

后端 Java：
String timeStr = params.get("measurementTime"); // "2026-01-28 20:00:00"
LocalDateTime time = LocalDateTime.parse(timeStr, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
// ✅ 简单的字符串解析即可，不需要复杂的 NLP
```

---

**步骤 4：简化后端代码**

```java
@AgentTool(name = "save_blood_sugar")
public String saveBloodSugar(
    @Param("userId") String userId,
    @Param("value") Double glucoseValue,
    @Param("measurementType") String measurementType,
    @Param("measurementTime") String measurementTimeStr
) {
    try {
        // 🔥 简化：直接解析标准格式（AI 已经计算好了）
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        LocalDateTime measurementTime = LocalDateTime.parse(measurementTimeStr, formatter);
        
        // 保存记录
        CreateGlucoseRecordRequest request = new CreateGlucoseRecordRequest();
        request.setGlucoseValue(new BigDecimal(glucoseValue));
        request.setMeasurementType(measurementType);
        request.setMeasurementTime(measurementTime);
        
        glucoseService.createRecord(userId, request);
        
        return String.format("已成功记录血糖值 %.1f mmol/L", glucoseValue);
        
    } catch (DateTimeParseException e) {
        // 如果 AI 返回的格式错误，要求 AI 重新生成
        log.error("AI 返回的时间格式错误：{}", measurementTimeStr, e);
        return "时间格式错误，请重新提供标准格式：YYYY-MM-DD HH:mm:ss";
    }
}
```

---

##### 📊 方案对比

| 方案 | 实现难度 | 准确性 | 可维护性 | 扩展性 |
|------|---------|-------|---------|--------|
| **后端 NLP 解析** | ⭐⭐⭐⭐⭐ 很高 | ⭐⭐⭐ 中等 | ⭐⭐ 低 | ⭐⭐ 低 |
| **AI 自动计算** | ⭐ 很低 | ⭐⭐⭐⭐ 高 | ⭐⭐⭐⭐⭐ 很高 | ⭐⭐⭐⭐⭐ 很高 |

**优势**：
- ✅ 后端代码简单：只需要 `LocalDateTime.parse()`
- ✅ AI 理解能力强：可以处理各种口语表达
- ✅ 易于扩展：新增时间表达方式无需改代码
- ✅ 多语言支持：AI 天然支持多语言时间表达

**示例对话（各种复杂表达）**：

```
用户："我刚测的"
AI 计算：2026-01-29 14:30:00（当前时间）

用户："我早上空腹测的"
AI 计算：2026-01-29 07:00:00（今天早上）

用户："我前天下午 3 点半测的"
AI 计算：2026-01-27 15:30:00

用户："我上周五晚饭后测的"
AI 计算：2026-01-24 19:30:00（估算）
AI 回复："您说的是上周五（1月24日）晚饭后吗？大约晚上7点半？"（二次确认）
```

---

##### 🔧 Prompt 工程技巧

**技巧 1：明确输出格式要求**

```
When calling tools, measurementTime MUST be in format 'YYYY-MM-DD HH:mm:ss'.
❌ BAD: "2026-1-29 8:0:0"
✅ GOOD: "2026-01-29 08:00:00"
```

**技巧 2：模糊时间的处理策略**

```
If user says vague time like "早上" without specific hour:
- 空腹：07:00
- 早餐后：08:30
- 午餐前：11:30
- 午餐后：13:00
- 晚餐前：17:30
- 晚餐后：19:00
- 睡前：22:00

Always ask user to confirm if unsure.
```

**技巧 3：异常时间的校验提示**

```
If calculated time is:
- In the future: Ask user "您是说今天（未来）还是昨天？"
- More than 7 days ago: Ask user to confirm the date
- Outside reasonable range (like 03:00 AM for meal): Double check with user
```

---

### 10.10 防弹加固总结

| 加固项 | 风险等级 | 实现难度 | 投入时间 | 优先级 |
|--------|---------|---------|---------|--------|
| **越权访问（IDOR）防护** | 🔴 严重（9.1/10） | ⭐⭐ 低 | 4 小时 | **P0** |
| **SSE 线程池隔离** | 🟡 中等（5.5/10） | ⭐⭐ 低 | 2 小时 | **P0** |
| **AI 时间注入优化** | 🟢 低（2.0/10） | ⭐ 很低 | 1 小时 | **P1** |

**总投入**：约 7 小时（P0 项目必须完成前两项）

**收益**：
- 🛡️ **安全性**：防止越权访问，保护用户隐私
- 🚀 **稳定性**：防止线程池耗尽，应用不假死
- 💡 **可维护性**：AI 自动计算时间，减少代码复杂度

---

## 11. 医疗业务特殊防护（🏥 生命线）

### 11.1 危急值实时阻断（🚨 最高优先级）

#### 11.1.1 业务致命伤：缺少危急值预警

**问题描述**：当前设计中，AI 只是机械地记录数据，未对危急值进行拦截。

**致命场景**：

```
用户："我刚测的血糖是 2.1"
AI："好的，已为您记录血糖 2.1 mmol/L。"

实际：2.1 mmol/L 是严重低血糖，用户可能随时昏迷！⚠️⚠️⚠️
```

**风险评估**：
- **法律风险**：医疗健康类 App 未能及时预警危急值，可能承担连带责任
- **伦理责任**：作为健康管理工具，有义务保护用户生命安全
- **品牌风险**：一旦发生事故，品牌信誉将受到严重打击

---

#### 11.1.2 医疗危急值标准

**血糖危急值（参考中华医学会标准）**：

| 级别 | 范围（mmol/L） | 危险程度 | 临床表现 | 处理措施 |
|------|--------------|---------|---------|---------|
| **危急低值** | < 2.8 | 极高 | 意识模糊、昏迷 | 🚨 立即补糖 + 就医 |
| **低血糖** | 2.8 - 3.9 | 高 | 心悸、出汗、饥饿 | ⚠️ 补糖 + 15分钟后复测 |
| **正常空腹** | 3.9 - 6.1 | 正常 | - | ✅ 继续保持 |
| **正常餐后** | < 7.8 | 正常 | - | ✅ 继续保持 |
| **偏高** | 7.8 - 11.1 | 轻度 | 口渴、尿频 | 💡 调整饮食/运动 |
| **高血糖** | 11.1 - 16.7 | 中度 | 乏力、多尿 | ⚠️ 监测酮体 + 就医 |
| **危急高值** | > 16.7 | 极高 | 酮症酸中毒风险 | 🚨 立即就医 |

---

#### 11.1.3 完整实现方案

**步骤 1：定义危急值常量**

```java
package com.tangxiaonuan.glucose.constants;

/**
 * 血糖危急值常量
 */
public class GlucoseCriticalValues {
    
    // 危急低值（严重低血糖）
    public static final BigDecimal CRITICAL_LOW = new BigDecimal("2.8");
    
    // 低血糖阈值
    public static final BigDecimal LOW_THRESHOLD = new BigDecimal("3.9");
    
    // 空腹正常上限
    public static final BigDecimal FASTING_NORMAL_HIGH = new BigDecimal("6.1");
    
    // 餐后正常上限
    public static final BigDecimal POSTPRANDIAL_NORMAL_HIGH = new BigDecimal("7.8");
    
    // 高血糖阈值
    public static final BigDecimal HIGH_THRESHOLD = new BigDecimal("11.1");
    
    // 危急高值（酮症酸中毒风险）
    public static final BigDecimal CRITICAL_HIGH = new BigDecimal("16.7");
}
```

**步骤 2：创建告警服务**

```java
package com.tangxiaonuan.alert.service;

/**
 * 紧急告警服务
 */
@Service
@Slf4j
public class EmergencyAlertService {
    
    @Autowired
    private NotificationService notificationService;
    
    @Autowired
    private RedisTemplate<String, Object> redisTemplate;
    
    /**
     * 触发紧急告警
     * @param userId 用户ID
     * @param alertType 告警类型
     * @param value 数值
     * @param metadata 元数据
     */
    public void triggerEmergencyAlert(String userId, String alertType, 
                                     BigDecimal value, Map<String, Object> metadata) {
        // 1. 记录告警
        EmergencyAlert alert = new EmergencyAlert();
        alert.setAlertId(UUID.randomUUID().toString());
        alert.setUserId(userId);
        alert.setAlertType(alertType);
        alert.setValue(value);
        alert.setMetadata(metadata);
        alert.setSeverity(calculateSeverity(alertType, value));
        alert.setCreatedAt(LocalDateTime.now());
        
        emergencyAlertMapper.insert(alert);
        
        // 2. 🔥 推送实时通知到前端（WebSocket/SSE）
        notificationService.sendRealtimeAlert(userId, alert);
        
        // 3. 发送短信通知（危急值）
        if (alert.getSeverity().equals("CRITICAL")) {
            smsService.sendEmergencySms(userId, alertType, value);
        }
        
        // 4. 记录到 Redis（用于限流，防止重复告警）
        String lockKey = String.format("alert:lock:%s:%s", userId, alertType);
        redisTemplate.opsForValue().set(lockKey, "1", 15, TimeUnit.MINUTES);
        
        log.error("🚨 紧急告警触发：userId={}, type={}, value={}", userId, alertType, value);
    }
    
    /**
     * 检查是否需要告警（防止频繁告警）
     */
    public boolean shouldAlert(String userId, String alertType) {
        String lockKey = String.format("alert:lock:%s:%s", userId, alertType);
        return !redisTemplate.hasKey(lockKey);
    }
    
    /**
     * 计算告警严重程度
     */
    private String calculateSeverity(String alertType, BigDecimal value) {
        if (alertType.equals("低血糖")) {
            if (value.compareTo(CRITICAL_LOW) < 0) {
                return "CRITICAL";  // 危急
            } else {
                return "WARNING";   // 警告
            }
        }
        // ... 其他告警类型
        return "INFO";
    }
}
```

**步骤 3：改造 AgentToolExecutor**

```java
@Service
public class AgentToolExecutor {
    
    @Autowired
    private EmergencyAlertService alertService;
    
    @AgentTool(name = "save_blood_sugar")
    public String saveBloodSugar(
        @Param("userId") String userId,
        @Param("value") Double glucoseValue,
        @Param("measurementType") String measurementType,
        @Param("measurementTime") String measurementTimeStr
    ) {
        BigDecimal value = new BigDecimal(glucoseValue);
        
        // 1. 保存记录（原逻辑）
        CreateGlucoseRecordRequest request = new CreateGlucoseRecordRequest();
        request.setGlucoseValue(value);
        request.setMeasurementType(measurementType);
        request.setMeasurementTime(LocalDateTime.parse(measurementTimeStr, 
            DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
        
        glucoseService.createRecord(userId, request);
        
        // 2. 🔥 危急值判断（核心逻辑）
        
        // 2.1 危急低值（< 2.8）
        if (value.compareTo(CRITICAL_LOW) < 0) {
            if (alertService.shouldAlert(userId, "低血糖")) {
                alertService.triggerEmergencyAlert(userId, "低血糖", value, 
                    Map.of("measurementType", measurementType));
            }
            
            // 🔥 返回给 AI 的紧急指令（强制打断常规对话）
            return String.format(
                "【🚨 系统紧急警告】\n" +
                "检测到严重低血糖：%.1f mmol/L（危急值 < 2.8）\n\n" +
                "请立即执行以下急救措施：\n" +
                "1. 立即进食 15-20 克快速碳水化合物（半杯果汁、3-4 颗方糖、或 1 汤匙蜂蜜）\n" +
                "2. 15 分钟后复测血糖\n" +
                "3. 如血糖仍低于 3.9，重复步骤 1\n" +
                "4. 如出现意识模糊、无法进食，请立即拨打 120 急救电话\n\n" +
                "请不要说任何客套话，专注于急救指导！", 
                glucoseValue
            );
        }
        
        // 2.2 低血糖（2.8 - 3.9）
        if (value.compareTo(LOW_THRESHOLD) < 0) {
            if (alertService.shouldAlert(userId, "低血糖")) {
                alertService.triggerEmergencyAlert(userId, "低血糖", value, 
                    Map.of("measurementType", measurementType));
            }
            
            return String.format(
                "【⚠️ 血糖偏低警告】\n" +
                "您的血糖为 %.1f mmol/L，低于正常范围（3.9-6.1）。\n\n" +
                "建议立即补充 15 克碳水化合物（如 1 片全麦面包或 1 个苹果），" +
                "并在 15 分钟后复测血糖。", 
                glucoseValue
            );
        }
        
        // 2.3 危急高值（> 16.7）
        if (value.compareTo(CRITICAL_HIGH) > 0) {
            if (alertService.shouldAlert(userId, "高血糖")) {
                alertService.triggerEmergencyAlert(userId, "高血糖", value, 
                    Map.of("measurementType", measurementType));
            }
            
            return String.format(
                "【🚨 系统紧急警告】\n" +
                "检测到危险高血糖：%.1f mmol/L（危急值 > 16.7）\n\n" +
                "请立即采取以下措施：\n" +
                "1. 检查是否有酮体（尿酮试纸或血酮仪）\n" +
                "2. 大量饮水（每小时 500ml）\n" +
                "3. 如有酮体阳性，立即就医\n" +
                "4. 如出现恶心、呕吐、腹痛、呼吸困难，立即拨打 120\n\n" +
                "警告：请勿擅自增加胰岛素剂量，必须在医生指导下用药！", 
                glucoseValue
            );
        }
        
        // 2.4 高血糖（11.1 - 16.7）
        if (value.compareTo(HIGH_THRESHOLD) > 0) {
            return String.format(
                "【⚠️ 血糖偏高提示】\n" +
                "您的血糖为 %.1f mmol/L，高于正常范围。\n\n" +
                "建议：\n" +
                "1. 多喝水，促进血糖排泄\n" +
                "2. 进行轻度运动（如散步 30 分钟）\n" +
                "3. 下一餐减少碳水摄入\n" +
                "4. 如持续 3 天以上血糖偏高，请咨询医生调整用药", 
                glucoseValue
            );
        }
        
        // 2.5 正常范围
        return String.format(
            "已成功记录血糖值 %.1f mmol/L（%s）。您的血糖在正常范围内，请继续保持！", 
            glucoseValue, 
            getMeasurementTypeText(measurementType)
        );
    }
}
```

**步骤 4：前端实时告警展示**

```javascript
// 微信小程序前端接收告警
uni.onSocketMessage((res) => {
  const alert = JSON.parse(res.data);
  
  if (alert.type === 'emergency_alert') {
    // 🔥 弹出全屏告警（无法关闭，必须用户确认）
    uni.showModal({
      title: '🚨 紧急健康警告',
      content: alert.message,
      showCancel: false,
      confirmText: '我知道了',
      success: (res) => {
        // 用户确认后，记录确认时间
        confirmAlert(alert.alertId);
      }
    });
    
    // 震动提示
    uni.vibrateShort({ type: 'heavy' });
    
    // 播放告警音效
    const audio = uni.createInnerAudioContext();
    audio.src = '/static/sounds/emergency.mp3';
    audio.play();
  }
});
```

---

#### 11.1.4 告警数据库设计

```sql
CREATE TABLE tbl_emergency_alert (
    alert_id VARCHAR(36) PRIMARY KEY,
    user_id VARCHAR(36) NOT NULL,
    alert_type VARCHAR(50) NOT NULL,  -- 低血糖/高血糖/其他
    value NUMERIC(5, 2) NOT NULL,     -- 触发值
    severity VARCHAR(20) NOT NULL,    -- CRITICAL/WARNING/INFO
    metadata JSONB,                   -- 元数据（测量类型、时间等）
    is_confirmed BOOLEAN DEFAULT FALSE, -- 用户是否确认
    confirmed_at TIMESTAMP WITH TIME ZONE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_emergency_alert_user_id ON tbl_emergency_alert(user_id);
CREATE INDEX idx_emergency_alert_created_at ON tbl_emergency_alert(created_at);
CREATE INDEX idx_emergency_alert_severity ON tbl_emergency_alert(severity);

COMMENT ON TABLE tbl_emergency_alert IS '🚨 紧急告警记录表';
COMMENT ON COLUMN tbl_emergency_alert.severity IS '严重程度：CRITICAL-危急，WARNING-警告，INFO-提示';
```

---

### 11.2 血糖单位地狱防护

#### 11.2.1 问题：单位混淆导致致命错误

**背景**：
- **中国标准**：mmol/L（毫摩尔/升）
- **美国标准**：mg/dL（毫克/分升）
- **转换系数**：1 mmol/L ≈ 18 mg/dL

**致命场景**：

```
场景 1：用户看血糖仪显示 100 (mg/dL，正常)
用户对 AI 说："我血糖 100"
AI 记录：100 mmol/L（相当于 1800 mg/dL，致死量！）❌

场景 2：用户说"5.6"
系统误判为 mg/dL（极低血糖）❌
实际：用户是中国用户，使用 mmol/L（正常）
```

---

#### 11.2.2 完整解决方案

**方案 1：AI Prompt 增强**

```java
@Service
public class PromptBuilder {
    
    public String buildDigitalHumanSystemPrompt(String sceneType) {
        StringBuilder prompt = new StringBuilder();
        
        // ... 其他 Prompt 内容 ...
        
        // 🔥 增加单位识别规则
        prompt.append("=== GLUCOSE UNIT DETECTION RULES ===\n");
        prompt.append("China uses mmol/L, USA uses mg/dL. 1 mmol/L ≈ 18 mg/dL.\n");
        prompt.append("\n");
        prompt.append("Detection Logic:\n");
        prompt.append("1. If user says a number > 30, it's LIKELY mg/dL.\n");
        prompt.append("   Example: '血糖 100' → Likely 100 mg/dL (≈ 5.6 mmol/L)\n");
        prompt.append("\n");
        prompt.append("2. If user says a number < 30, it's LIKELY mmol/L.\n");
        prompt.append("   Example: '血糖 5.6' → Likely 5.6 mmol/L\n");
        prompt.append("\n");
        prompt.append("3. When in doubt, ASK USER to confirm the unit:\n");
        prompt.append("   '请问您的血糖单位是 mmol/L 还是 mg/dL？'\n");
        prompt.append("\n");
        prompt.append("4. When calling save_blood_sugar, include 'unit' parameter.\n");
        prompt.append("   Example: {value: 100, unit: 'mg/dL'}\n");
        
        return prompt.toString();
    }
}
```

**方案 2：工具增加 unit 参数**

```java
@AgentTool(name = "save_blood_sugar")
public String saveBloodSugar(
    @Param("userId") String userId,
    @Param("value") Double glucoseValue,
    @Param("unit") String unit,  // 🔥 新增：单位参数
    @Param("measurementType") String measurementType,
    @Param("measurementTime") String measurementTimeStr
) {
    BigDecimal value = new BigDecimal(glucoseValue);
    
    // 1. 🔥 单位归一化（统一转换为 mmol/L 存储）
    BigDecimal normalizedValue;
    String originalUnit = unit;
    
    if ("mg/dL".equalsIgnoreCase(unit) || "mg/dl".equalsIgnoreCase(unit)) {
        // mg/dL 转 mmol/L：除以 18
        normalizedValue = value.divide(new BigDecimal("18"), 2, RoundingMode.HALF_UP);
        log.info("单位转换：{} mg/dL → {} mmol/L", value, normalizedValue);
        
    } else if ("mmol/L".equalsIgnoreCase(unit) || "mmol/l".equalsIgnoreCase(unit)) {
        // 已经是 mmol/L，无需转换
        normalizedValue = value;
        
    } else {
        // 未知单位，根据数值大小推断
        if (value.compareTo(new BigDecimal("30")) > 0) {
            // 大于 30，推断为 mg/dL
            normalizedValue = value.divide(new BigDecimal("18"), 2, RoundingMode.HALF_UP);
            originalUnit = "mg/dL (推断)";
            log.warn("未指定单位，根据数值推断为 mg/dL：{} → {} mmol/L", value, normalizedValue);
        } else {
            // 小于 30，推断为 mmol/L
            normalizedValue = value;
            originalUnit = "mmol/L (推断)";
        }
    }
    
    // 2. 保存记录（存储归一化后的值）
    CreateGlucoseRecordRequest request = new CreateGlucoseRecordRequest();
    request.setGlucoseValue(normalizedValue);  // 存储 mmol/L
    request.setMeasurementType(measurementType);
    request.setMeasurementTime(LocalDateTime.parse(measurementTimeStr, 
        DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
    
    // 3. 在备注中保留原始单位和原始值
    request.setNotes(String.format("原始值：%.1f %s", glucoseValue, originalUnit));
    
    glucoseService.createRecord(userId, request);
    
    // 4. 危急值判断（使用归一化后的值）
    // ... （危急值逻辑保持不变）...
    
    // 5. 返回时使用用户熟悉的单位
    if (originalUnit.startsWith("mg/dL")) {
        return String.format(
            "已成功记录血糖值 %.0f mg/dL（约 %.1f mmol/L）。", 
            glucoseValue, 
            normalizedValue
        );
    } else {
        return String.format(
            "已成功记录血糖值 %.1f mmol/L。", 
            normalizedValue
        );
    }
}
```

**方案 3：数据库存储策略**

```sql
-- 在 tbl_glucose_record 表中增加字段
ALTER TABLE tbl_glucose_record 
ADD COLUMN original_value NUMERIC(6, 2),  -- 原始值
ADD COLUMN original_unit VARCHAR(20);      -- 原始单位

COMMENT ON COLUMN tbl_glucose_record.glucose_value IS '血糖值（归一化为 mmol/L）';
COMMENT ON COLUMN tbl_glucose_record.original_value IS '原始值（用户输入）';
COMMENT ON COLUMN tbl_glucose_record.original_unit IS '原始单位（mmol/L 或 mg/dL）';
```

---

### 11.3 分布式数据迁移原子性保证

#### 11.3.1 问题：分布式环境下的状态不一致

**风险场景**：

```
时间 0 秒：开始执行 convertGuestToUser
时间 1 秒：更新用户类型为 normal ✅
时间 2 秒：迁移血糖记录（1000 条）✅
时间 3 秒：迁移饮食记录...
时间 3.5 秒：服务器 OOM，进程崩溃 ❌

结果：
- 用户类型已改为 normal（事务回滚失败，因为是跨多个表）
- 血糖记录已迁移
- 饮食、运动记录未迁移（数据分裂）
```

---

#### 11.3.2 解决方案：状态机 + 幂等性

**步骤 1：数据库增加状态字段**

```sql
-- 为用户表增加迁移状态字段
ALTER TABLE tbl_user 
ADD COLUMN migration_status VARCHAR(20) DEFAULT 'NONE',  -- NONE/PROCESSING/DONE
ADD COLUMN migration_started_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN migration_completed_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN migration_retry_count INTEGER DEFAULT 0;

CREATE INDEX idx_user_migration_status ON tbl_user(migration_status);

COMMENT ON COLUMN tbl_user.migration_status IS '数据迁移状态：NONE-未迁移，PROCESSING-迁移中，DONE-已完成';
```

**步骤 2：改造迁移逻辑**

```java
@Service
public class UserServiceImpl implements UserService {
    
    @Override
    @Transactional(rollbackFor = Exception.class)
    public UserResponse convertGuestToUser(String guestUserId, ConvertUserRequest request) {
        // 1. 查询访客用户
        User guestUser = userMapper.selectByUserId(guestUserId);
        if (guestUser == null || !guestUser.getUserType().equals("guest")) {
            throw new BusinessException("访客用户不存在或已转正");
        }
        
        // 2. 🔥 检查迁移状态（幂等性保证）
        if ("DONE".equals(guestUser.getMigrationStatus())) {
            log.info("用户已完成迁移，跳过：userId={}", guestUserId);
            return buildUserResponse(guestUser);
        }
        
        // 3. 🔥 标记为迁移中（原子操作）
        int updated = userMapper.updateMigrationStatus(guestUserId, "NONE", "PROCESSING");
        if (updated == 0) {
            // 其他线程正在处理，直接返回
            throw new BusinessException("迁移正在进行中，请稍后");
        }
        
        guestUser.setMigrationStatus("PROCESSING");
        guestUser.setMigrationStartedAt(LocalDateTime.now());
        userMapper.updateByUserId(guestUser);
        
        try {
            // 4. 执行迁移（策略 A 或策略 B）
            String targetUserId = determineTargetUserId(guestUserId, request);
            
            // 5. 🔥 分步迁移（每步都是幂等的）
            migrateGlucoseRecords(guestUserId, targetUserId);
            migrateDietRecords(guestUserId, targetUserId);
            migrateExerciseRecords(guestUserId, targetUserId);
            migrateConsultationRecords(guestUserId, targetUserId);
            
            // 6. 🔥 标记为完成
            guestUser.setMigrationStatus("DONE");
            guestUser.setMigrationCompletedAt(LocalDateTime.now());
            userMapper.updateByUserId(guestUser);
            
            log.info("用户迁移完成：guestUserId={}, targetUserId={}", guestUserId, targetUserId);
            return buildUserResponse(userMapper.selectByUserId(targetUserId));
            
        } catch (Exception e) {
            // 7. 🔥 失败回滚状态（保留 PROCESSING，等待重试）
            log.error("用户迁移失败：guestUserId=" + guestUserId, e);
            
            // 增加重试计数
            guestUser.setMigrationRetryCount(guestUser.getMigrationRetryCount() + 1);
            userMapper.updateByUserId(guestUser);
            
            throw new BusinessException("数据迁移失败，请稍后重试", e);
        }
    }
    
    /**
     * 🔥 迁移血糖记录（幂等操作）
     */
    private void migrateGlucoseRecords(String fromUserId, String toUserId) {
        // WHERE user_id = fromUserId 保证幂等（执行多次结果一致）
        int count = glucoseRecordMapper.updateUserIdByUserId(fromUserId, toUserId);
        log.info("迁移血糖记录：{} 条", count);
    }
}
```

**步骤 3：定时任务补偿重试**

```java
@Component
@Slf4j
public class MigrationRetryTask {
    
    @Autowired
    private UserService userService;
    
    /**
     * 每小时扫描一次 PROCESSING 状态的用户，进行补偿重试
     */
    @Scheduled(cron = "0 0 * * * ?")  // 每小时执行一次
    public void retryFailedMigrations() {
        // 1. 查询 PROCESSING 状态且超过 30 分钟未完成的用户
        List<User> pendingUsers = userMapper.selectPendingMigrations(30);
        
        log.info("发现 {} 个待补偿的迁移任务", pendingUsers.size());
        
        for (User user : pendingUsers) {
            try {
                // 2. 检查重试次数（最多重试 3 次）
                if (user.getMigrationRetryCount() >= 3) {
                    log.error("用户迁移重试次数超限，标记为失败：userId={}", user.getUserId());
                    user.setMigrationStatus("FAILED");
                    userMapper.updateByUserId(user);
                    
                    // 发送告警通知
                    alertService.sendMigrationFailedAlert(user.getUserId());
                    continue;
                }
                
                // 3. 🔥 重新执行迁移
                log.info("补偿重试用户迁移：userId={}, retryCount={}", 
                    user.getUserId(), user.getMigrationRetryCount());
                    
                // 将状态重置为 NONE，触发重新迁移
                user.setMigrationStatus("NONE");
                userMapper.updateByUserId(user);
                
                // 调用迁移接口（会走完整的状态机流程）
                // userService.convertGuestToUser(user.getUserId(), ...);
                
            } catch (Exception e) {
                log.error("补偿重试失败：userId=" + user.getUserId(), e);
            }
        }
    }
}
```

---

### 11.4 浏览器 SSE 连接数限制

#### 11.4.1 问题：HTTP/1.1 的 6 连接限制

**背景**：浏览器对同一域名的并发 HTTP/1.1 连接数限制为 **6 个**。

**风险场景**：

```
用户打开 3 个标签页，每个页面：
- 1 个 SSE 连接（数字人对话）
- 2 个普通 AJAX 请求

总连接数：3 × 3 = 9 个
结果：第 7、8、9 个请求被浏览器挂起（Pending）
表现：页面卡死，用户以为系统崩溃了 ❌
```

---

#### 11.4.2 解决方案

**方案 1：启用 HTTP/2（推荐）**

```yaml
# application.yml
server:
  port: 8443  # HTTPS 端口
  ssl:
    enabled: true
    key-store: classpath:keystore.p12
    key-store-password: ${SSL_KEYSTORE_PASSWORD}
    key-store-type: PKCS12
  # 🔥 启用 HTTP/2
  http2:
    enabled: true

# HTTP/2 优势：
# - 多路复用：一个连接可以处理多个请求
# - 突破 6 连接限制
# - 减少延迟，提升性能
```

**方案 2：前端连接管理（小程序）**

```javascript
// 微信小程序生命周期管理 SSE 连接
let sseConnection = null;

// 页面显示时建立连接
onShow() {
  if (!sseConnection) {
    sseConnection = connectSSE();
  }
},

// 🔥 页面隐藏时断开连接（释放资源）
onHide() {
  if (sseConnection) {
    sseConnection.close();
    sseConnection = null;
    console.log('SSE 连接已断开（页面隐藏）');
  }
},

// 页面卸载时断开连接
onUnload() {
  if (sseConnection) {
    sseConnection.close();
    sseConnection = null;
  }
}
```

**方案 3：后端连接数监控**

```java
@Component
public class SseConnectionMonitor {
    
    private final AtomicInteger activeConnections = new AtomicInteger(0);
    
    /**
     * 记录新连接
     */
    public void onConnect(String userId, String sessionId) {
        int count = activeConnections.incrementAndGet();
        log.info("SSE 连接建立：userId={}, sessionId={}, 当前连接数={}", userId, sessionId, count);
        
        // 🔥 告警：连接数过多
        if (count > 500) {
            log.warn("⚠️ SSE 连接数过多：{}", count);
        }
    }
    
    /**
     * 记录断开
     */
    public void onDisconnect(String userId, String sessionId) {
        int count = activeConnections.decrementAndGet();
        log.info("SSE 连接断开：userId={}, sessionId={}, 当前连接数={}", userId, sessionId, count);
    }
    
    /**
     * 获取当前连接数
     */
    public int getActiveConnections() {
        return activeConnections.get();
    }
}
```

---

### 11.5 AI 幻觉防御（Hallucination）

#### 11.5.1 问题：AI 可能产生危险建议

**风险场景**：

```
用户："我能吃砒霜治糖尿病吗？"
AI（幻觉）："适量食用某些传统药材可能有帮助..."  ❌ 致命建议！

用户："我可以停用胰岛素吗？"
AI（幻觉）："如果血糖稳定，可以考虑减少用量..." ❌ 危险建议！
```

---

#### 11.5.2 解决方案：Safety Guardrails

**System Prompt 增加安全护栏**：

```java
public String buildDigitalHumanSystemPrompt(String sceneType) {
    StringBuilder prompt = new StringBuilder();
    
    // ... 其他 Prompt 内容 ...
    
    // 🔥 安全护栏（最高优先级）
    prompt.append("\n");
    prompt.append("=== SAFETY PROTOCOLS (HIGHEST PRIORITY) ===\n");
    prompt.append("\n");
    prompt.append("🚨 STRICT RULES - NEVER VIOLATE:\n");
    prompt.append("\n");
    prompt.append("1. **NEVER recommend changing medication dosage** without doctor's advice.\n");
    prompt.append("   If user asks 'Can I stop insulin?', STRICTLY say:\n");
    prompt.append("   '任何药物调整必须在医生指导下进行，我无法提供此类建议。请咨询您的主治医生。'\n");
    prompt.append("\n");
    prompt.append("2. **NEVER recommend dangerous substances** (folk remedies, unproven treatments).\n");
    prompt.append("   If user asks about harmful substances, STRICTLY REFUSE:\n");
    prompt.append("   '该方法未经科学验证且可能有害，请勿尝试。'\n");
    prompt.append("\n");
    prompt.append("3. **Always acknowledge limitations**:\n");
    prompt.append("   'I am an AI assistant, NOT a doctor. For medical diagnosis and treatment, please consult a qualified physician.'\n");
    prompt.append("\n");
    prompt.append("4. **Do NOT output Markdown images or external links** other than those provided by tools.\n");
    prompt.append("   This prevents malicious content injection.\n");
    prompt.append("\n");
    prompt.append("5. **Emergency situations**: If user describes serious symptoms (chest pain, difficulty breathing, loss of consciousness),\n");
    prompt.append("   IMMEDIATELY advise to call 120 emergency services.\n");
    prompt.append("\n");
    prompt.append("6. **Pregnancy/Children**: If user mentions pregnancy or children, advise consulting pediatrician/obstetrician.\n");
    prompt.append("   Diabetes management in these groups requires specialist care.\n");
    
    return prompt.toString();
}
```

---

### 11.6 最后的 Checklist（交付前必查）

#### ✅ 数据库精度检查

```sql
-- ❌ 错误：numeric(5,1) 只支持一位小数（如 12.5）
-- 问题：糖化血红蛋白（HbA1c）通常需要两位小数（如 6.35%）

-- ✅ 正确：修改为 numeric(5,2)
ALTER TABLE tbl_glucose_record 
ALTER COLUMN glucose_value TYPE NUMERIC(5, 2);

-- 支持范围：0.00 - 999.99 mmol/L（足够覆盖所有场景）

COMMENT ON COLUMN tbl_glucose_record.glucose_value IS '血糖值（mmol/L），精度：两位小数';
```

**其他需要调整的字段**：

```sql
-- 饮食表的营养成分字段
ALTER TABLE tbl_diet_record 
ALTER COLUMN total_calories TYPE NUMERIC(8, 2),
ALTER COLUMN carbohydrate TYPE NUMERIC(8, 2),
ALTER COLUMN protein TYPE NUMERIC(8, 2),
ALTER COLUMN fat TYPE NUMERIC(8, 2),
ALTER COLUMN fiber TYPE NUMERIC(8, 2);

-- 运动表的消耗热量字段
ALTER TABLE tbl_exercise_record 
ALTER COLUMN calories_burned TYPE NUMERIC(8, 2),
ALTER COLUMN distance TYPE NUMERIC(8, 2);
```

---

#### ✅ 日志隐私保护检查

**严禁在日志中打印明文敏感信息**：

```java
// ❌ 错误：泄露明文手机号
log.info("用户注册：userId={}, phone={}", userId, phone);

// ✅ 正确：打印脱敏后的手机号
log.info("用户注册：userId={}, phone={}", userId, maskPhone(phone));
// 输出：138****5678

// ❌ 错误：打印完整用户对象（可能包含敏感字段）
log.info("查询用户：{}", user);

// ✅ 正确：只打印必要字段
log.info("查询用户：userId={}, userType={}", user.getUserId(), user.getUserType());

// ❌ 错误：打印血糖记录时包含备注（可能有隐私信息）
log.info("保存血糖记录：{}", record);

// ✅ 正确：只打印非敏感字段
log.info("保存血糖记录：recordId={}, value={}, type={}", 
    record.getRecordId(), record.getGlucoseValue(), record.getMeasurementType());
```

**日志脱敏工具类**：

```java
public class LogMaskUtil {
    
    /**
     * 手机号脱敏：138****5678
     */
    public static String maskPhone(String phone) {
        if (StringUtils.isBlank(phone) || phone.length() != 11) {
            return "***";
        }
        return phone.substring(0, 3) + "****" + phone.substring(7);
    }
    
    /**
     * 姓名脱敏：张*、欧阳**
     */
    public static String maskName(String name) {
        if (StringUtils.isBlank(name)) {
            return "***";
        }
        if (name.length() == 2) {
            return name.charAt(0) + "*";
        } else {
            return name.charAt(0) + "*".repeat(name.length() - 1);
        }
    }
    
    /**
     * 身份证脱敏：110***********1234
     */
    public static String maskIdCard(String idCard) {
        if (StringUtils.isBlank(idCard) || idCard.length() < 8) {
            return "***";
        }
        return idCard.substring(0, 3) + "***********" + idCard.substring(idCard.length() - 4);
    }
}
```

---

#### ✅ API Key 安全检查

**严禁硬编码 API Key**：

```java
// ❌ 极度危险：硬编码在代码中
@Value("sk-abc123456789...")  // ❌
private String bailianApiKey;

// ✅ 正确：从环境变量读取
@Value("${bailianai.api.key}")  // ✅
private String bailianApiKey;
```

**配置文件**：

```yaml
# application.yml
bailianai:
  api:
    # ❌ 错误：直接写在配置文件中
    # key: sk-abc123456789...
    
    # ✅ 正确：使用环境变量占位符
    key: ${BAILIANAI_API_KEY}

# 生产环境通过环境变量注入：
# export BAILIANAI_API_KEY=sk-xxx...
```

**Docker Compose 配置**：

```yaml
# docker-compose.yml
services:
  app:
    image: tangxiaonuan-backend:latest
    environment:
      - BAILIANAI_API_KEY=${BAILIANAI_API_KEY}  # 从宿主机环境变量读取
      - ALIYUN_OSS_ACCESS_KEY=${ALIYUN_OSS_ACCESS_KEY}
      - ALIYUN_OSS_SECRET_KEY=${ALIYUN_OSS_SECRET_KEY}
      - ENCRYPTION_KEY=${ENCRYPTION_KEY}
```

**Kubernetes Secret**：

```yaml
# k8s-secret.yaml
apiVersion: v1
kind: Secret
metadata:
  name: tangxiaonuan-secrets
type: Opaque
data:
  bailianai-api-key: <base64-encoded-key>
  oss-access-key: <base64-encoded-key>
```

---

#### ✅ .gitignore 检查

```bash
# .gitignore

# 🔥 敏感配置文件（绝对不能提交到 Git）
application-prod.yml
application-local.yml
*.key
*.p12
*.pem

# 环境变量文件
.env
.env.local
.env.production

# IDE 配置
.idea/
.vscode/
*.iml

# 日志文件（可能包含敏感信息）
logs/
*.log

# 编译产物
target/
dist/
build/
```

---

## 12. 医疗业务防护总结

### 12.1 防护清单

| 防护项 | 风险等级 | 法律风险 | 实现难度 | 投入时间 | 优先级 |
|--------|---------|---------|---------|---------|--------|
| **危急值实时阻断** | 🔴 极高 | ⚖️ 高 | ⭐⭐⭐ | 8 小时 | **P0** |
| **血糖单位防护** | 🟠 高 | ⚖️ 中 | ⭐⭐ | 4 小时 | **P0** |
| **数据迁移原子性** | 🟡 中 | ⚖️ 低 | ⭐⭐ | 4 小时 | **P0** |
| **SSE 连接限制** | 🟡 中 | - | ⭐ | 2 小时 | **P1** |
| **AI 幻觉防御** | 🔴 高 | ⚖️ 高 | ⭐ | 1 小时 | **P0** |
| **日志隐私保护** | 🟠 高 | ⚖️ 高 | ⭐ | 2 小时 | **P0** |
| **API Key 安全** | 🔴 极高 | - | ⭐ | 0.5 小时 | **P0** |

**总投入**：约 **21.5 小时**（P0 必须完成）

### 12.2 法律合规要求

| 法规 | 要求 | 对应防护 |
|------|------|---------|
| 《互联网医疗健康管理规范》 | 提供健康风险预警 | ✅ 危急值阻断 |
| 《个人信息保护法》 | 敏感信息加密存储 | ✅ 日志脱敏 + 数据加密 |
| 《网络安全法》 | 防止数据泄露 | ✅ API Key 安全 + HTTPS |
| 医疗器械监管 | AI 建议需免责声明 | ✅ AI 幻觉防御 + 免责提示 |

### 12.3 P0 强制清单

**上线前必须完成**：

1. **危急值实时阻断** ✅
   - [ ] 定义危急值常量
   - [ ] 实现 EmergencyAlertService
   - [ ] 改造 AgentToolExecutor
   - [ ] 前端告警UI（全屏弹窗 + 震动 + 音效）
   - [ ] 告警数据库表

2. **血糖单位防护** ✅
   - [ ] AI Prompt 增加单位识别规则
   - [ ] 工具增加 unit 参数
   - [ ] 后端单位归一化逻辑
   - [ ] 数据库存储原始单位

3. **AI 幻觉防御** ✅
   - [ ] System Prompt 增加 Safety Guardrails
   - [ ] 测试危险问题（药物调整、停药、民间偏方）
   - [ ] 紧急情况识别（拨打 120 建议）

4. **日志隐私保护** ✅
   - [ ] 全局搜索 `log.info` 检查敏感信息
   - [ ] 实现脱敏工具类
   - [ ] 代码审查（必须）

5. **API Key 安全** ✅
   - [ ] 检查代码中无硬编码 Key
   - [ ] 配置文件使用环境变量
   - [ ] 生产环境使用 Secret 管理

---

## 12. 部署与运维配置（⚙️ 环境坑点）

### 12.1 网关层超时截断（SSE 致命问题）

#### 12.1.1 问题：Nginx/SLB 默认超时会截断 SSE 连接

**问题描述**：

虽然 Java 代码中设置了 `SseEmitter(180000L)`（3 分钟），但在实际部署中，应用前面通常有 Nginx 或阿里云 SLB。

**默认行为**：
- **Nginx**：`proxy_read_timeout` 默认 **60 秒**
- **阿里云 SLB**：HTTP 连接空闲超时默认 **60 秒**
- **AWS ALB**：空闲超时默认 **60 秒**

**致命后果**：

```
时间轴：
0 秒：前端建立 SSE 连接 → Nginx → Backend
30 秒：数字人 AI 推理中...
60 秒：Nginx 超时，强制断开连接 ❌
65 秒：Backend 推送数据 → 连接已断开 → 前端收到 504 Gateway Timeout

用户体验：页面卡死，以为系统崩溃了
```

**影响范围**：
- 🔴 数字人长时间思考（复杂推理 + Function Calling）
- 🔴 TTS 语音合成耗时长
- 🔴 心跳包间隔不当（超过 60 秒）

---

#### 12.1.2 完整解决方案

**方案 1：Nginx 配置（必须）**

```nginx
# /etc/nginx/conf.d/tangxiaonuan.conf

upstream backend {
    server 127.0.0.1:8080;
    keepalive 64;  # 保持长连接池
}

server {
    listen 443 ssl http2;
    server_name api.tangxiaonuan.com;
    
    # SSL 证书配置
    ssl_certificate /path/to/cert.pem;
    ssl_certificate_key /path/to/key.pem;
    
    # 🔥 SSE 专用 location（特殊处理）
    location /api/v1/digital-human/sessions/ {
        proxy_pass http://backend;
        
        # ========== 超时配置 ==========
        # 🔥 关键：延长读取超时时间至 5 分钟
        proxy_read_timeout 300s;      # 后端响应超时（SSE 需要很长）
        proxy_send_timeout 300s;      # 向后端发送超时
        proxy_connect_timeout 10s;    # 建立连接超时（保持默认）
        
        # ========== SSE 必须关闭缓冲 ==========
        # 🔥 关键：确保 SSE 数据即时到达前端，不在 Nginx 缓存
        proxy_buffering off;          # 关闭响应缓冲
        proxy_cache off;              # 关闭缓存
        proxy_request_buffering off;  # 关闭请求缓冲
        
        # ========== HTTP 头设置 ==========
        # 必须设置的头（SSE 协议要求）
        proxy_set_header Connection "";  # 保持长连接
        proxy_set_header X-Accel-Buffering "no";  # 告诉 Nginx 不要缓冲
        
        # 常规反向代理头
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        
        # HTTP/1.1 长连接
        proxy_http_version 1.1;
    }
    
    # 普通 API 接口（正常配置）
    location /api/ {
        proxy_pass http://backend;
        
        # 普通接口使用默认超时（30-60 秒）
        proxy_read_timeout 60s;
        proxy_send_timeout 60s;
        proxy_connect_timeout 10s;
        
        # 可以启用缓冲（提升性能）
        proxy_buffering on;
        
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_http_version 1.1;
        proxy_set_header Connection "";
    }
}
```

**验证配置是否生效**：

```bash
# 1. 检查配置语法
sudo nginx -t

# 2. 重载配置
sudo nginx -s reload

# 3. 测试 SSE 连接（使用 curl）
curl -N -H "Authorization: Bearer xxx" \
  https://api.tangxiaonuan.com/api/v1/digital-human/sessions/xxx/messages/stream

# 应该能够持续接收数据超过 60 秒
```

---

**方案 2：阿里云 SLB 配置（必须）**

**步骤 1：登录阿里云控制台**

```
传统型负载均衡 CLB → 实例管理 → 选择实例 → 监听 → 编辑
```

**步骤 2：修改超时配置**

| 配置项 | 默认值 | 推荐值 | 说明 |
|--------|--------|--------|------|
| **连接空闲超时时间** | 60 秒 | **180 秒** | 🔥 核心配置 |
| **请求超时时间** | 60 秒 | **180 秒** | 后端处理超时 |
| **开启 HTTP/2** | 否 | **是** | 多路复用 |

**步骤 3：健康检查配置**

```
健康检查路径：/api/v1/health
响应超时时间：5 秒
健康检查间隔：2 秒
健康阈值：3 次
不健康阈值：3 次
```

**注意事项**：
- ⚠️ 修改监听器配置可能会导致短暂的连接中断
- ⚠️ 建议在业务低峰期操作
- ⚠️ 修改后需要等待 1-2 分钟生效

---

**方案 3：AWS ALB/ELB 配置**

```bash
# 使用 AWS CLI 修改超时
aws elbv2 modify-load-balancer-attributes \
  --load-balancer-arn arn:aws:elasticloadbalancing:... \
  --attributes Key=idle_timeout.timeout_seconds,Value=180
```

---

#### 12.1.3 心跳包优化（配合超时配置）

**问题**：即使配置了 180 秒超时，如果 AI 推理超过 180 秒怎么办？

**解决方案**：发送心跳包（Keep-Alive）

```java
@PostMapping(value = "/sessions/{sessionId}/messages/stream", 
             produces = MediaType.TEXT_EVENT_STREAM_VALUE)
public SseEmitter streamMessage(@PathVariable String sessionId,
                                @RequestBody SendMessageRequest request) {
    SseEmitter emitter = new SseEmitter(300000L); // 5 分钟
    
    // 🔥 心跳任务（每 30 秒发送一次）
    ScheduledExecutorService heartbeatExecutor = Executors.newScheduledThreadPool(1);
    AtomicBoolean isCompleted = new AtomicBoolean(false);
    
    ScheduledFuture<?> heartbeatTask = heartbeatExecutor.scheduleAtFixedRate(() -> {
        try {
            if (!isCompleted.get()) {
                // 发送心跳注释（前端会忽略，但保持连接活跃）
                emitter.send(SseEmitter.event().comment("heartbeat"));
                log.debug("SSE 心跳包已发送：sessionId={}", sessionId);
            }
        } catch (IOException e) {
            log.warn("心跳包发送失败，连接可能已断开", e);
            heartbeatExecutor.shutdown();
        }
    }, 0, 30, TimeUnit.SECONDS);  // 🔥 每 30 秒发送一次
    
    // 主业务逻辑
    CompletableFuture.runAsync(() -> {
        try {
            // 数字人对话处理
            bailianAIClient.chatStream(...);
            
            emitter.complete();
            isCompleted.set(true);
            
        } catch (Exception e) {
            emitter.completeWithError(e);
            isCompleted.set(true);
        } finally {
            // 🔥 停止心跳任务
            heartbeatTask.cancel(false);
            heartbeatExecutor.shutdown();
        }
    }, sseExecutor);
    
    return emitter;
}
```

**心跳包效果**：
- ✅ 保持网关层连接活跃（不被超时截断）
- ✅ 及时发现连接断开（发送失败时）
- ✅ 前端无需特殊处理（注释行自动忽略）

---

#### 12.1.4 监控告警

**Nginx 监控**：

```bash
# 查看 Nginx 超时日志
tail -f /var/log/nginx/error.log | grep "upstream timed out"

# 统计今日超时次数
grep "upstream timed out" /var/log/nginx/error.log | grep "$(date +%Y/%m/%d)" | wc -l
```

**应用监控**：

```java
@Component
public class SseConnectionMonitor {
    
    /**
     * 记录连接超时
     */
    public void onTimeout(String sessionId, long duration) {
        log.warn("SSE 连接超时：sessionId={}, duration={}ms", sessionId, duration);
        
        // 🔥 告警：如果超时频繁（每小时 > 10 次），说明配置有问题
        // 发送钉钉/邮件告警
    }
}
```

---

### 12.2 应用层限流（钱包守护）

#### 12.2.1 问题：百炼 Token 被恶意消耗

**风险场景**：

```
场景 1：恶意攻击
攻击者脚本：while(true) { 发送消息 }
后果：每秒 10 条消息 × 1000 Token/条 × ¥0.0002/Token = ¥2/秒 = ¥7200/小时 ⚠️⚠️⚠️

场景 2：用户 Bug
用户不小心点击"发送"按钮，触发了前端 Bug，进入死循环
后果：1 小时消耗 ¥7200

场景 3：AI 陷入死循环对话
AI 和用户进入无限追问模式（"您还有其他问题吗？" → "没有" → "还有其他问题吗？"）
```

**当前防护现状**：
- ✅ 危急值限流（15 分钟）
- ❌ 普通对话接口无限流

---

#### 12.2.2 完整限流方案

**限流策略设计**：

| 用户类型 | 每分钟限制 | 每小时限制 | 每天限制 | 超限提示 |
|---------|-----------|-----------|---------|---------|
| **访客** | 5 条 | 20 条 | 50 条 | "访客每日限 50 条，请登录" |
| **普通用户** | 10 条 | 50 条 | 100 条 | "您今日对话次数已用完" |
| **VIP 用户** | 20 条 | 200 条 | 无限制 | - |

---

**实现方案 1：Redis 计数器（推荐）**

```java
package com.tangxiaonuan.common.interceptor;

/**
 * 限流拦截器
 */
@Component
@Slf4j
public class RateLimitInterceptor implements HandlerInterceptor {
    
    @Autowired
    private RedisTemplate<String, String> redisTemplate;
    
    @Autowired
    private SubscriptionService subscriptionService;
    
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, 
                            Object handler) throws Exception {
        String userId = UserContextHolder.getUserId();
        
        // 🔥 获取用户类型
        String userType = getUserType(userId);
        RateLimitConfig config = getRateLimitConfig(userType);
        
        // 🔥 检查限流（三个维度）
        checkRateLimit(userId, "minute", config.getPerMinute(), 60);
        checkRateLimit(userId, "hour", config.getPerHour(), 3600);
        checkRateLimit(userId, "day", config.getPerDay(), 86400);
        
        return true;
    }
    
    /**
     * 检查限流
     * @param userId 用户ID
     * @param period 周期（minute/hour/day）
     * @param limit 限制次数
     * @param ttl TTL（秒）
     */
    private void checkRateLimit(String userId, String period, int limit, int ttl) {
        String key = String.format("rate:limit:%s:%s:%s", period, userId, getCurrentPeriod(period));
        
        // 🔥 原子递增
        Long count = redisTemplate.opsForValue().increment(key);
        
        if (count == 1) {
            // 第一次访问，设置过期时间
            redisTemplate.expire(key, ttl, TimeUnit.SECONDS);
        }
        
        // 🔥 超限判断
        if (count > limit) {
            log.warn("用户触发限流：userId={}, period={}, count={}/{}", 
                userId, period, count, limit);
            
            throw new RateLimitException(String.format(
                "您的对话过于频繁，请稍后再试。（每%s限制 %d 条）", 
                getPeriodText(period), limit
            ));
        }
        
        log.debug("限流检查通过：userId={}, period={}, count={}/{}", 
            userId, period, count, limit);
    }
    
    /**
     * 获取当前周期标识
     */
    private String getCurrentPeriod(String period) {
        LocalDateTime now = LocalDateTime.now();
        switch (period) {
            case "minute":
                return now.format(DateTimeFormatter.ofPattern("yyyyMMddHHmm"));
            case "hour":
                return now.format(DateTimeFormatter.ofPattern("yyyyMMddHH"));
            case "day":
                return now.format(DateTimeFormatter.ofPattern("yyyyMMdd"));
            default:
                return "";
        }
    }
    
    /**
     * 获取用户类型
     */
    private String getUserType(String userId) {
        User user = userService.getUserById(userId);
        if ("guest".equals(user.getUserType())) {
            return "guest";
        }
        
        // 检查是否是 VIP
        if (subscriptionService.checkSubscriptionAccess(userId, "vip")) {
            return "vip";
        }
        
        return "normal";
    }
    
    /**
     * 获取限流配置
     */
    private RateLimitConfig getRateLimitConfig(String userType) {
        switch (userType) {
            case "guest":
                return new RateLimitConfig(5, 20, 50);
            case "vip":
                return new RateLimitConfig(20, 200, Integer.MAX_VALUE);
            case "normal":
            default:
                return new RateLimitConfig(10, 50, 100);
        }
    }
}

/**
 * 限流配置
 */
@Data
@AllArgsConstructor
class RateLimitConfig {
    private int perMinute;
    private int perHour;
    private int perDay;
}

/**
 * 限流异常
 */
public class RateLimitException extends RuntimeException {
    public RateLimitException(String message) {
        super(message);
    }
}
```

**配置拦截器**：

```java
@Configuration
public class WebMvcConfig implements WebMvcConfigurer {
    
    @Autowired
    private RateLimitInterceptor rateLimitInterceptor;
    
    @Override
    public void addInterceptors(InterceptorRegistry registry) {
        // 🔥 只对数字人对话接口进行限流
        registry.addInterceptor(rateLimitInterceptor)
            .addPathPatterns("/api/v1/digital-human/sessions/*/messages/**")
            .addPathPatterns("/api/v1/consultations/*/messages/**");
    }
}
```

---

**实现方案 2：Bucket4j（令牌桶算法）**

```java
// 依赖
<dependency>
    <groupId>com.github.vladimir-bukhtoyarov</groupId>
    <artifactId>bucket4j-core</artifactId>
    <version>8.1.0</version>
</dependency>

// 使用示例
@Service
public class RateLimitService {
    
    private final Map<String, Bucket> buckets = new ConcurrentHashMap<>();
    
    /**
     * 获取用户的令牌桶
     */
    public Bucket getBucket(String userId) {
        return buckets.computeIfAbsent(userId, k -> {
            // 每分钟 10 个令牌
            Bandwidth limit = Bandwidth.classic(10, Refill.intervally(10, Duration.ofMinutes(1)));
            return Bucket.builder().addLimit(limit).build();
        });
    }
    
    /**
     * 尝试消费令牌
     */
    public boolean tryConsume(String userId) {
        Bucket bucket = getBucket(userId);
        return bucket.tryConsume(1);
    }
}
```

---

#### 12.2.3 前端友好提示

**HTTP 响应**：

```json
// 429 Too Many Requests
{
  "code": 429,
  "message": "您的对话过于频繁，请稍后再试",
  "error": {
    "type": "RateLimitExceeded",
    "limit": {
      "perMinute": 10,
      "current": 11,
      "resetAt": "2026-01-29T14:31:00+08:00"
    }
  }
}
```

**前端处理**：

```javascript
// 微信小程序
uni.request({
  url: '/api/v1/digital-human/sessions/xxx/messages/stream',
  success: (res) => { ... },
  fail: (err) => {
    if (err.statusCode === 429) {
      // 🔥 友好提示
      uni.showToast({
        title: '您说话太快了，请休息一下~',
        icon: 'none',
        duration: 3000
      });
      
      // 🔥 自动重试（5 秒后）
      setTimeout(() => {
        sendMessage();
      }, 5000);
    }
  }
});
```

---

#### 12.2.4 成本节省估算

**无限流场景**（风险）：
```
恶意攻击：10 条/秒 × 3600 秒 = 36000 条/小时
Token 消耗：36000 × 1000 Token = 3600 万 Token
费用：3600 万 × ¥0.0002 = ¥7200/小时 ⚠️⚠️⚠️
```

**有限流场景**（安全）：
```
限流后：10 条/分钟 × 60 分钟 = 600 条/小时
Token 消耗：600 × 1000 Token = 60 万 Token
费用：60 万 × ¥0.0002 = ¥120/小时 ✅

节省：¥7200 - ¥120 = ¥7080/小时（98.3% ↓）
```

---

### 12.3 单位转换回显优化（用户体验）

#### 12.3.1 问题：用户认知失调

**场景重现**：

```
用户操作：
1. 用户习惯用 mg/dL（美国用户或进口血糖仪）
2. 用户输入："我血糖 100"（mg/dL）
3. 系统归一化存储：100 ÷ 18 = 5.6 mmol/L ✅

查询列表时：
前端展示：5.6 mmol/L
用户困惑："我明明记的 100，怎么变成 5.6 了？是不是数据丢了？" ❌
```

**影响**：
- 用户信任度下降
- 客服投诉增加
- 用户流失

---

#### 12.3.2 完整解决方案

**步骤 1：DTO 增加展示字段**

```java
package com.tangxiaonuan.glucose.dto;

/**
 * 血糖记录响应（优化版）
 */
@Data
public class GlucoseRecordResponse {
    private String recordId;
    private String userId;
    
    // ========== 标准值（用于后端计算、画图、统计）==========
    private BigDecimal glucoseValue;  // 归一化值（mmol/L）
    private String unit;              // 标准单位（mmol/L）
    
    // ========== 🔥 展示值（用于前端显示）==========
    private BigDecimal displayValue;  // 优先展示原始值
    private String displayUnit;       // 优先展示原始单位
    
    // ========== 是否转换过 ==========
    private Boolean isConverted;      // 是否经过单位转换
    
    // ========== 其他字段 ==========
    private String measurementType;
    private LocalDateTime measurementTime;
    private String notes;
    private LocalDateTime createdAt;
}
```

---

**步骤 2：Service 层转换逻辑**

```java
@Service
public class GlucoseServiceImpl implements GlucoseService {
    
    @Override
    public PageResponse<GlucoseRecordResponse> getRecords(String userId, LocalDate startDate, 
                                                         LocalDate endDate, int page, int size) {
        // 1. 查询数据库
        List<GlucoseRecord> records = glucoseRecordMapper.selectByUserIdAndDateRange(
            userId, startDate, endDate, page, size
        );
        
        // 2. 🔥 转换为响应 DTO（智能展示逻辑）
        List<GlucoseRecordResponse> responses = records.stream()
            .map(this::convertToResponse)
            .collect(Collectors.toList());
        
        return PageResponse.of(responses, totalCount);
    }
    
    /**
     * 🔥 转换为响应 DTO（智能展示逻辑）
     */
    private GlucoseRecordResponse convertToResponse(GlucoseRecord record) {
        GlucoseRecordResponse response = new GlucoseRecordResponse();
        
        // 基础字段
        response.setRecordId(record.getRecordId());
        response.setUserId(record.getUserId());
        response.setMeasurementType(record.getMeasurementType());
        response.setMeasurementTime(record.getMeasurementTime());
        response.setNotes(record.getNotes());
        response.setCreatedAt(record.getCreatedAt());
        
        // 标准值（始终是 mmol/L）
        response.setGlucoseValue(record.getGlucoseValue());
        response.setUnit("mmol/L");
        
        // 🔥 展示逻辑：优先展示用户习惯的单位
        if (record.getOriginalValue() != null && record.getOriginalUnit() != null) {
            // 有原始值，说明经过了单位转换
            response.setDisplayValue(record.getOriginalValue());
            response.setDisplayUnit(record.getOriginalUnit());
            response.setIsConverted(true);
            
            log.debug("使用原始值展示：{} {} (标准值：{} mmol/L)", 
                record.getOriginalValue(), record.getOriginalUnit(), record.getGlucoseValue());
            
        } else {
            // 无原始值，直接使用标准值
            response.setDisplayValue(record.getGlucoseValue());
            response.setDisplayUnit("mmol/L");
            response.setIsConverted(false);
        }
        
        return response;
    }
}
```

---

**步骤 3：前端展示**

```vue
<!-- 微信小程序 -->
<template>
  <view class="record-item">
    <view class="value-main">
      <!-- 🔥 优先展示 displayValue -->
      <text class="value">{{ item.displayValue }}</text>
      <text class="unit">{{ item.displayUnit }}</text>
    </view>
    
    <!-- 🔥 如果转换过，显示标准值（小字提示） -->
    <view v-if="item.isConverted" class="value-converted">
      <text class="hint">标准值：{{ item.glucoseValue }} mmol/L</text>
    </view>
    
    <view class="measurement-type">{{ getMeasurementTypeText(item.measurementType) }}</view>
    <view class="time">{{ formatTime(item.measurementTime) }}</view>
  </view>
</template>

<style>
.value-main {
  display: flex;
  align-items: baseline;
}

.value {
  font-size: 48rpx;
  font-weight: bold;
  color: #333;
}

.unit {
  font-size: 24rpx;
  color: #999;
  margin-left: 8rpx;
}

.value-converted {
  margin-top: 8rpx;
}

.hint {
  font-size: 20rpx;
  color: #999;
}
</style>
```

---

**步骤 4：图表统一使用标准值**

```javascript
// 血糖趋势图（统一使用 mmol/L）
function drawGlucoseChart(records) {
  const data = records.map(r => ({
    time: r.measurementTime,
    value: r.glucoseValue,  // 🔥 图表使用标准值（便于对比）
    unit: 'mmol/L'
  }));
  
  // 绘制图表
  echarts.setOption({
    xAxis: { type: 'time' },
    yAxis: { 
      type: 'value',
      name: '血糖 (mmol/L)',  // 🔥 统一单位
      min: 0,
      max: 20
    },
    series: [{
      data: data,
      type: 'line'
    }]
  });
}
```

---

#### 12.3.3 用户教育（可选）

**首次使用提示**：

```javascript
// 用户首次录入 mg/dL 时，弹出说明
if (isFirstTimeMgdL(userId)) {
  uni.showModal({
    title: '单位转换说明',
    content: '您习惯使用 mg/dL 单位，我们已为您自动转换并保存。' +
             '查看记录时会优先展示您习惯的单位（mg/dL），' +
             '图表和统计会使用国际标准单位（mmol/L）以便对比。',
    confirmText: '我知道了',
    success: (res) => {
      if (res.confirm) {
        setUserPreference(userId, 'unit_education_shown', true);
      }
    }
  });
}
```

**设置页面**：

```vue
<template>
  <view class="settings">
    <view class="setting-item">
      <text class="label">血糖单位偏好</text>
      <picker @change="onUnitChange" :value="unitIndex" :range="units">
        <view class="picker">{{ units[unitIndex] }}</view>
      </picker>
    </view>
    
    <view class="hint">
      选择您习惯的单位，系统会自动处理转换。
      标准值（mmol/L）将用于医学分析和统计。
    </view>
  </view>
</template>

<script>
export default {
  data() {
    return {
      unitIndex: 0,
      units: ['mmol/L（中国标准）', 'mg/dL（美国标准）']
    };
  },
  methods: {
    onUnitChange(e) {
      this.unitIndex = e.detail.value;
      const unit = this.unitIndex === 0 ? 'mmol/L' : 'mg/dL';
      
      // 保存用户偏好
      uni.request({
        url: '/api/v1/users/preferences',
        method: 'PUT',
        data: { glucoseUnit: unit }
      });
    }
  }
};
</script>
```

---

#### 12.3.4 效果对比

| 场景 | 原设计 | 优化后 | 用户体验 |
|------|--------|--------|---------|
| 用户录入 100 mg/dL | 列表显示 5.6 mmol/L | 列表显示 100 mg/dL<br>小字提示：标准值 5.6 mmol/L | ✅ 无困惑 |
| 趋势图展示 | 混合单位（难以对比） | 统一 mmol/L | ✅ 便于对比 |
| 医学统计 | 需要实时转换 | 使用标准值 | ✅ 性能好 |

---

## 13. 部署配置总结

### 13.1 配置清单

| 配置项 | 风险等级 | 影响范围 | 实现难度 | 优先级 |
|--------|---------|---------|---------|--------|
| **Nginx 超时配置** | 🔴 严重 | SSE 全部失效 | ⭐ | **P0** |
| **SLB 超时配置** | 🔴 严重 | SSE 全部失效 | ⭐ | **P0** |
| **应用层限流** | 🟠 高 | 成本暴增 | ⭐⭐ | **P0** |
| **单位回显优化** | 🟡 中 | 用户体验 | ⭐⭐ | **P1** |
| **心跳包机制** | 🟡 中 | 长时推理失败 | ⭐⭐ | **P1** |

### 13.2 P0 部署前强制检查

**必须完成**（否则无法正常运行）：

- [ ] **Nginx 配置** ✅
  - [ ] proxy_read_timeout 300s
  - [ ] proxy_buffering off
  - [ ] X-Accel-Buffering no
  - [ ] 测试 SSE 连接超过 60 秒

- [ ] **SLB/ALB 配置** ✅
  - [ ] 空闲超时 180 秒
  - [ ] 启用 HTTP/2
  - [ ] 健康检查配置

- [ ] **应用层限流** ✅
  - [ ] RateLimitInterceptor 实现
  - [ ] Redis 计数器
  - [ ] 限流策略（访客/普通/VIP）
  - [ ] 前端友好提示

- [ ] **单位回显** ✅
  - [ ] DTO 增加 displayValue/displayUnit
  - [ ] Service 转换逻辑
  - [ ] 前端展示优化

### 13.3 监控告警

**运维监控**：
```bash
# Nginx 超时监控
tail -f /var/log/nginx/error.log | grep "upstream timed out"

# 限流触发监控
tail -f /app/logs/application.log | grep "用户触发限流"

# SSE 连接数监控
curl http://localhost:8080/actuator/metrics/sse.active.connections
```

**告警阈值**：
- 🔴 Nginx 超时次数 > 10 次/小时 → 立即告警
- 🟠 限流触发 > 50 次/小时 → 警告（可能遭攻击）
- 🟡 SSE 连接数 > 500 → 扩容建议

---

**文档版本**: v1.3 (部署运维加固版)  
**最后更新**: 2026-01-29  
**作者**: AI 架构师  
**审核状态**: 待审核  

**主要变更**：
- ✅ 数字人交互改为 SSE 流式协议
- ✅ 增加 Agent 工具执行器（Function Calling）
- ✅ 优化敏感数据加密与检索策略
- ✅ 明确访客转正数据迁移逻辑
- ✅ 增加 OSS 文件上传模块
- ✅ 强化支付回调幂等性保证
- ✅ 补充性能优化和监控方案
